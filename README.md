...
- Concise cleaner tasks are tied separately to the speech and photo notes that generated them; later source changes mark the bullets out of date and block submission until the customer summarises again, while manual review edits to the current bullets remain allowed
- Live scan-readiness guidance tracks eight required items as the customer works: connected request details, one-to-ten photos, room labels and notes, instructions, concise tasks, photographed-room coverage, final-scope confirmation and photo-sharing permission; it names each photographed room still missing a cleaner task, the action explains every outstanding check when clicked, and the server independently repeats every security-critical check
- Genuine room-scan submission shows per-photo and saving progress, has bounded photo-preparation and request timeouts, and restores an actionable button with the customer's entries preserved if an upload fails
