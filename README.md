...
- One-click private handoff from a completed request into the required room scan, with reference and email prefilled for that browser session; the private customer tracker can also resume the scan on another device through its fragment token without repeating or exposing the email, while the server rechecks that token against the exact request
