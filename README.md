...
- Focused cleaner intake at `/join` that removes the customer form and separates basic details, work/travel preferences and availability/confirmations into three short stages while preserving mandatory service selection, matchable postcode coverage, right-to-work confirmation and consent
- High-entropy private cleaner tracker created with each new application; it reports only evidence-backed receipt, screening, approval, exact-availability and opportunity-readiness stages, lets a screened approved cleaner submit retry-safe exact future windows for confirmation, never promises work, removes its token from the URL before loading and exposes no identity, contact, travel or screening-note data
