...
- Uber-simple customer intake at `/request` that removes the marketing page and cleaner application, presents the complete server-validated request as three short stages—job, timing/access and contact—keeps entries when moving back, keeps each new step title visible below the sticky phone header, and only unlocks the required room scan after the request is created
...
- Focused cleaner intake at `/join` that removes the customer form and separates basic details, work/travel preferences and availability/confirmations into three short stages while preserving mandatory service selection, matchable postcode coverage, right-to-work confirmation and consent; a successful application continues automatically into its private tracker with a visible recovery button
...
- Audited human review decisions for room scans; approval requires the founder to load every private visual and room note and separately confirm the concise checklist was reconciled against all spoken notes, room notes and shown rooms. Missing evidence fails closed across matching, quoting and booking
...
- Prioritised founder-action dispatch queue derived from recorded scan, matching, offer, booking and safety state; urgent safety reports, rematching, booking finalisation and overdue follow-ups are surfaced without sending messages or changing records. A required scan, or a requested scan revision, that remains pending for 24 hours is promoted from passive monitoring to founder attention without inferring scope or contacting the customer automatically
