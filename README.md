...
- Uber-simple customer intake that presents the complete server-validated request as three short stages—job, timing/access and contact—keeps entries when moving back, keeps each new step title visible below the sticky phone header, and only unlocks the required room scan after the request is created
