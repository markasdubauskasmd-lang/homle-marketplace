...
-- Allow each insurance policy selected during onboarding to keep its own
-- encrypted evidence document. Existing document types remain supported for
-- backwards compatibility with applications already in progress.
...
BEGIN
  ...
  IF NOT (
    ...
    OR (supplied_section='insurance' AND supplied_document_type IN (
