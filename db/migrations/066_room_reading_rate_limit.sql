-- Reading a captured room photograph calls the same metered provider as the
-- walkthrough summary, and costs more per call because it carries an image.
-- Without its own reviewed policy an authenticated Landlord could replay their
-- session and drive unbounded provider spend one photo at a time.
--
-- 40 in 15 minutes covers a generous walkthrough of a large home with retries,
-- while capping a replay loop at a bounded number of image reads.
--
-- (Original note for the walkthrough summary scope below.)
...
ALTER TABLE tideway_private.request_rate_limits ADD CONSTRAINT request_rate_limits_scope_check CHECK (scope IN (
  ...
  'marketplace-landlord:scan-summary','marketplace-landlord:room-reading'
));
