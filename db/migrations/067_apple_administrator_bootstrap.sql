...
-- Migration 060 added Apple as a verified authentication provider after the
-- first-Administrator bootstrap function was locked in migration 034. Replace
-- the function forward-only so an active verified Apple-only founder account
-- is eligible under the same owner-only, first-only and audited boundary.
CREATE OR REPLACE FUNCTION tideway_private.provision_bootstrap_administrator(
...
BEGIN
  ...
  WHERE identity.user_id = target_account.id
    AND identity.provider IN ('password','google','apple','facebook')
  FOR UPDATE;
