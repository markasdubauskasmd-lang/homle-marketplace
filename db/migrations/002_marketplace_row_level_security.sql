...
CREATE POLICY properties_confirmed_cleaner_read ON properties FOR SELECT USING (EXISTS (SELECT 1 FROM bookings b WHERE b.property_id = properties.id AND b.cleaner_user_id = tideway_private.current_user_id() AND b.status IN ('confirmed', 'cleaner-en-route', 'cleaner-arrived', 'cleaning-in-progress', 'awaiting-review')));
...
CREATE POLICY property_photos_confirmed_cleaner_read ON property_photos FOR SELECT USING (EXISTS (SELECT 1 FROM bookings b WHERE b.property_id = property_photos.property_id AND b.cleaner_user_id = tideway_private.current_user_id() AND b.status IN ('confirmed', 'cleaner-en-route', 'cleaner-arrived', 'cleaning-in-progress', 'awaiting-review')));
