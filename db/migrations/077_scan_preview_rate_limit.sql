-- The review step asks the server to assess a scan the customer is still
-- holding. It stores nothing, but it normalises up to 200 objects, scores them
-- and reads the published rates, so an authenticated session replaying it could
-- spend CPU without bound. A read-shaped allowance is the right shape: generous
-- for a customer editing their checklist and correcting objects — every edit
-- re-previews — and cheap to exceed deliberately.
BEGIN;
...
BEGIN
  ...
  SELECT policy.maximum_requests, policy.window_seconds INTO maximum_requests, window_seconds FROM (VALUES
    ...
    ('marketplace-landlord:scan-preview',120,900)
