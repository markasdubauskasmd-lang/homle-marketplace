-- Authenticated marketplace writes had no allowance of any kind. Sign-in,
-- signup and the metered provider calls were each throttled, but once a session
-- existed every mutation behind `protect({ mutation: true })` — properties,
-- cleaning requests, messages, disputes, notifications — could be replayed as
-- fast as the database would accept them. One session created 400 properties in
-- under three seconds.
--
-- This is not an authorisation gap: every one of those writes was correctly
-- scoped to the account that made it. It is a volume gap, so the allowance is
-- keyed by account rather than by address: an office or carrier NAT must not
-- put unrelated people in one bucket, and the budget should follow whoever is
-- actually spending it.
--
-- 300 in fifteen minutes is well clear of real use. The heaviest genuine
-- session is a Landlord walking the booking journey — property, request, a
-- photo intent and completion per room, scan saves, submission — or a Cleaner
-- working a job, ticking tasks and posting progress. Both land near a hundred.
BEGIN;

-- `consume_rate_limit` clamps the stored counter to `maximum_requests + 1`, so
-- the table's ceiling has to cover the largest policy. Migration 020 sized it
-- for 120; the new allowance is 300. Without this the counter reaches 121, the
-- check rejects the insert, and the boundary answers 503 "abuse control
-- unavailable" — which is what happens the first time this is exercised.
ALTER TABLE tideway_private.request_rate_limits DROP CONSTRAINT request_rate_limits_request_count_check;
ALTER TABLE tideway_private.request_rate_limits ADD CONSTRAINT request_rate_limits_request_count_check CHECK (request_count BETWEEN 1 AND 301);
