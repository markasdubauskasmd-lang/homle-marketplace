...
-- PostgreSQL treats an unqualified PL/pgSQL variable and table column with the
-- same name as ambiguous. Keep the deployed migration history immutable and
-- replace only the affected invitation function with an explicit variable name.
CREATE OR REPLACE FUNCTION tideway_private.invite_cleaner(
...
BEGIN
  ...
  request_outward_postcode:=substring(upper(replace(property_record.postcode,' ','')) FROM '^([A-Z]{1,2}[0-9][A-Z0-9]?)');
  IF NOT EXISTS (
