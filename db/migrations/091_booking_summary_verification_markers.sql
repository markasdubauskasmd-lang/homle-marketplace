...
-- Preserve every independently verified booking-summary contract after the
-- client-conversation projection was added. Migration 090 may already be in
-- the immutable ledger, so this forward-only migration restores the earlier
-- response-deadline marker without rewriting that recorded migration.
