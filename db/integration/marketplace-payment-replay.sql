...
DELETE FROM tideway_private.payment_command_attempt_windows WHERE command_id IN (SELECT id FROM payment_commands WHERE payment_id=p);
  ...
  result:=pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_reverse','transfer-reversed','tr_replay',p,t,command.amount_pence,'gbp',occurred,repeat('1',64));
  ...
  result:=pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_created','transfer-succeeded','tr_replay',p,t,command.amount_pence,'gbp',occurred,repeat('2',64));
  ...
  result:=pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_refunded','refund-succeeded','re_replay',p,r,payment.amount_pence,'gbp',occurred,repeat('3',64));
  ...
  result:=pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_failed','refund-failed','re_replay',p,r,payment.amount_pence,'gbp',occurred,repeat('4',64));
  ...
  result:=pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_failed','refund-failed','re_replay',p,r,payment.amount_pence,'gbp',occurred,repeat('4',64));
  result:=pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_refunded_again','refund-succeeded','re_replay',p,r,payment.amount_pence,'gbp',occurred,repeat('5',64));
  ...
  result:=pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_success_last','refund-succeeded','re_replay_first',p,r,1000,'gbp',occurred,repeat('7',64));
  ...
  result:=pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_after_reply_failure','refund-succeeded','re_reply_failure',p,r,1000,'gbp',occurred,repeat('8',64));
  ...
  FOR variation IN 1..3 LOOP
    blocked:=false;
    BEGIN
      PERFORM pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_after_reply_failure','refund-succeeded',CASE WHEN variation=1 THEN 're_wrong' ELSE 're_reply_failure' END,p,r,
  ...
  result:=pg_temp.reconcile_bound_fixture_event('stripe','evt_replay_after_reply_failure','refund-succeeded','re_reply_failure',p,r,1000,'gbp',occurred,repeat('9',64));
