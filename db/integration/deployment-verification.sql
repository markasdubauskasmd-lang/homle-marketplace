...
DO $terminal_cleanup_verification$
DECLARE installed boolean:=false; signature text;
BEGIN
  IF to_regclass('tideway_private.schema_migrations') IS NOT NULL THEN
    EXECUTE 'SELECT EXISTS (SELECT 1 FROM tideway_private.schema_migrations WHERE migration_order=116)' INTO installed;
  END IF;
  IF installed THEN
    FOREACH signature IN ARRAY ARRAY['tideway_private.claim_job_photo_terminal_cleanup(integer)','tideway_private.claim_request_photo_terminal_cleanup(integer)','tideway_private.acknowledge_job_photo_terminal_cleanup(uuid,text)','tideway_private.acknowledge_request_photo_terminal_cleanup(uuid,text)'] LOOP
      IF to_regprocedure(signature) IS NULL OR NOT has_function_privilege('tideway_worker',signature,'EXECUTE')
        OR has_function_privilege('tideway_app',signature,'EXECUTE')
      THEN RAISE EXCEPTION 'Terminal photo cleanup function/grant is incomplete or unsafe'; END IF;
    END LOOP;
  END IF;
END;
$terminal_cleanup_verification$;
...
SELECT json_build_object(
  ...
  'workerFunctionChecks', 14
    ...
    + CASE WHEN to_regprocedure('tideway_private.claim_request_photo_terminal_cleanup(integer)') IS NULL THEN 0 ELSE 4 END
