...
BEGIN
 IF (SELECT count(*) FROM tideway_private.payment_observed_objects WHERE payment_id=p)<>2
   OR (SELECT applied_pence FROM tideway_private.payment_observed_objects WHERE provider_object_id='re_upgrade_original_2') IS DISTINCT FROM 1000
   OR (SELECT terminal_failure FROM tideway_private.payment_observed_objects WHERE provider_object_id='re_upgrade_original_3') IS DISTINCT FROM true
   THEN RAISE EXCEPTION '117 did not backfill verified pre117 success and terminal failure anchors'; END IF;
 IF (SELECT evidence FROM observation_upgrade_fixture WHERE kind='reconciler-oid') IS DISTINCT FROM 'tideway_private.reconcile_payment_provider_event(text,text,text,text,uuid,uuid,integer,character,timestamptz,character,text,text,text)'::regprocedure::oid::text
 ...
 END LOOP;
 IF (SELECT amount_refunded_pence FROM booking_payments WHERE id=p) IS DISTINCT FROM 1000 THEN RAISE EXCEPTION '117 replay counted a historical refund twice'; END IF;
 ...
 IF (SELECT amount_refunded_pence FROM booking_payments WHERE id=p) IS DISTINCT FROM 1000 THEN RAISE EXCEPTION 'Removing metadata created a second accounting path'; END IF;
 ...
 IF (SELECT amount_refunded_pence FROM booking_payments WHERE id=p) IS DISTINCT FROM 0 THEN RAISE EXCEPTION 'Upgrade failed exact reversal or resurrected terminal failure'; END IF;
END;
$upgrade$;
ROLLBACK;
\echo 'Actual PostgreSQL116-to117 observation upgrade passed: signed historical anchors, retained OID, replay, metadata removal, failure reversal and terminal ordering.'
