...
INSERT INTO user_roles (user_id, role) VALUES
  ...
  ('10000000-0000-4000-8000-000000000001', 'cleaner'),
...
INSERT INTO cleaner_profiles (
  ...
  '10000000-0000-4000-8000-000000000001', 'integration-landlord-cleaner-test', 'Dual-role self-match exclusion fixture.',
  2000, 10, 1, ARRAY['English'], ARRAY['Vacuum'], ARRAY['General cleaner'], 100, 'available', true
);

INSERT INTO cleaner_services (cleaner_user_id, service_code, pricing_model, price_pence)
VALUES
  ('10000000-0000-4000-8000-000000000001', 'standard-clean', 'hourly', 2000),
  ('10000000-0000-4000-8000-000000000002', 'standard-clean', 'hourly', 2500);
...
VALUES
  ...
  ('10000000-0000-4000-8000-000000000002', now() + interval '47 hours', now() + interval '55 hours', 'available');
...
INSERT INTO properties (id, landlord_user_id, name, address_line_1, locality, postcode, property_type, access_instructions_ciphertext) VALUES
  ...
  ('20000000-0000-4000-8000-000000000003', '10000000-0000-4000-8000-000000000001', 'Integration Property C', 'Private test address C', 'London', 'SW1A 1AA', 'flat', decode('070809', 'hex'));
...
INSERT INTO cleaning_requests (
  ...
  ('30000000-0000-4000-8000-000000000003', '10000000-0000-4000-8000-000000000001', '20000000-0000-4000-8000-000000000003', 'draft', now() + interval '51 hours', now() + interval '53 hours', 'standard', ARRAY['standard-clean'], 10000, repeat('c', 64), NULL);
...
INSERT INTO cleaning_request_tasks (cleaning_request_id, room_name, description, sort_order) VALUES
  ...
  ('30000000-0000-4000-8000-000000000003', 'Hallway', 'Vacuum floor', 0);
...
INSERT INTO cleaning_request_photos (id,cleaning_request_id,storage_key,room_name,note,mime_type,byte_size,checksum_sha256,width_pixels,height_pixels,sanitized_at) VALUES
  ...
  ('60000000-0000-4000-8000-000000000003','30000000-0000-4000-8000-000000000003','request-photos/30000000-0000-4000-8000-000000000003/60000000-0000-4000-8000-000000000003.jpg','Hallway','Synthetic integration room photo','image/jpeg',1000,decode(repeat('cc',32),'hex'),800,600,now());
...
SELECT tideway_private.submit_cleaning_request('30000000-0000-4000-8000-000000000003',true,false);
