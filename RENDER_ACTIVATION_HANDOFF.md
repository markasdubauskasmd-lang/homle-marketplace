...
| **OPEN - blocked on a database** | Six database defects are found and specified but **not fixed**, because they need a migration and no Postgres was available to execute one against. Worst first: the administrator cleaner-vetting queue in `063` applies `LIMIT/OFFSET` to a bare `jsonb_agg`, so **page 1 returns every row and page 2 returns nothing**; `bookings(cleaning_request_id)` has no plain index, costing up to 100 sequential scans per landlord request page; `payment_commands(payment_id, command_kind, created_at)` is missing, costing ~400 per admin payment page; the public Cleaner directory full-scans and full-sorts `cleaner_profiles` on an unauthenticated route; `cleaning_requests.budget_pence` is nullable but automatic dispatch requires it, so authorising dispatch without a budget fails every attempt until the limit is exhausted; and `disputes`/`reviews` cascade-delete with `bookings`, so deleting a booking erases its dispute and review record. **Do not write this migration without running it against a real PostgreSQL 16 first.** | No |
| **KNOWN ISSUE (not fixed)** | `public/room-scan.js:10` - the standalone `/landlord/scan` route (direct link or bookmark) serialises the whole finished scan **including room photo data URLs** into `sessionStorage` under `homle_scan_result`, to carry it across the navigation to the booking page. So photos of the inside of a home do reach browser storage on that route. Found during review of #81 and left alone deliberately: two consumers read that key (`landlord-journey.js:199`, `landlord-dashboard.js:1960`), so removing the writer changes the booking handoff and needs its own change. The in-place journey path (`landlord-journey.js:337`) is already correct and keeps everything in memory. **Do not "fix" this by deleting the write without redirecting the route** - it would silently break the scan-to-booking handoff. | No |
