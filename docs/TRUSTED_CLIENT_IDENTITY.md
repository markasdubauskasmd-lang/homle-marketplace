...
Render is an explicit separate provider mode because its public service port is reachable only through Render's Cloudflare-fronted load balancer. Render appends to any browser-supplied `X-Forwarded-For` instead of clearing it, so the first chain entry is attacker-controlled and is never used. Instead the resolver requires the Cloudflare-set `True-Client-IP` header, validates every address in a bounded one-to-16-entry `X-Forwarded-For` chain, and accepts the client identity only when `True-Client-IP` appears in that validated chain; any missing, malformed or mismatched header fails closed. `TRUST_PROXY_PROVIDER=render` is accepted only with `NODE_ENV=production`, `RENDER=true`, a platform-shaped `RENDER_SERVICE_ID`, the platform `*.onrender.com` hostname and no generic proxy CIDRs. These platform variables must come from Render; do not define them manually. A different provider cannot select this mode.
