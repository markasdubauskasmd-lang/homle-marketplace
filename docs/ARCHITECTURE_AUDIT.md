...
| Files | Room photos and videos are validated, stored locally and served through authorised non-cacheable routes | Media consent, type/size limits and lifecycle audit; account-marketplace signed upload/read, quarantine and sanitation contracts | Attach an approved private object-storage adapter, prove malware/decode/EXIF handling in staging and run production retention jobs |
