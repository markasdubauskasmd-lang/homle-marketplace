...
The current suite passed from a fresh 55-migration schema on PostgreSQL 16.14 on 17 July 2026. It proved owner-only first-Administrator provisioning, signed-provider deletion persistence, restricted-role denial, exact retry and session revocation, owner isolation, unrelated-user denial, private access-instruction timing, revoked direct workflow writes, current-payment journey gating, audited disputes, exactly-once history and a two-transaction overlap race in which exactly one acceptance succeeded. The integrated participant rehearsal then proved the complete synthetic Landlord-to-Cleaner journey, cleaning, completion and review lifecycle. Its authorization prerequisite was a local synthetic ledger fixture, not a provider transaction. Cleanup left zero reserved users, bookings, reviews or synthetic payments; the disposable database and temporary migration-owner role were removed. Repeat this same proof in managed staging and separately complete the genuine provider-backed mobile rehearsal before attachment.
