...
`/tracking-test` is a deliberately separate, development-only, non-booking lab for trying the real browser journey and cleaning handoff before the account runtime is enabled. It can be created only through the computer's loopback interface. The server returns separate 256-bit Cleaner-controller and Landlord-viewer tokens, stores only their SHA-256 digests, and keeps the whole session in memory for at most 30 minutes. Tokens travel in a browser fragment for handoff, are removed before the first request, and then authorise API calls through `X-Tracking-Test-Token` rather than a query string. Production does not construct its in-memory store or expiry timer and returns 404 for the friendly path, direct HTML/script assets and every `/api/tracking-test/*` route.
...
Create the controller directly at `http://127.0.0.1:4173/tracking-test`. A trusted localhost origin can request geolocation on the computer. The same-Wi-Fi HTTP phone site may open a token-authorised Landlord viewer and watch both journey and cleaning updates, but it cannot be used as the real-location controller because mobile geolocation requires trusted HTTPS. Do not create a tunnel or public deployment merely to bypass this boundary; the production route is intentionally unavailable.
