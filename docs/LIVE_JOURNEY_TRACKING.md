...
Create the controller at `http://127.0.0.1:4173/tracking-test`. A trusted localhost origin can request geolocation on the computer. The same-Wi-Fi HTTP phone site may open a token-authorised Landlord viewer, but it cannot be used as the real-location controller because mobile geolocation requires trusted HTTPS. Do not create a tunnel or public deployment merely to bypass this boundary.
