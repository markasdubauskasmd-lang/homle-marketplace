...
- The policy derives Cleaner pay from the Cleaner’s active service prices and requested duration, covers approved labour on-cost, payment, risk contingency, distance-aware travel, supplies and other costs, then solves the smallest customer price meeting both the approved contribution-margin and minimum-contribution floors.
...
Apply `009_booking_invitation_and_acceptance.sql`, request matching migration 010 and expiry migration `011_invitation_expiry_and_requeue.sql` after migration 008, then apply `028_invitation_eligibility_hardening.sql`, `029_consent_bound_automatic_dispatch.sql` and `031_fix_invitation_service_area_lookup.sql` in locked order. Migration 009 backfills evidence fields for legacy booking rows without fabricating legacy margin evidence. New and changed rows must pass the positive-contribution and target-margin checks. Migration 011 distinguishes user and system history actors and adds bounded idempotent expiry/requeue. Migration 028 makes the matching eligibility and Cleaner-pay assumptions authoritative again at the final invitation write; migration 029 adds explicit request-level consent, bounded attempts and leased worker functions; migration 031 removes the ambiguous outward-postcode variable found by real PostgreSQL execution without rewriting deployed history.
...
The fresh disposable PostgreSQL 16.14 run on 16 July 2026 passed the participant privacy, current-payment and simultaneous overlap-acceptance checks after migration 031. Before enabling routes, repeat the migrations against managed staging; test out-of-area, wrong-property, changed-price, inactive-account and overlapping-invitation rejection; validate every legacy row; race two invitations for one Cleaner; race two concurrent accept transactions; race two expiry workers; and verify `tideway_app` cannot directly update a booking, call a superseded function or execute the expiry batch.
