...
Real account testing can be activated independently with `AUTHENTICATION_ENABLED=true` after the database, email, monitoring, provider credentials and approved-account fingerprint pass. It does not require media storage or payments and does not expose booking routes. Keep `MARKETPLACE_ENABLED=false`, `PILOT_INTAKE_ENABLED=false` and `PAYMENTS_ENABLED=false` during this stage. Follow [STAGING_ACCOUNT_ACTIVATION.md](./STAGING_ACCOUNT_ACTIVATION.md); callback URLs are provider destinations and must never be opened as login pages.
