...
The root `render.yaml` defines one free Docker web service and the connected durable `basic_256mb` PostgreSQL 16 staging database in Frankfurt. It disables automatic source deployment, pins the connected canonical `APP_ORIGIN` (`https://homlle.com`), generates the Administrator and restricted-role secrets, and keeps pilot intake, public marketplace approval and payments off. The database uses the guarded `_homle_staging` name, a distinct migration-owner login and no public IP allow-list. It creates no separate worker or disk; the already-connected custom domain remains Dashboard-owned. The database started as the owner-approved free staging instance on 17 July 2026 and Render reported it available, not suspended and on `basic_256mb` on 16 August 2026.

### Blueprint and dashboard ownership

Render Blueprint sync overwrites a conflicting Dashboard value when `render.yaml` supplies a `value`. On 4 August 2026, that behaviour reset the working restricted-staging marketplace and postcode provider to their source defaults. The deployed landing page still loaded, but `/api/health` correctly reported the marketplace, scanner, speech summary, media, realtime and matching paths unavailable. This was an environment-ownership regression, not a missing database or scanner implementation.

`MARKETPLACE_ENABLED`, `MAP_PROVIDER`, `GEOCODING_PROVIDER`, `ADDRESS_LOOKUP_PROVIDER` and `ETA_PROVIDER` are therefore declared with `sync: false`. Render leaves their existing Dashboard values unchanged during later Blueprint updates, and a newly created service must receive deliberate values before activation. Never add a source `value` back to these entries. The source-controlled safety gates remain different: pilot intake, public-marketplace approval, legal/insurance/supply/pricing/support/terms attestations, all worker switches and all payment switches stay explicitly false and a Blueprint sync must continue to enforce them.
...
The Docker entrypoint connects over Render's private database reference before starting the web process. It requires the exact staging suffix, PostgreSQL 16+, a separate non-superuser migration owner, an empty schema and valid locked migration checksums. A production connection uses verified TLS everywhere except Render's documented same-workspace internal `dpg-*` hostname: that narrowly recognised target uses Render's private-network transport and never traverses the public internet. The exemption requires Render's platform identity and a Render web, worker, private or cron service type, and is enforced for both the web runtime and the restricted maintenance worker. An external, dotted, IP-based or non-Render target still requires certificate verification and cannot request `sslmode=disable`. A fresh database receives every currently locked migration and both grant files once. The existing managed staging database retains its deliberate 45-migration historical baseline; on the next approved release the bootstrap first verifies the installed baseline with its migration-level function signatures, creates the private ledger, records 1-45 as baselined and applies every later locked migration in order. It then reapplies least-privilege grants and requires the current read-only verifier before starting the app. A later restart verifies the ledger and schema before rotating either restricted role and never replays an applied migration. Any partial, drifted or unexpected database fails closed. The web service remains intentionally gated until account email, private object storage, monitoring, authentication providers and approved booking economics pass the guarded staging checks.
...
The report proves whether the restricted Google account preview remains closed to unapproved users and whether public marketplace, intake, dispatch and payments are still off. It then names only missing key labels for the next activation layer. A safe account-preview pass is not marketplace readiness: transactional email, all five private object-storage settings, `DATABASE_URL`, the dedicated `REALTIME_DATABASE_URL` and the complete founder-approved `BOOKING_*` pricing policy must all be connected and checked before `MARKETPLACE_ENABLED` changes. The report validates the booking settings against the same bounded integer ranges as the live pricing engine without printing their values. Stripe test keys are reported separately and remain a later payment rehearsal, never an instruction to enable charges.
...
Render documents that free web services use an ephemeral filesystem, cannot attach a persistent disk, spin down when idle and are not for production applications. The database is no longer on Render's expiring free plan, but its current `basic_256mb` size and non-HA configuration are still staging boundaries that require capacity, backup and recovery review before public launch. Render also does not offer a free background-worker instance. See Render's [free-instance limits](https://render.com/docs/free) and [current Blueprint specification](https://render.com/docs/blueprint-spec).
...
`HOST` may be `127.0.0.1` when a same-host reverse proxy is the only caller, or a reviewed interface binding such as `0.0.0.0` inside an isolated container network. Render mode additionally requires the platform-provided `RENDER=true`, `RENDER_SERVICE_ID` and `RENDER_EXTERNAL_HOSTNAME` runtime values and, because Render appends to rather than replaces a client-supplied `X-Forwarded-For`, identifies the client from the Cloudflare-verified `True-Client-IP` header after confirming it appears in the bounded forwarding chain; it never trusts the forgeable first `X-Forwarded-For` entry. Do not manually set those platform values. For a different host, leave `TRUST_PROXY_PROVIDER` blank, configure the actual immediate proxy networks in `TRUSTED_PROXY_CIDRS`, and require that proxy to replace forwarding input with exactly one address.
...
Do not change `MARKETPLACE_ENABLED` until managed PostgreSQL, transactional email, private object storage, monitoring, proxy identity, migrations/grants and two-account HTTPS tests pass. Marketplace mode additionally requires a normal request `DATABASE_URL`, a direct session-capable `REALTIME_DATABASE_URL` to the same database, separate session/token/encryption credentials, an email sender, private-storage settings and every `BOOKING_*` cost/fee/travel/profit/invitation setting listed in `.env.example`. Both the percentage margin and minimum contribution in pounds require explicit founder approval; blank, partial, fractional or out-of-range booking policy values fail closed. Transaction pooling is suitable for the normal URL after load testing but not for the real-time URL because booking updates use PostgreSQL `LISTEN/NOTIFY`. Use the shipped `homle:monitoring-webhook` module with an approved private HTTPS collector and secret, or a reviewed absolute custom adapter implementing the same monitoring/shutdown contract. Startup connects to and verifies each dependency before it exposes an account route.
