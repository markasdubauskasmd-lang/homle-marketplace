...
SMTP is not configured in this local workspace. The internal [SMTP delivery adapter](SMTP_EMAIL_DELIVERY.md) is prepared and exact-dependency locked, but email/password capability flags stay false until PostgreSQL, `SESSION_SECRET`, the distinct `AUTH_TOKEN_SECRET`, exact `APP_ORIGIN`, `SMTP_URL` and `EMAIL_FROM` are configured and the provider passes staging verification.
