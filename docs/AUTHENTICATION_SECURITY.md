...
The controller cannot compose unless trusted email delivery and a server-derived client-key resolver are supplied, email configuration is present and PostgreSQL exposes the reviewed shared-limiter function. Authentication and the two public marketplace reads use one reusable fail-closed boundary backed by atomic database buckets: bounded denials include `Retry-After`, while a missing key, malformed decision or limiter outage returns a generic 503 and sends the private cause only to monitoring. The adapter purpose-HMACs the trusted key per scope before storage. It remains detached from `server.mjs`; public entry stays closed until real PostgreSQL migrations, SMTP/provider adapters, two-instance abuse-control evidence and HTTPS browser tests exist.
