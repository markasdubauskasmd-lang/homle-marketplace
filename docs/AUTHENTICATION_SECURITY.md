...
- Connecting Google or Facebook from Settings requires the existing live session, same-origin CSRF and either current-password step-up or a fresh OAuth step-up whose exact provider subject already belongs to that account. The recent social proof is user/session/provider-bound, expires after ten minutes and its browser cookie is cleared when another connection starts. Removal requires password proof or proof through the different social provider that will remain, refuses the final identity, writes safe audit evidence and revokes every active session atomically. The pre-login resolver never performs these actions.
...
Transactional email is not configured in this local workspace. The internal [HTTPS/SMTP delivery adapters](SMTP_EMAIL_DELIVERY.md) are prepared, but email/password capability flags stay false until PostgreSQL, `SESSION_SECRET`, the distinct `AUTH_TOKEN_SECRET`, exact `APP_ORIGIN`, exactly one of `RESEND_API_KEY` or `SMTP_URL`, and `EMAIL_FROM` are configured and the provider passes staging verification.
...
The attachment constructs trusted email delivery and the server-derived client-key resolver internally; email configuration must be complete and PostgreSQL must expose the reviewed shared-limiter function. Authentication and the two public marketplace reads use one reusable fail-closed boundary backed by atomic database buckets: bounded denials include `Retry-After`, while a missing key, malformed decision or limiter outage returns a generic 503 and sends the private cause only to monitoring. It remains default-off; public entry stays closed until real PostgreSQL migrations, provider verification, two-instance abuse-control evidence and HTTPS browser tests exist.
