...
- `/landlord/dashboard` shows the signed-in Landlord's pending Cleaner decisions, active/upcoming bookings and history alongside their properties and request-draft workspace. Each pending invitation names the Cleaner, shows the exact London response deadline and explains that no booking is confirmed and no payment is taken while the Cleaner decides. The waiting list receives the existing participant-only live booking updates, without giving the Landlord Cleaner response controls. Eligible confirmed bookings link to payment authorization and confirmed work links to the participant-only active-job screen.
