...
- `/landlord/dashboard` shows the signed-in Landlord's pending Cleaner decisions, active/upcoming bookings and history alongside their properties and request-draft workspace. Each pending invitation names the Cleaner, shows a concise time remaining plus the exact London response deadline, and explains that no booking is confirmed and no payment is taken while the Cleaner decides. Its display timer makes no server request. At the deadline it performs one read-only booking/request reconciliation, never sends or repeats an invitation action, and then relies on the existing participant-only live update stream. The Landlord receives no Cleaner response controls. Eligible confirmed bookings link to payment authorization and confirmed work links to the participant-only active-job screen.
...
1. Apply all migrations in `db/migration-lock.json` (currently 59) and the restricted runtime grants to PostgreSQL 16.
