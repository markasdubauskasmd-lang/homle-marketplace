...
For a Cleaner explicitly selected by the Landlord, a read-only invitation quote returns only the request reference, Cleaner reference, exact customer total and response deadline. The dashboard requires the Landlord to approve that total before sending the invitation. Invitation creation independently recalculates all private terms and rejects a missing, stale or altered approval; Cleaner pay, costs and margin details never enter the browser response.

Consent-bound automatic dispatch remains separate: the Landlord sees and approves the request's persisted maximum total together with the next one-at-a-time attempt limit. The owner-bound server transaction exact-compares that amount before enabling dispatch, the worker excludes candidates without a cap or above it, and migration 058 repeats the cap check inside the final worker-only invitation function. A missing or exceeded cap therefore cannot become a booking through either the browser, application service or worker credential.
