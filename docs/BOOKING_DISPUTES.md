...
- The category-specific `tideway-case-response-v2` safeguard identifies the smallest useful evidence set and the category's explicit boundary. The server rejects a final resolution unless the Administrator submits the exact current version and confirms evidence review, sensitive-data minimisation and that nothing was done beyond what is recorded on the form. Version 1 asked instead for a promise that no payment action was performed; that stopped being true when the desk gained the ability to refund, so the field was renamed rather than quietly redefined, and a client still sending version 1 is refused. A refund additionally requires its own authorisation of the exact amount, is sent before the decision is recorded, and can never exceed what was captured and not already returned.
- The resolution dialog requires the note to identify the evidence sources used. Safety guidance explicitly says Tideway is not an emergency service; payment guidance explicitly says the case action cannot capture, cancel, refund or transfer money.
