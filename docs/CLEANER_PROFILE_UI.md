...
The form covers every field accepted by the profile service: photo URL, biography, experience, languages, preferences, status, hourly/fixed prices, supported services, radius, outward areas, supplied equipment/products and public visibility. Pounds are converted to exact integer pence. Fixed-price lines, list limits, UK outward postcodes and numeric bounds are validated before submission, then validated again by the server.

Editing outward postcodes preserves stored private coordinates only for unchanged codes. A new code has no invented coordinates; a removed code is omitted. Publication is automatically unavailable unless the same ten deterministic completion requirements used by the server reach 100%. The server remains authoritative and rejects any bypass.
