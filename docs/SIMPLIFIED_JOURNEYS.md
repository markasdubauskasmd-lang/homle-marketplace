...
- Every room photo demanded a second typed description even when the Landlord had already selected its room and confirmed the spoken checklist.
- On mobile, the selected-room camera controls and speech action were split across separate panels, forcing the Landlord to scroll while walking through the property.
- Recording a new room video required leaving the direct camera path or relying on the browser's existing-media chooser.
- The public request required a typed room/size summary immediately before the mandatory room scan captured the same scope more reliably.
- The operator had to leave a matched-Cleaner proposal, use a separate calculator and manually copy a safe price back before saving the draft.
- The working pilot asked customers to classify their relationship to the property and describe access before either answer affected matching or a booking existed.
- The customer contact step required both email and phone and exposed organisation alongside the primary fields, even though the private follow-up route already depends on email.
- Optional recurrence and free-text notes competed visually with the required hazards, date and arrival-window decisions.
...
The old concierge forms, sample directory, demo preview, standalone scan, quote, payment, booking-pack, request tracker, Cleaner availability and application tracker pages are permanently removed. All capability modes now keep homepage actions account-first: Landlords use `/signup?intent=book`, Cleaners use `/signup?intent=work`, and signed-in users reopen `/landlord/dashboard`, `/landlord/book` or `/cleaner/dashboard`.

The private control desk now keeps the profitability decision inside each matched-Cleaner proposal. As hours, customer rate, Cleaner pay or extra costs change, one compact panel shows the resulting totals and margin. A below-floor draft exposes one target-safe hourly-rate action instead of making the operator use the separate rehearsal calculator and copy the result. Unconfirmed cost assumptions, duration below the reviewed/minimum scope and below-floor economics disable draft saving in the browser; the server independently enforces the same rules.
...
Dashboard → **Do this next** → add the minimum property details → choose date/type → speak room notes → automatic concise tasks → confirm scope → camera/room scan → submit → **Find my Cleaner**.

After a completed visit, **Book again** carries the same Cleaner and owned property directly into that request form. It does not reuse an old schedule, checklist, photos, quote or consent: the Landlord still reviews the current rooms and the Cleaner must receive and accept a new profitable invitation.

The top action changes automatically between add property, start request, continue room scan, authorize one next Cleaner invitation, authorize a confirmed booking and open live progress. Property setup now requires only type, address line 1, town and postcode; a custom label is optional and the server otherwise creates a privacy-safe label such as “Flat in London”, never the street address. A Landlord's only saved property is selected automatically. The request safely suggests regular domestic for a home, workplace cleaning for an office/retail/clinic, and communal-area cleaning for communal property; it never guesses for “Other” and never overwrites the Landlord's explicit change. Optional property, access, parking, recurring and budget fields stay available in disclosure panels without blocking the primary path. One cleaning-type value now supplies the required service. The request then leads with one large speech action; stopping speech updates a room-grouped bullet review automatically. The raw transcript and manual bullet editor stay collapsed unless the Landlord chooses them, while unsupported browsers open the typed fallback automatically. Photo notes are optional: the selected room and reviewed room-labelled checklist already carry the required cleaning context, while a note remains available for a genuine visual exception. Scope review, photo consent, one-at-a-time Cleaner invitation, price/payment and final submission remain deliberate because they are safety/contract boundaries rather than avoidable friction.

When that request carries a deliberately selected Cleaner, the only extra decision is one exact-price confirmation. The Landlord sees the server-calculated total, can keep the request open, or approves that amount once; the server recalculates it again before creating the invitation and takes no payment at this step.

In the working private pilot scan, choosing the current room now unlocks the camera, existing media and speech together in one mobile panel. All three inherit that room, so speech cannot become an ungrouped note and the Landlord does not scroll away from capture controls while walking. The following panel is review-only; unsupported browsers continue to offer typed instructions.

That capture panel now also includes **Record a room video**. On supported phones it requests one rear-camera video for the selected room, while existing photos/videos remain a separate multi-file choice. Captured video passes through the same two-video, 30-second, 15 MB per-file and 20 MB total limits; browsers that ignore the capture hint may still show their own chooser, which the interface states rather than promising native-app behaviour.
...
Cleaner onboarding and availability now remain inside the authenticated Cleaner workspace rather than separate public or token-tracker pages.
...
The Cleaner dashboard's single next action follows real readiness order: urgent invitation/live job first, then complete and publish the profile, review availability and finish payout setup. The standalone availability page is retired; dashboard services remain the source of truth.
...
- Retired routes return 404 and have no shipped HTML or JavaScript controller.
