...
Apple adds gated `/api/marketplace/auth/apple/start` and `/api/marketplace/auth/apple/callback` routes. First sign-in uses Apple's required `form_post` response for `name email`, a secure host-only `SameSite=None` flow cookie, a five-minute server-signed ES256 client secret and server-only code exchange. Homle verifies Apple's RS256/JWKS signature, issuer, Services-ID audience, expiry, nonce, subject and verified email before account resolution. Authenticated connection and step-up omit scopes and use Apple's query response so the existing Lax Homle session and provider-binding cookies remain available. Apple tokens and private keys never enter the browser or database.
