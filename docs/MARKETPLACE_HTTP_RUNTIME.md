...
`src/marketplace/attachment.mjs` returns a zero-resource disabled boundary unless `MARKETPLACE_ENABLED=true`. Enablement then fails closed unless `DATABASE_URL`, separate 32+ character `SESSION_SECRET` and `AUTH_TOKEN_SECRET`, exact `APP_ORIGIN`, distinct 32+ character `DATA_ENCRYPTION_KEY`, email and private-object-storage configuration, valid direct/trusted-proxy client identity, trusted email delivery and complete private storage adapters are present. It loads deployment adapters only from the explicitly configured absolute `MARKETPLACE_ADAPTER_MODULE`, constructs the [trusted client resolver](TRUSTED_CLIENT_IDENTITY.md) internally, verifies that the database connection is PostgreSQL 16+, authenticated as non-bypass `tideway_app`, and has the critical migrations/functions—including the shared limiter—then composes the PostgreSQL limiter and router. Startup failure closes the pool; shutdown closes the realtime listener and pool once.
...
5. Configure `TRUST_PROXY=false` for a direct listener, or set `TRUST_PROXY=true` with the immediate proxy's reviewed `TRUSTED_PROXY_CIDRS` and guarantee that it replaces incoming forwarding headers with exactly one client IP. The resolver is internal and fail-closed. Provide an absolute deployment-owned adapter module exporting `createMarketplaceDeploymentAdapters({ env })`; it must return trusted email delivery, private object storage with inspection/re-encoding/deletion, and private error monitoring. Prove the same limiter denial across two application instances and authentication behavior under HTTPS.
