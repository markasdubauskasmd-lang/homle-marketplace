...
`src/marketplace/attachment.mjs` returns a zero-resource disabled boundary unless `MARKETPLACE_ENABLED=true`. Enablement then fails closed unless `DATABASE_URL`, separate 32+ character `SESSION_SECRET` and `AUTH_TOKEN_SECRET`, exact `APP_ORIGIN`, distinct 32+ character `DATA_ENCRYPTION_KEY`, SMTP and private-object-storage configuration, valid direct/trusted-proxy client identity and complete private storage adapters are present. It loads storage/monitoring adapters only from the explicitly configured absolute `MARKETPLACE_ADAPTER_MODULE`, constructs the [trusted client resolver](TRUSTED_CLIENT_IDENTITY.md) and [SMTP delivery](SMTP_EMAIL_DELIVERY.md) internally, verifies PostgreSQL 16+ as non-bypass `tideway_app`, verifies the SMTP transport, then composes the PostgreSQL limiter and router. Startup failure closes SMTP, realtime and pool resources; shutdown closes them once.
...
5. Configure `TRUST_PROXY=false` for a direct listener, or set `TRUST_PROXY=true` with the immediate proxy's reviewed `TRUSTED_PROXY_CIDRS` and guarantee that it replaces incoming forwarding headers with exactly one client IP. Configure the credentialed SMTP URL and aligned sender, then prove strict-TLS verification and reserved-recipient delivery. Provide an absolute deployment-owned adapter module exporting `createMarketplaceDeploymentAdapters({ env })`; it now needs private object storage with inspection/re-encoding/deletion and private error monitoring. Prove the same limiter denial across two application instances and authentication behavior under HTTPS.
