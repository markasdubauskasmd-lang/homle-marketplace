...
- **P0-7 complete in launch documentation**: the Polsia and independent-local scorecards are separated, the current inventory is 139 JavaScript modules / 46 test files, and the unsupported £20 referral claim is inventoried.
- Current full evidence: 121 syntax/encoding/database-asset commands and 41 test commands pass; real lead and scan hashes remain unchanged. The earlier verified source archive remains the tracked `5cc8f99` checkpoint: `../Tideway-independent-source-2026-07-16.zip`, 192 entries, SHA-256 `237D331367EF0B0C85AE5420B8FBDB1E405FEFE4BDDCB8ACB84CFAFDFE18E705`.
...
- **E1 fail-closed server attachment prepared:** `server.mjs` now has an isolated, default-off `/api/marketplace/*` dispatch boundary. Enablement requires complete database/secrets/email/storage configuration, a deployment adapter module, shared limiter, trusted client key, email delivery, private media adapter, PostgreSQL 16 and a non-bypass `tideway_app` connection with critical functions present. Failure closes resources and exposes no partial routes. Provider discovery advertises email routes only when attached and Google only when both its verified callback and credentials are attached; configured credentials alone cannot claim readiness. Apple/Facebook stay false. The reviewed/locked `pg` dependency and real adapters remain outstanding, so the flag stays false.
