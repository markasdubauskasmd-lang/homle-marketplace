...
- **P0-7 complete in launch documentation**: the Polsia and independent-local scorecards are separated, the current inventory is 165 JavaScript modules / 58 test files, and the unsupported £20 referral claim is inventoried.
- Current full evidence after the detached payment-ledger checkpoint: 149 syntax/encoding/database/dependency-asset commands and 58 test commands pass (207 total); real lead and scan hashes remain unchanged. The earlier verified source archive remains the tracked `5cc8f99` checkpoint: `../Tideway-independent-source-2026-07-16.zip`, 192 entries, SHA-256 `237D331367EF0B0C85AE5420B8FBDB1E405FEFE4BDDCB8ACB84CFAFDFE18E705`.
...
- **PostgreSQL source activation gate added:** all 22 ordered migrations plus the runtime/worker least-privilege grant scripts are SHA-256 locked and checked for missing, unexpected, reordered, modified or transaction-boundary-damaged assets. This prevents an unreviewed SQL set from silently entering staging, but does not claim the absent real PostgreSQL execution/RLS/concurrency evidence required by E1/E2.
...
- Prepared source: the provider-neutral workflow and migration 022 now freeze authorization to the accepted booking total, hash retry keys, reserve role-bound capture/cancel/refund/transfer commands, keep payout destinations server-owned and reconcile only allowlisted signature-verified provider events without storing raw payloads or client secrets. It remains detached and default-off. The Stripe SDK adapter, raw webhook route, Connect onboarding, payment UI, journey gate and real test-mode cycle are still required; no provider was contacted.
