...
- **P0-7 complete in launch documentation**: the Polsia and independent-local scorecards are separated, the current inventory is 186 JavaScript modules / 66 test files, and the unsupported £20 referral claim is inventoried.
- Current full evidence after the fresh PostgreSQL 16.14 checkpoint: 170 syntax/encoding/database/dependency-asset commands and 66 test commands pass (236 total); the production dependency audit reports no known vulnerabilities and real lead/scan data was not touched. The earlier verified source archive remains the tracked `5cc8f99` checkpoint: `../Tideway-independent-source-2026-07-16.zip`, 192 entries, SHA-256 `237D331367EF0B0C85AE5420B8FBDB1E405FEFE4BDDCB8ACB84CFAFDFE18E705`.
...
2. **The detached marketplace** — `src/marketplace/*` + `db/migrations/001–031` (PostgreSQL, RLS, auth, bookings, matching, consent-bound dispatch, tracking, reviews, private media, shared limiting, payments and maintenance). Exact database/mail/S3/image dependencies plus the limiter, SMTP delivery, object storage and client resolver are integrated behind the default-off attachment. Local PostgreSQL 16.14 execution now passes. **It still cannot be enabled safely:** managed PostgreSQL, SMTP and a real private bucket have not passed staging; CORS/lifecycle/public-access/threat controls and final proxy topology are unverified; approved monitoring remains absent (F2, F8).
...
- Steps: add and version-lock a maintained Postgres driver; construct a pool from `DATABASE_URL`; compose `createMarketplaceRuntime(pool)` into a real server entrypoint behind capability gates; apply `db/migrations/001–031` + `runtime-role-grants.sql` / `worker-role-grants.sql` to a provisioned database.
...
- Prepared source: takeover-safe Google resolution, the Facebook pending-mailbox-verification boundary and authenticated password-account provider connection pass adversarial source tests. Local PostgreSQL execution now passes; activation still requires managed PostgreSQL staging, final HTTPS domain, approved SMTP, real provider credentials and Meta operational review/deletion evidence. Social-only provider step-up and lockout-safe removal remain open; all public controls remain hidden.
