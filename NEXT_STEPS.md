...
- **Account-first booking journey complete behind the attachment gate:** every public Book/Request cleaning action opens `/signup?intent=book`. Configured Google and Facebook starts carry only that fixed action in signed, expiring provider state; their callbacks automatically create or safely reuse the verified Tideway account and preserve the action through Landlord onboarding. Email/password remains available, the browser remembers the allowlisted action for 30 minutes, Landlord is preselected but still confirmed by the user, and an existing Cleaner-only account is never silently changed. Arbitrary or duplicated intent values are rejected to prevent open redirects. Provider controls remain hidden locally until managed infrastructure, the final HTTPS domain and approved credentials are configured.
