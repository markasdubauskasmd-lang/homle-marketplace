...
- **Pre-booking access-secret collection closed:** the working `/request` pilot now asks only for a general access approach and explains that exact instructions are collected privately after acceptance. One shared browser/server rule blocks door, gate, alarm and key-safe codes, passwords and exact hidden-key locations before any record is written; safe approaches such as reception, concierge or office key collection still pass. The 30-minute tab recovery draft removes an accidentally typed secret instead of persisting or restoring it. Unit, draft and isolated HTTP regressions prove helpful rejection and zero lead write. Final exact access still uses the existing protected confirmed-booking pack.
