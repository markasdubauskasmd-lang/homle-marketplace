...
- Prepared source: the provider-neutral workflow and migration 022 freeze authorization to the accepted booking total, hash retry keys, reserve role-bound capture/cancel/refund/transfer commands and keep payout destinations server-owned. Exact Stripe Node 22.1.1 now supplies a test-key-only manual-capture/refund/source-backed-transfer adapter pinned to API `2026-03-25.dahlia`; the raw signed-webhook route preserves exact bytes, rejects live/wrong-version events and ignores unrelated signed events. Composition remains detached and default-off behind both `MARKETPLACE_ENABLED` and `PAYMENTS_ENABLED`. Connect onboarding, participant payment UI, journey authorization gate, approved platform/connected accounts and a real test-mode cycle are still required; no provider was contacted.
