...
- **P0-7 complete in launch documentation**: the Polsia and independent-local scorecards are separated, the current inventory is 167 JavaScript modules / 59 test files, and the unsupported £20 referral claim is inventoried.
- Current full evidence after the default-off Stripe test-adapter checkpoint: 151 syntax/encoding/database/dependency-asset commands and 59 test commands pass (210 total); the production dependency audit reports no known vulnerabilities and real lead/scan data was not touched. The earlier verified source archive remains the tracked `5cc8f99` checkpoint: `../Tideway-independent-source-2026-07-16.zip`, 192 entries, SHA-256 `237D331367EF0B0C85AE5420B8FBDB1E405FEFE4BDDCB8ACB84CFAFDFE18E705`.
...
- **PostgreSQL source activation gate added:** all 23 ordered migrations plus the runtime/worker least-privilege grant scripts are SHA-256 locked and checked for missing, unexpected, reordered, modified or transaction-boundary-damaged assets. This prevents an unreviewed SQL set from silently entering staging, but does not claim the absent real PostgreSQL execution/RLS/concurrency evidence required by E1/E2.
...
- Prepared source: the provider-neutral workflow and migrations 022-023 freeze authorization to the accepted booking total, hash retry keys, reserve role-bound capture/cancel/refund/transfer commands and keep payout destinations server-owned. Exact Stripe Node 22.1.1 supplies a test-key-only manual-capture/refund/source-backed-transfer adapter pinned to API `2026-03-25.dahlia`; the raw signed-webhook route preserves exact bytes, rejects live/wrong-version events and ignores unrelated signed events. A Landlord-only booking-payment endpoint now starts authorization through session/origin/CSRF plus a strong retry key and reads an owner-scoped safe status projection without provider references or client secrets; it is absent unless the complete payment runtime attaches. Composition remains detached and default-off behind both `MARKETPLACE_ENABLED` and `PAYMENTS_ENABLED`. Connect onboarding, Payment Element UI, journey authorization gate, approved platform/connected accounts and a real test-mode cycle are still required; no provider was contacted.
