...
- **PostgreSQL source activation gate added:** all 36 ordered migrations plus the runtime/worker least-privilege grant scripts are SHA-256 locked and checked for missing, unexpected, reordered, modified or transaction-boundary-damaged assets. Migration 031 preserves deployed history while correcting the Cleaner service-area lookup defect found by real execution; migration 032 adds exact-subject social step-up and lockout-safe provider removal; migration 033 adds function-only booking cases; migration 034 adds owner-only first-Administrator provisioning; migration 035 adds function-only account privacy-request intake; migration 036 adds actor-bound Cleaner payout onboarding.
...
- Steps: add and version-lock a maintained Postgres driver; construct a pool from `DATABASE_URL`; compose `createMarketplaceRuntime(pool)` into a real server entrypoint behind capability gates; apply `db/migrations/001–036` + `runtime-role-grants.sql` / `worker-role-grants.sql` to a provisioned database.
