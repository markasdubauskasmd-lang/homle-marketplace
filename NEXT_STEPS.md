...
- **P0-7 complete in launch documentation**: the Polsia and independent-local scorecards are separated, the current inventory is 135 JavaScript modules / 44 test files, and the unsupported £20 referral claim is inventoried.
- Current full evidence: 117 syntax/encoding/database-asset commands and 39 test commands pass; real lead and scan hashes remain unchanged. The earlier verified source archive remains the tracked `5cc8f99` checkpoint: `../Tideway-independent-source-2026-07-16.zip`, 192 entries, SHA-256 `237D331367EF0B0C85AE5420B8FBDB1E405FEFE4BDDCB8ACB84CFAFDFE18E705`.
...
- **E1 fail-closed server attachment prepared:** `server.mjs` now has an isolated, default-off `/api/marketplace/*` dispatch boundary. Enablement requires complete database/secrets/email/storage configuration, a deployment adapter module, shared limiter, trusted client key, email delivery, private media adapter, PostgreSQL 16 and a non-bypass `tideway_app` connection with critical functions present. Failure closes resources and exposes no partial routes. Provider discovery now advertises only attached email routes; configured OAuth credentials alone can no longer claim Google/Apple/Facebook readiness. The reviewed/locked `pg` dependency and real adapters remain outstanding, so the flag stays false.
