...
- **P0-7 complete in launch documentation**: the Polsia and independent-local scorecards are separated, the current inventory is 147 JavaScript modules / 50 test files, and the unsupported £20 referral claim is inventoried.
- Current full evidence: 130 syntax/encoding/database/dependency-asset commands and 45 test commands pass; real lead and scan hashes remain unchanged. The earlier verified source archive remains the tracked `5cc8f99` checkpoint: `../Tideway-independent-source-2026-07-16.zip`, 192 entries, SHA-256 `237D331367EF0B0C85AE5420B8FBDB1E405FEFE4BDDCB8ACB84CFAFDFE18E705`.
...
- **E1 fail-closed server attachment prepared; driver, client identity and SMTP boundaries complete:** `server.mjs` owns an isolated, default-off `/api/marketplace/*` dispatch boundary. The attachment constructs the shared limiter, trusted client resolver and strict-TLS SMTP delivery internally. The mail transport rejects URL options, IP hosts, weak certificate settings, header injection, off-origin auth links, file/URL fetching and debug logs; startup verifies SMTP only after the database probe, then closes every resource on failure. Exact Nodemailer 9.0.3 has no transitive dependency and joins `pg` 8.22.0 in the locked graph. Enablement still requires real PostgreSQL/SMTP/proxy evidence and a deployment adapter for private media/monitoring, so the flag stays false.
...
2. **The detached marketplace** — `src/marketplace/*` + `db/migrations/001–020` (PostgreSQL, RLS, auth, bookings, matching, tracking, reviews, shared limiting and maintenance). Exact `pg`/Nodemailer dependencies, the database limiter, strict SMTP delivery and fail-closed direct/single-proxy client resolver are integrated behind the default-off attachment. **It still cannot be enabled safely:** no PostgreSQL 16 or SMTP provider has passed staging, the final proxy topology is unverified, and no approved deployment adapter supplies private storage or monitoring (F2, F8).
