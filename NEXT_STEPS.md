...
- **P0-7 complete in launch documentation**: the Polsia and independent-local scorecards are separated, the current inventory is 145 JavaScript modules / 49 test files, and the unsupported £20 referral claim is inventoried.
- Current full evidence: 128 syntax/encoding/database/dependency-asset commands and 44 test commands pass; real lead and scan hashes remain unchanged. The earlier verified source archive remains the tracked `5cc8f99` checkpoint: `../Tideway-independent-source-2026-07-16.zip`, 192 entries, SHA-256 `237D331367EF0B0C85AE5420B8FBDB1E405FEFE4BDDCB8ACB84CFAFDFE18E705`.
...
- **E1 fail-closed server attachment prepared; driver and client-identity boundaries complete:** `server.mjs` owns an isolated, default-off `/api/marketplace/*` dispatch boundary. The attachment now constructs both the shared limiter and trusted client resolver internally. Direct mode ignores forwarding headers; single-proxy mode requires a configured immediate-peer CIDR plus exactly one sanitized client IP and otherwise fails closed. Enablement still requires complete database/secrets/email/storage configuration, a deployment adapter module for email/private media/monitoring, PostgreSQL 16 and a non-bypass `tideway_app` connection with critical functions present. `pg` 8.22.0 and pnpm 11.7.0 are exact and integrity gated. The real PostgreSQL run, final proxy evidence and remaining deployment adapters are outstanding, so the flag stays false.
...
2. **The detached marketplace** — `src/marketplace/*` + `db/migrations/001–020` (PostgreSQL, RLS, auth, bookings, matching, tracking, reviews, shared limiting and maintenance). The exact `pg` driver, database limiter and fail-closed direct/single-proxy client resolver are integrated behind the default-off attachment. **It still cannot be enabled safely:** no PostgreSQL 16 database has executed the migrations/RLS/concurrency suite, the final proxy topology is unverified, and no approved deployment adapter supplies email, private storage or monitoring (F2, F8).
