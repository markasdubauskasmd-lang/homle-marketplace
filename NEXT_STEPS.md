...
- **P0-7 complete in launch documentation**: the Polsia and independent-local scorecards are separated, the current inventory is 141 JavaScript modules / 47 test files, and the unsupported £20 referral claim is inventoried.
- Current full evidence: 124 syntax/encoding/database/dependency-asset commands and 42 test commands pass; real lead and scan hashes remain unchanged. The earlier verified source archive remains the tracked `5cc8f99` checkpoint: `../Tideway-independent-source-2026-07-16.zip`, 192 entries, SHA-256 `237D331367EF0B0C85AE5420B8FBDB1E405FEFE4BDDCB8ACB84CFAFDFE18E705`.
...
2. **The detached marketplace** — `src/marketplace/*` + `db/migrations/001–019` (PostgreSQL, RLS, auth, bookings, matching, tracking, reviews and maintenance). The exact `pg` driver is now declared and locked, and the main server has a default-off attachment. **It still cannot be enabled safely:** no PostgreSQL 16 database has executed the migrations/RLS/concurrency suite, and no approved deployment adapter supplies shared limiting, email, private storage or monitoring (F2, F8).
