...
- **PostgreSQL source activation gate added:** all 35 ordered migrations plus the runtime/worker least-privilege grant scripts are SHA-256 locked and checked for missing, unexpected, reordered, modified or transaction-boundary-damaged assets. Migration 031 preserves deployed history while correcting the Cleaner service-area lookup defect found by real execution; migration 032 adds exact-subject social step-up and lockout-safe provider removal; migration 033 adds function-only booking cases; migration 034 adds owner-only first-Administrator provisioning; migration 035 adds function-only account privacy-request intake.
- **PostgreSQL deployment verifier passes locally:** on 16 July 2026, a fresh disposable PostgreSQL 16.14 database bound only to loopback passed the read-only audit for 40 RLS tables, restricted roles, ownership, critical constraints/indexes, trusted definer functions, 36 app-function checks, 13 worker-function checks and protected-table revocations. The report counts are guarded against drifting from the exact function arrays. The URL stayed out of process arguments. Repeat this proof against approved managed staging before attachment.
...
2. **The detached marketplace** — `src/marketplace/*` + `db/migrations/001–035` (PostgreSQL, RLS, auth, bookings, matching, consent-bound dispatch, tracking, reviews, disputes, privacy requests, private media, shared limiting, payments, maintenance and first-Administrator bootstrap). Exact database/mail/S3/image dependencies plus the limiter, SMTP delivery, object storage and client resolver are integrated behind the default-off attachment. Local PostgreSQL 16.14 execution now passes. **It still cannot be enabled safely:** managed PostgreSQL, SMTP and a real private bucket have not passed staging; CORS/lifecycle/public-access/threat controls and final proxy topology are unverified; approved monitoring remains absent (F2, F8).
...
- Steps: add and version-lock a maintained Postgres driver; construct a pool from `DATABASE_URL`; compose `createMarketplaceRuntime(pool)` into a real server entrypoint behind capability gates; apply `db/migrations/001–035` + `runtime-role-grants.sql` / `worker-role-grants.sql` to a provisioned database.
