...
- **P0-7 complete in launch documentation**: the Polsia and independent-local scorecards are separated, the current inventory is 143 JavaScript modules / 48 test files, and the unsupported £20 referral claim is inventoried.
- Current full evidence: 126 syntax/encoding/database/dependency-asset commands and 43 test commands pass; real lead and scan hashes remain unchanged. The earlier verified source archive remains the tracked `5cc8f99` checkpoint: `../Tideway-independent-source-2026-07-16.zip`, 192 entries, SHA-256 `237D331367EF0B0C85AE5420B8FBDB1E405FEFE4BDDCB8ACB84CFAFDFE18E705`.
...
- **E3 takeover hardening and Google OIDC complete in detached source:** verified social-provider email may deduplicate automatically only into an already verified social-only account. Pre-authenticated callbacks cannot attach to any password account—even a verified one—and must use a future authenticated settings/step-up flow. An attacker-pre-registered unverified account is never upgraded or linked. Google now has signed short-lived state, nonce, PKCE, server-only code exchange, bounded JWKS/RS256 verification, exact claim validation, secure callback/session handoff and capability-gated mobile UI. It remains disabled until real PostgreSQL tests, HTTPS origin and verified credentials pass. Facebook remains disabled because its email must not be treated as verified without a separate Tideway verification/linking flow.
- **PostgreSQL source activation gate added:** all 20 ordered migrations plus the runtime/worker least-privilege grant scripts are SHA-256 locked and checked for missing, unexpected, reordered, modified or transaction-boundary-damaged assets. This prevents an unreviewed SQL set from silently entering staging, but does not claim the absent real PostgreSQL execution/RLS/concurrency evidence required by E1/E2.
...
- **E1 fail-closed server attachment prepared; driver lock complete:** `server.mjs` owns an isolated, default-off `/api/marketplace/*` dispatch boundary. Enablement requires complete database/secrets/email/storage configuration, a deployment adapter module, shared limiter, trusted client key, email delivery, private media adapter, PostgreSQL 16 and a non-bypass `tideway_app` connection with critical functions present. `pg` 8.22.0 and pnpm 11.7.0 are now exact, and the complete transitive lockfile is SHA-256/integrity gated; a frozen isolated install and `pg.Pool` import pass without creating project `node_modules`. `pnpm audit --prod --audit-level high` reported no known vulnerabilities on 16 July 2026; this is checkpoint evidence, not a future guarantee. Provider discovery advertises only attached routes. The real PostgreSQL run and deployment adapters remain outstanding, so the flag stays false.
...
2. **The detached marketplace** — `src/marketplace/*` + `db/migrations/001–020` (PostgreSQL, RLS, auth, bookings, matching, tracking, reviews, shared limiting and maintenance). The exact `pg` driver and database limiter are now integrated behind the default-off attachment. **It still cannot be enabled safely:** no PostgreSQL 16 database has executed the migrations/RLS/concurrency suite, and no approved deployment adapter supplies trusted client identity, email, private storage or monitoring (F2, F8).
...
- Prepared source: takeover-safe resolution and the complete Google OIDC/callback/session boundary pass adversarial source tests. Activation still requires the E2 PostgreSQL run, final HTTPS domain and real Google Web OAuth credentials. Facebook requires a separate verified-email or authenticated step-up design and remains hidden.
