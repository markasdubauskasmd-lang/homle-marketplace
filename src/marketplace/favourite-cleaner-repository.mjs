export function createFavouriteCleanerRepository(database) {
  ...
  return Object.freeze({
    listOwn(actor) {
      return database.withUserTransaction(actor, async (client) => {
        const result = await client.query(
          `SELECT cleaner.cleaner_id, cleaner.public_slug, cleaner.profile_photo_url,
             cleaner.current_availability_status, cleaner.average_rating, cleaner.review_count,
             cleaner.completed_job_count, cleaner.display_name, favourite.created_at,
             cleaner.services
           FROM favourite_cleaners favourite
           JOIN LATERAL tideway_private.get_public_cleaner_profile(favourite.cleaner_user_id) cleaner ON true
    ...
    setOwn(actor, cleanerId, favourite) {
      return database.withUserTransaction(actor, async (client) => {
        if (favourite) {
          const visible = await client.query("SELECT 1 FROM tideway_private.get_public_cleaner_profile($1::uuid) LIMIT 1", [cleanerId]);
