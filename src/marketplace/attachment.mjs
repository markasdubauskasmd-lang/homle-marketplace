...
export async function probeMarketplaceDatabase(pool) {
  ...
  try {
    ...
    if (row.lookup_session_ready !== true || row.booking_workflow_ready !== true || row.rate_limit_ready !== true || row.facebook_pending_identity_ready !== true || row.payment_ledger_ready !== true || row.payment_access_ready !== true || row.payment_journey_gate_ready !== true) throw new Error("Marketplace database migrations or runtime grants are incomplete.");
