...
function verifiedFlow(value, secret, nowSeconds) {
  ...
  if (![undefined, "", "book", "work"].includes(payload.intent)) throw new TypeError("The Google sign-in attempt is missing or expired.");
...
export function createGoogleOidcProvider(options = {}) {
  ...
  return Object.freeze({
    ...
    begin(options = {}) {
      ...
      if (!["", "book", "work"].includes(intent) || (purpose !== "sign-in" && intent)) throw new TypeError("Google sign-in intent is invalid.");
      ...
      const payload = { v: 1, purpose, intent, state, nonce, verifier, iat: nowSeconds, exp: nowSeconds + flowLifetimeSeconds };
