...
// Structured scans carry up to 20 rooms with 200 item groups each. Allow
// their bounded labels, keys, evidence and notes in UTF-8 without truncation.
// This allowance is used only by structured scan/assessment routes; ordinary
// JSON and photographed-room request limits remain independent.
export const maximumRoomScanBodyBytes = 6 * 1024 * 1024;
...
export function errorResponse(error) {
  ...
  if ([400, 403, 404, 409, 410, 413, 422, 429, 503].includes(error?.statusCode)) return { statusCode: error.statusCode, code: error.code || ({ 400: "invalid-request", 403: "forbidden", 404: "not-found", 409: "conflict", 410: "gone", 413: "request-too-large", 422: "validation-failed", 429: "rate-limited", 503: "temporarily-unavailable" }[error.statusCode]), message: error.message };
