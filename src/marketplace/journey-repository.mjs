function mapJourneyError(error) {
  const errors = {
    ...
    "payment-authorization-required": [409, "payment-authorization-required", "The Landlord must authorize the current booking total before this journey can start."],
    ...
    getJourneyContext(actor, bookingId) {
      return database.withUserTransaction(actor, async (client) => {
        const result = await client.query(
          "SELECT booking.scheduled_start_at, property.latitude AS destination_latitude, property.longitude AS destination_longitude, tideway_private.current_booking_payment_authorized(booking.id) AS payment_authorized FROM bookings booking JOIN properties property ON property.id=booking.property_id WHERE booking.id=$1::uuid AND booking.cleaner_user_id=$2::uuid AND booking.status IN ('confirmed', 'cleaner-en-route')",
