...
const expectedSvixVersion = "2.0.0";
...
const expectedLockSha256 = "76b6f13a2983e095949d72d1f02c0b0dc1f0689512da12232e11880a25267cde";
...
const expectedSvixIntegrity = "sha512-rywid6EHCJZwonqXCuo3nF/tovnz+CIoHlH0sMdxCTtgKwTRNmqJn2TCM40DTvMZUgshLhiP1KUfOZoTrli2/Q==";
...
export function validateDependencyAssets(packageJsonText, lockText) {
  ...
  if (dependencyNames.join(",") !== "@anthropic-ai/sdk,@aws-sdk/client-s3,@aws-sdk/s3-request-presigner,nodemailer,pg,sharp,stripe,svix") throw new TypeError("Only the reviewed database, SMTP, private-media, payment, signed-webhook and walkthrough-summary runtime dependencies are allowed in the production manifest.");
  ...
  if (manifest.dependencies.svix !== expectedSvixVersion) throw new TypeError(`svix must be pinned exactly to ${expectedSvixVersion}.`);
  ...
  if (!lockText.includes(`svix@${expectedSvixVersion}:`) || !lockText.includes(`resolution: {integrity: ${expectedSvixIntegrity}}`)) throw new TypeError("The lockfile does not contain the reviewed Svix webhook verifier and integrity evidence.");
  ...
  return Object.freeze({ dependencies: Object.freeze({ awsS3: expectedS3Version, nodemailer: expectedMailerVersion, pg: expectedDriverVersion, sharp: expectedSharpVersion, stripe: expectedStripeVersion, svix: expectedSvixVersion }), lockSha256: actualHash });
}
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(fileURLToPath(import.meta.url))) {
  ...
  console.log(`Dependency assets verified: AWS S3 ${result.dependencies.awsS3}, nodemailer ${result.dependencies.nodemailer}, pg ${result.dependencies.pg}, sharp ${result.dependencies.sharp}, Stripe ${result.dependencies.stripe}, Svix ${result.dependencies.svix}, lock SHA-256 ${result.lockSha256}.`);
}

export const dependencyLockEvidence = Object.freeze({ expectedDriverVersion, expectedMailerVersion, expectedS3Version, expectedSharpVersion, expectedStripeVersion, expectedSvixVersion, expectedLockSha256, expectedDriverIntegrity, expectedMailerIntegrity, expectedS3ClientIntegrity, expectedS3PresignerIntegrity, expectedSharpIntegrity, expectedStripeIntegrity, expectedSvixIntegrity });
