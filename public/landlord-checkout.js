...
function showState(title, copy, { kind = "info", allowSignIn = false, allowRetry = false } = {}) {
  ...
  if (allowSignIn) signIn.href = "/login?intent=book";
...
async function load() {
  ...
  try {
    ...
    else if ([404, 503].includes(error.statusCode)) showState("Secure account payments are not available", "No payment was attempted. This booking or the protected Stripe test runtime is not ready yet.", { kind: "unavailable", allowRetry: true });
