...
import { readSignedInAccount } from "./account-menu.js?v=20260729-1";
import { createRequestJson } from "./request-json.js";
import { unreadBadge, workspaceShell } from "./workspace-shell-model.js";

// Keep the frozen shared account-menu module byte-for-byte unchanged. The
// workspace shell creates its account controls after modules have evaluated,
// so it owns the small dynamic-control binding instead of extending the shared
// module used by the Cleaner workspace.
const accountRequest = createRequestJson({
  failureMessage: "Account action failed.",
  timeoutMs: 15_000,
  timeoutMessage: "Sign out took too long. It may have completed; reload before trying again."
});
...
async function recoverCsrf() {
  ...
  const result = await accountRequest("/api/marketplace/auth/session", {
    method: "POST",
...
function bindWorkspaceSignOut(button) {
  ...
  button.addEventListener("click", async () => {
    ...
    if (!navigator.onLine) {
      if (status) { status.textContent = "Reconnect before signing out securely."; status.hidden = false; }
      return;
    }
    ...
    try {
      ...
      await accountRequest("/api/marketplace/auth/logout", {
        method: "POST",
      ...
      if (status) {
        status.textContent = error?.message || "Homle could not sign you out. Please try again.";
        status.hidden = false;
      }
...
export async function renderWorkspaceShell(config = {}) {
  ...
  for (const button of document.querySelectorAll("[data-account-sign-out]")) bindWorkspaceSignOut(button);
