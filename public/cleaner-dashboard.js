...
function renderSetupSteps(progress) {
  ...
  // Identity / DBS / insurance chips. Only the two checks Homle actually stores can read
  // as verified; insurance has no record anywhere, so it never claims one.
  ...
  if (checkHost) {
    ...
    const chips = [
      ...
      ["DBS", byKey.get("dbs")],
      ["Insurance", byKey.get("insurance")]
    ];
    checkHost.replaceChildren(...chips.map(([label, step]) => {
      const chip = element("span", "hc-setup-chip");
      const state = !step?.tracked ? "held by Homle" : step.done ? "verified ✓" : "outstanding";
      chip.append(document.createTextNode(`${label} `), element("i", "", "·"), document.createTextNode(` ${state}`));
      return chip;
    }));
  }
}
...
function renderWorkOverview(summary, capabilities, buckets) {
  ...
  // Homle dispatches by invitation; there is no browse-jobs-in-your-area feature behind
  // the design's "jobs available" tile, so it shows the offers actually waiting.
