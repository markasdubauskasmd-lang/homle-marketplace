...
function renderSetupSteps(summary) {
  ...
  // Identity / DBS / insurance chips in the design are document checks. Homle has no
  // document upload or verification provider, so this states who holds them instead of
  // showing a status Homle cannot evidence.
  const chipHost = document.querySelector("[data-cleaner-check-chips]");
  if (chipHost) {
    const chip = element("span", "hc-setup-chip");
    chip.append(document.createTextNode("Identity, DBS and insurance "), element("i", "", "·"), document.createTextNode(" verified with the Homle team"));
