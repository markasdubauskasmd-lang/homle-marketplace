...
(() => {
  ...
  function collectProps(node, kind, host) {
    ...
    for (const { name, value } of [...node.attributes]) {
      ...
      let key = name === 'data-preview-style' ? 'style' : name.replace(/^data-preview-on/, 'on');
  ...
  function walkComponent(el, host) {
    ...
    const styleRaw = el.getAttribute("data-preview-style");
    el.removeAttribute("data-preview-style");
  ...
  function createHelmetManager(doc, isStreaming) {
    ...
    function setDesignDocMode(mode) {
      ...
      if (mode === "canvas") {
        ...
        canvasStyleEl = previewStyle(doc);
  ...
  function init() {
    ...
    let rootName = "Root";
    const baseCss = previewStyle(document);
