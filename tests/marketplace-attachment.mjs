...
assert(realtimeProbeQueries.some((query) => String(query).includes("LISTEN tideway_booking_events")) && realtimeProbeQueries.some((query) => String(query).includes("UNLISTEN tideway_booking_events")), "The dedicated real-time connection did not prove booking LISTEN/UNLISTEN support.");
assert(realtimeProbeQueries.some((query) => String(query).includes("LISTEN tideway_account_events")) && realtimeProbeQueries.some((query) => String(query).includes("UNLISTEN tideway_account_events")), "The dedicated real-time connection did not prove account notification LISTEN/UNLISTEN support.");
assert.ok(probeQueries[0].includes("current_user") && probeQueries[0].includes("tideway_private.lookup_session") && probeQueries[0].includes("list_my_booking_summaries") && probeQueries[0].includes("configure_automatic_dispatch") && probeQueries[0].includes("get_cleaning_request_realtime_snapshot") && probeQueries[0].includes("submit_cleaning_request") && probeQueries[0].includes("withdraw_cleaning_request") && probeQueries[0].includes("reschedule_open_cleaning_request") && probeQueries[0].includes("create_request_photo_upload_intent") && probeQueries[0].includes("connect_social_identity") && probeQueries[0].includes("request_my_privacy_action") && probeQueries[0].includes("request_facebook_data_deletion") && probeQueries[0].includes("begin_my_cleaner_payout_onboarding"));
