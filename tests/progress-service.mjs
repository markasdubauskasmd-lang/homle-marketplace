...
function progress(overrides = {}) {
  return {
    ...
    tasks: [{ taskId, roomName: "Kitchen", description: "Clean worktops", status: "completed", unexpected: false, cleanerFrozenTermsConfirmed: false, cleanerFrozenTermsConfirmedAt: null, landlordApprovalStatus: null, latestNote: "Done", updatedAt: "2026-07-20T09:10:00.000Z", updatedBy: cleaner.userId }],
...
const fakeRepository = {
  ...
  async addUnexpectedTask(actor, id, input) { calls.push({ kind: "add", actor, id, input }); return progress({ totalTasks: 3, tasks: [...progress().tasks, { taskId: "99999999-9999-4999-8999-999999999999", ...input, status: "not-started", unexpected: true, cleanerFrozenTermsConfirmed: true, cleanerFrozenTermsConfirmedAt: "2026-07-20T09:12:00.000Z", landlordApprovalStatus: "pending", latestNote: input.note, updatedAt: "2026-07-20T09:12:00.000Z", updatedBy: actor.userId }] }); },
  async confirmUnexpectedTaskTerms(actor, id, suppliedTaskId) { calls.push({ kind: "confirm-terms", actor, id, suppliedTaskId }); return progress({ tasks: [{ ...progress().tasks[0], unexpected: true, cleanerFrozenTermsConfirmed: true, cleanerFrozenTermsConfirmedAt: "2026-07-20T09:13:00.000Z", landlordApprovalStatus: "pending" }] }); },
...
const added = await service.addUnexpectedTask(cleaner, bookingId, { roomName: "Hall", description: "Remove unexpected packaging", estimatedAdditionalMinutes: 20, withinBookedTermsConfirmed: true, note: "Approval required" });
const confirmedTerms = await service.confirmUnexpectedTaskTerms(cleaner, bookingId, "99999999-9999-4999-8999-999999999999");
...
assert(started.status === "cleaning-in-progress" && paused.isPaused && task.tasks[0].status === "issue-reported" && added.tasks.at(-1).cleanerFrozenTermsConfirmed === true && confirmedTerms.tasks[0].cleanerFrozenTermsConfirmed === true && finished.status === "awaiting-review" && landlordView.eventVersion === 4, "Cleaning start, pause, task, frozen-terms confirmation, unexpected approval, finish or participant projection failed.");
...
assert(await rejects(() => service.addUnexpectedTask(cleaner, bookingId, { roomName: "Hall", description: "Extra work", estimatedAdditionalMinutes: 20 }), "remaining booked time"), "A Cleaner proposed extra work without accepting the frozen time and pay boundary.");
...
await repository.addUnexpectedTask(cleaner, bookingId, { roomName: "Hall", description: "Clear packaging", estimatedAdditionalMinutes: 15, withinBookedTermsConfirmed: true, note: null });
await repository.confirmUnexpectedTaskTerms(cleaner, bookingId, taskId);
...
for (const name of ["get_cleaning_progress", "start_booking_cleaning", "set_booking_cleaning_pause", "update_booking_cleaning_task", "add_unexpected_cleaning_task", "confirm_unexpected_task_frozen_terms", "decide_unexpected_cleaning_task", "finish_booking_cleaning"]) assert(databaseCalls.some((call) => call.text.includes(name)), `Progress repository did not call ${name}.`);
...
const frozenTermsMigration = await readFile(new URL("../db/migrations/039_unexpected_task_frozen_terms.sql", import.meta.url), "utf8");
...
for (const required of ["frozen_terms_confirmed_by_cleaner", "confirm_unexpected_task_frozen_terms", "unexpected-task-terms-unconfirmed", "unexpected-task-exceeds-booked-time", "scheduled_end_at", "priceChange", "supplied_frozen_terms_confirmation IS NOT TRUE"]) assert(frozenTermsMigration.includes(required), `Unexpected-task economics hardening omitted ${required}.`);
assert(grants.includes("add_unexpected_cleaning_task(uuid,text,text,integer,boolean,text)") && grants.includes("confirm_unexpected_task_frozen_terms(uuid,uuid)"), "The runtime role cannot use the hardened unexpected-task terms workflow.");

console.log("Progress tests passed: role-bound start/pause/task/issue/finish, explicit frozen-time/pay consent before unexpected-task approval, booked-time enforcement, resolved-task gate, durable actor events and participant-only projections.");
