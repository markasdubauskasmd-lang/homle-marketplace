...
import { cleanerApplicationDraftLifetimeMs, clearCleanerApplicationDraft, readCleanerApplicationDraft, saveCleanerApplicationDraft } from "../public/cleaner-application-draft.js";
...
assert.equal(readCleanerApplicationDraft(storage, now + 10_000)?.fields.email, "applicant@example.com");
...
saveCleanerApplicationDraft(storage, { fields: { fullName: "Temporary" }, currentStep: 9 }, now);
assert.equal(readCleanerApplicationDraft(storage, now)?.currentStep, 3, "Restored steps must stay inside the three-stage application.");
