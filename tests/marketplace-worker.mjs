...
assert.equal(maintenanceJobs.length, 7);
...
const zeroRepository = Object.fromEntries(["expireInvitations", "purgeLocations", "purgeSessions", "purgeRateLimits", "purgePendingSocialIdentities"].map((name) => [name, async () => ({ processedCount: 0, batchFull: false })]));
const runtime = createMarketplaceWorkerRuntime({ query() {} }, { createMaintenanceRepository: () => zeroRepository, onUnexpectedError() {} });
assert.deepEqual(runtime.snapshot().jobs.map((job) => job.name), ["invitation-expiry", "location-expiry", "session-expiry", "rate-limit-retention", "social-identity-retention"]);
await runtime.close();
...
const verification = await runPostgresWorkerVerification({
  ...
  probeDatabase: async () => ({ postgresqlVersionNumber: 160014, functionCount: 13 }),
