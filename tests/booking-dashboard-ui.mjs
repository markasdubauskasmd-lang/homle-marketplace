...
assert(cleanerPage.includes("data-cleaner-next") && cleanerPage.includes("Do this next") && cleanerPage.includes('href="/cleaner/availability"') && cleanerScript.includes("function renderNextAction(buckets, payout, availability)") && cleanerScript.includes('link.textContent = "Review request"') && cleanerScript.includes('link.textContent = active ? "Open active job" : "View job checklist"') && cleanerScript.includes('link.href = "/cleaner/availability"') && cleanerScript.includes('link.href = "/cleaner/payouts"'), "The Cleaner dashboard makes the Cleaner search multiple sections instead of presenting one role-relevant next action, including exact availability.");
