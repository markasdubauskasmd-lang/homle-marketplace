...
assert(cleanerPage.includes("data-cleaner-next") && cleanerPage.includes("Do this next") && cleanerScript.includes("function renderNextAction(buckets)") && cleanerScript.includes('link.textContent = "Review request"') && cleanerScript.includes('link.textContent = active ? "Open active job" : "View job checklist"'), "The Cleaner dashboard makes the Cleaner search multiple sections instead of presenting one role-relevant next action.");
