...
assert(page.includes("/styles.css?v=20260728-3") && page.includes("/noncleaner-scroll.css?v=20260814-1") && page.includes("/landlord-journey-property.css?v=20260831-1") && page.includes("/landlord-supply-recovery.css?v=20260821-1") && page.includes("/landlord-journey.js?v=journey16") && script.includes("./landlord-journey-model.js?v=journey9"), "The property-first journey, page styling, zero-supply recovery, isolated non-Cleaner scrolling or matching model can remain stuck behind previous cached assets.");
assert(script.includes("selectScanProperty(property)") && script.includes('setRequestScopeValue("propertyId", property.propertyId)') && script.includes("checkSupply(parsed.outward)"), "Selecting a saved property does not bind its private postcode to coverage and the rest of the journey.");
...
assert(/async function checkSupply[\s\S]{0,1100}requestJson\(`\/api\/marketplace\/cleaners\?outwardPostcode=[\s\S]{0,220}timeoutMs: DIRECTORY_REQUEST_TIMEOUT_MS/.test(script), "The optional area-supply check can leave the journey waiting indefinitely.");
assert(/async function loadCleaners[\s\S]{0,1400}requestJson\(`\/api\/marketplace\/cleaners\?\$\{params\}`[\s\S]{0,160}timeoutMs: DIRECTORY_REQUEST_TIMEOUT_MS/.test(script), "The Cleaner-choice step can remain stuck on a slow directory response.");
...
assert(script.includes('supplyLink.href = "/landlord/help"') && script.includes('supplyLink.target = "_blank"') && script.includes('supplyLink.rel = "noopener"') && script.includes("Continue to save this exact request for Homle review") && script.includes("No Cleaner is contacted and no payment is taken") && script.includes('classList.add("empty-supply")'), "Zero Cleaner supply remains a dead end, makes an untrue notification promise, destroys the in-progress journey or does not expose customer help.");
...
await import("./customer-journey-recovery.mjs");

await import("./customer-service-options.mjs");
