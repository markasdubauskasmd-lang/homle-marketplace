...
assert(script.includes("function showRequestCompletion(submission") && script.includes("submission?.cleaningRequestId") && script.includes("submission?.photoCount") && script.includes("submission?.taskCount") && script.includes('history.replaceState(null, "", "/landlord/dashboard")') && script.includes("showRequestCompletion(submission, { automaticDispatch: auto.checked })") && script.includes("could not verify automatic invitation authorisation") && script.includes('document.querySelector("[data-request-complete-another]")'), "The private completion state cannot show the submitted reference/scope, clear the booking-start action or recover safely from an uncertain dispatch response.");
assert(page.includes("The walkthrough stays in this browser") && page.includes("Changing the walkthrough or checklist clears the review confirmation") && script.includes("function invalidateScopeReview(message)") && script.includes("confirmation.checked = false") && script.includes('requestForm.elements.transcript.addEventListener("input"') && script.includes('requestForm.elements.tasks.addEventListener("input"') && script.includes("The spoken walkthrough changed") && script.includes("The concise checklist changed"), "The speech-to-checklist handoff can retain a stale scope confirmation or misstate transcript storage.");
...
console.log("Landlord dashboard UI tests passed: owner APIs, account-to-room-scan continuation, private completion, change-sensitive speech scope, exact draft scope, scan-first boundary, safe rendering and mobile accessibility.");
