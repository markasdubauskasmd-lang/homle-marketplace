...
import "./property-form-draft.mjs";
...
const manualPricingRequest = pricingRequestFromManualTasks(tasks, { cleaningType: "deep-cleans", frequency: "fortnightly", requestedMinutes: 180 });
assert(manualPricingRequest.serviceType === "deep" && manualPricingRequest.frequency === "fortnightly" && manualPricingRequest.requestedMinutes === 180 && manualPricingRequest.rooms.length === 2 && manualPricingRequest.rooms[0].roomType === "kitchen" && manualPricingRequest.rooms[1].items[0].code === "limescale", "Manual room tasks or the selected duration cannot enter the same server-authoritative pricing model as a scan.");
...
assert(script.includes('requestJson("/api/marketplace/landlord/bootstrap")') && script.includes('const path = updating ? `/api/marketplace/properties/${encodeURIComponent(selectedPropertyId)}` : "/api/marketplace/properties"') && script.includes('saveManualRequest(csrf, body)') && requestRecovery.includes('requestJson("/api/marketplace/cleaning-requests"') && requestRecovery.includes('"X-CSRF-Token": csrf') && script.includes('"X-CSRF-Token": csrf'), "The workspace is not bound to one authenticated Landlord bootstrap, owner mutations and CSRF-protected writes.");
...
assert(script.includes('requestedMinutes: Number(data.get("durationMinutes"))') && script.includes("paymentsReady = health?.marketplace?.paymentsReady === true") && script.includes("requestCompleteSandbox.hidden = !paymentsReady") && page.includes('href="/stripe-sandbox?start=1"'), "Manual requests can still be created without their selected duration in the frozen server quote or lose the fail-closed Stripe test capability gate.");
