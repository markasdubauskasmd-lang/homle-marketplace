...
assert(script.indexOf("const primaryWorkspaceReads = Promise.allSettled") < script.indexOf('const accountResult = await requestJson("/api/marketplace/account")') && script.indexOf("renderAccountAvatar(account)") < script.indexOf("await primaryWorkspaceReads") && !script.slice(script.indexOf("const primaryWorkspaceReads = Promise.allSettled"), script.indexOf('const accountResult = await requestJson("/api/marketplace/account")')).includes("favourite-cleaners"), "The independently authorised Landlord reads no longer overlap account verification, reveal data before the role check, or pull optional Saved Cleaners back into primary startup.");
