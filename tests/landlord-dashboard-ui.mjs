...
assert(page.includes("automatically creates concise room tasks") && script.includes("function invalidateScopeReview(message)") && script.includes("confirmation.checked = false") && script.includes('requestForm.elements.transcript.addEventListener("input"') && script.includes('requestForm.elements.tasks.addEventListener("input"') && script.includes("summariseSpeech({ automatic: true })") && script.includes("Concise room tasks were updated automatically"), "The speech-to-checklist handoff can retain a stale confirmation or still requires a separate summary click after speech.");
...
console.log("Landlord dashboard UI tests passed: one-next-action guidance, reduced decisions, automatic speech summary, owner APIs, direct room-scan continuation, safe rendering and mobile accessibility.");
