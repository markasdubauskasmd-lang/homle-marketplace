...
assert(page.includes("data-match-outcome-dialog") && page.includes("No Cleaner is free at this time") && page.includes("data-match-outcome-result") && page.includes("data-match-outcome-timing") && page.includes("data-match-outcome-date") && page.includes("data-match-outcome-start") && page.includes("data-match-outcome-duration") && page.includes("data-match-outcome-builder-mount") && script.includes("function showNoEligibleCleanerOutcome(requestId)") && script.includes("function prepareAnotherTime(requestId)") && script.includes("function continueAnotherTime(requestId)") && script.includes('showMatchOutcomeStep("timing")') && script.includes('showMatchOutcomeStep("result")') && script.includes("matchOutcomeBuilderMount.append(requestBuilderPanel)") && script.includes("requestBuilderDialog?.append(requestBuilderPanel)") && !script.includes('element("a", "button button-outline", "Open Stripe test checkout")') && v2Styles.includes(".landlord-match-outcome-timing-fields") && v2Styles.includes(".landlord-match-outcome-dialog.is-builder-sequence"), "A no-match result still leaves its popup to choose another time or review the replacement, links to an unrelated Stripe test, or cannot preserve the saved request throughout one modal sequence.");
...
assert(page.includes("data-request-complete-sandbox") && page.includes("Test Stripe payment now (&pound;0.30)") && page.includes("opens Stripe immediately in test mode") && page.includes("does not confirm this cleaning booking") && page.indexOf("data-request-complete-quote-note") < page.indexOf("data-request-complete-sandbox") && page.indexOf("data-request-complete-sandbox") < page.indexOf('class="completion-next"'), "The submitted request screen does not place the authenticated Stripe test payment directly after the price or distinguish it from a real accepted booking.");
...
// itself expands on click while the reveal button stays the accessible control.
{
  ...
  assert(page.includes('class="pac-head-icon"') && page.includes("Type the basics") && page.includes("Or speak the rooms") && page.includes("One exact price"), "The banner lost the icon badge or the chips that distinguish the manual path from the scan.");
...
assert(page.includes('class="ld-mobile-brand" href="/"') && /\.ld-mobile-brand\s*\{[^}]*grid-area:\s*brand;[^}]*display:\s*inline-grid;/s.test(mobileShellRules), "Removing the oversized mobile header also removed the compact Homle identity from the authenticated workspace.");
