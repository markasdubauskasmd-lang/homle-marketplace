...
assert(script.includes("requestResult.cleaningRequests") && script.includes("result.cleaningRequest") && !script.includes("requestResult.requests") && !script.includes("result.request)"), "The Landlord workspace does not consume the server's cleaning-request response contract.");
