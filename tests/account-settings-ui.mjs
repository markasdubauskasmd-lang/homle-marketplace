...
const [page, script, styles, server, authenticationHttp, migration, providerSecurityMigration, appleMigration, privacyMigration, grants] = await Promise.all([
  ...
  readFile(new URL("../src/marketplace/authentication-http.mjs", import.meta.url), "utf8"),
...
assert.match(page, /data-connect-provider="apple" hidden/);
...
assert.ok(authenticationHttp.includes('`/settings#provider=${provider}-connected`') && authenticationHttp.includes('`/settings#provider=${provider}-verified`') && authenticationHttp.split('accountFlow ? "/settings" : "/login"').length - 1 === 3, "A social-provider connection can still complete on a missing account destination.");
...
assert.ok(appleMigration.includes("'apple-start','apple-callback'") && appleMigration.includes("asserted_provider NOT IN ('google','apple','facebook')") && appleMigration.includes("identity.provider IN ('google','apple','facebook')") && appleMigration.includes("WHEN 'apple' THEN 2"), "Apple connection, step-up, removal or rate-limit support is missing from the function-only identity boundary.");
