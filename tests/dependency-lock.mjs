...
assert(verified.dependencies.pg === "8.22.0" && verified.dependencies.nodemailer === "9.0.3" && verified.dependencies.awsS3 === "3.1084.0" && verified.dependencies.sharp === "0.35.3" && verified.dependencies.stripe === "22.1.1" && verified.dependencies.svix === "2.0.0" && verified.lockSha256 === dependencyLockEvidence.expectedLockSha256, "The reviewed production dependency graph did not validate.");
...
assert(rejects(() => validateDependencyAssets(JSON.stringify({ ...manifest, dependencies: { ...manifest.dependencies, stripe: "^22.1.1" } }), lockText), "pinned exactly"), "A mutable Stripe version range passed the dependency gate.");
assert(rejects(() => validateDependencyAssets(JSON.stringify({ ...manifest, dependencies: { ...manifest.dependencies, svix: "^2.0.0" } }), lockText), "pinned exactly"), "A mutable Svix version range passed the dependency gate.");
...
assert(rejects(() => validateDependencyAssets(packageText, lockText.replace(dependencyLockEvidence.expectedStripeIntegrity, `${dependencyLockEvidence.expectedStripeIntegrity.slice(0, -2)}xx`)), "lockfile"), "A modified Stripe integrity value passed the dependency gate.");
assert(rejects(() => validateDependencyAssets(packageText, lockText.replace(dependencyLockEvidence.expectedSvixIntegrity, `${dependencyLockEvidence.expectedSvixIntegrity.slice(0, -2)}xx`)), "lockfile"), "A modified Svix integrity value passed the dependency gate.");
assert(rejects(() => validateDependencyAssets(packageText, ""), "required"), "A missing lockfile passed the dependency gate.");

console.log("Dependency lock tests passed: exact pg/Nodemailer/AWS-S3/Sharp/Stripe/Svix versions, private Node 20.9+ manifest, reviewed integrity graph and missing/mutable/unreviewed dependency rejection.");
