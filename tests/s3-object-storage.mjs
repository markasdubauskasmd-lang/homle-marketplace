...
class S3Client {
  ...
  async send(command) {
    ...
    if (command instanceof GetObjectCommand && (command.input.Key === requestFinalKey || command.input.Key === finalKey)) return { ContentType: "image/jpeg", ContentLength: outputBytes.length, Body: (async function* () { yield outputBytes; })() };
...
assert.deepEqual(await storage.readPrivateImage({ storageKey: requestFinalKey, byteSize: outputBytes.length }), outputBytes);
assert.equal(signed.length, signCount, "Authenticated delivery must not create another bearer URL.");
assert.deepEqual(await storage.readPrivateImage({ storageKey: finalKey, byteSize: outputBytes.length }), outputBytes);
await assert.rejects(() => storage.readPrivateImage({ storageKey: requestQuarantineKey, byteSize: outputBytes.length }), /outside/);
await assert.rejects(() => storage.readPrivateImage({ storageKey: requestFinalKey, byteSize: outputBytes.length + 1 }), /operation-failed/);
