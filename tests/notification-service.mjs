...
const unconditionalAppFunctions = deploymentVerification.match(/app_functions text\[\] := ARRAY\[([\s\S]*?)\];\s*worker_functions/)?.[1] || "";
assert(!unconditionalAppFunctions.includes("record_resend_email_suppression"), "The pre-upgrade deployment verifier requires the email suppression function before migration 099 can create it.");
assert(deploymentVerification.includes("IF email_suppression_migration_installed THEN") && deploymentVerification.includes("selected_function := to_regprocedure('tideway_private.record_resend_email_suppression"), "Deployment verification no longer checks the email suppression boundary after migration 099 is installed.");
...
for (const required of ["CREATE OR REPLACE FUNCTION tideway_private.queue_due_booking_payment_reminders", "scheduled_start_at<=now()+interval '5 days'", "scheduled_start_at<=now()+interval '24 hours'", "payment-window-opened", "payment-action-required", "payment-window-opened:", "payment-readiness:", "LIMIT batch_limit", "FOR UPDATE OF booking SKIP LOCKED", "ON CONFLICT(idempotency_key) DO NOTHING"]) assert(paymentScheduleMigration.includes(required), `Two-stage payment reminders omitted ${required}.`);
assert(!paymentScheduleMigration.includes("UPDATE booking_payments") && !paymentScheduleMigration.includes("INSERT INTO booking_payments") && !paymentScheduleMigration.includes("UPDATE bookings"), "Two-stage payment reminders can change money or booking state.");
for (const required of ["CREATE FUNCTION tideway_private.queue_due_booking_visit_reminders", "batch_limit NOT BETWEEN 1 AND 500", "booking.status='confirmed'", "scheduled_start_at<=now()+interval '24 hours'", "scheduled_start_at>now()+interval '2 hours'", "participant.participant_role='cleaner'", "booking.landlord_user_id,'landlord'", "booking.cleaner_user_id,'cleaner'", "payment.status='authorized'", "payment.amount_pence=booking.customer_price_pence", "payment.terms_fingerprint=booking.terms_fingerprint", "booking-reminder", "cleaner-start-journey", "booking-reminder:'||participant.participant_role", "cleaner-start-journey:", "LIMIT batch_limit", "FOR UPDATE OF booking SKIP LOCKED", "ON CONFLICT(idempotency_key) DO NOTHING", "REVOKE ALL ON FUNCTION tideway_private.queue_due_booking_visit_reminders(integer) FROM PUBLIC"]) assert(visitReminderMigration.includes(required), `Confirmed-visit reminders omitted ${required}.`);
...
const paymentWindowEmail = notificationEmail({ notificationId: "ffffffff-ffff-4fff-8fff-ffffffffffff", recipientEmail: "landlord@example.com", eventType: "payment-window-opened", bookingId, payload: {}, attemptNumber: 1 }, "https://tideway.example");
const bookingReminderEmail = notificationEmail({ notificationId: "12121212-1212-4212-8212-121212121212", recipientEmail: "landlord@example.com", eventType: "booking-reminder", bookingId, payload: {}, attemptNumber: 1 }, "https://tideway.example");
const journeyReadyEmail = notificationEmail({ notificationId: "13131313-1313-4313-8313-131313131313", recipientEmail: "cleaner@example.com", eventType: "cleaner-start-journey", bookingId, payload: {}, attemptNumber: 1 }, "https://tideway.example");
...
assert(paymentEmail.text.includes("https://tideway.example/landlord/dashboard") && paymentEmail.text.includes("exact total") && !paymentEmail.text.includes("10 Downing Street"), "Payment readiness email lost its exact dashboard action or disclosed booking details.");
assert(paymentWindowEmail.text.includes("https://tideway.example/landlord/dashboard") && paymentWindowEmail.text.includes("exact booking total") && !paymentWindowEmail.text.includes("10 Downing Street"), "Payment-window email lost its exact dashboard action or disclosed booking details.");
assert(bookingReminderEmail.text.includes(`https://tideway.example/bookings/${bookingId}`) && bookingReminderEmail.text.includes("within 24 hours") && !bookingReminderEmail.text.includes("10 Downing Street"), "Confirmed-booking reminder lost its private booking action or disclosed booking details.");
assert(journeyReadyEmail.text.includes(`https://tideway.example/bookings/${bookingId}`) && journeyReadyEmail.text.includes("payment-ready") && journeyReadyEmail.text.includes("ready to set off") && !journeyReadyEmail.text.includes("10 Downing Street"), "Cleaner journey prompt lost its payment-ready boundary, private action or safe copy.");
...
for (const requiredEvent of ["new-booking-request", "booking-confirmed", "payment-window-opened", "payment-action-required", "booking-reminder", "cleaner-start-journey", "cleaner-started-travelling", "cleaner-nearby", "cleaner-arrived", "cleaning-progress-update", "issue-reported", "cleaning-completed", "booking-completed", "review-requested", "review-submitted", "booking-message", "dispute-opened", "dispute-reviewing", "dispute-resolved"]) assert(notificationEmailEventCopy[requiredEvent], `Email copy omitted ${requiredEvent}.`);
