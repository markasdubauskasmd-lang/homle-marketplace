...
const queuedEventBlock = disputeMigration.match(/CREATE OR REPLACE FUNCTION tideway_private\.queue_email_for_in_app_notification\(\)[\s\S]*?NEW\.event_type IN \(([\s\S]*?)\) THEN/);
...
for (const requiredEvent of ["new-booking-request", "booking-confirmed", "cleaner-started-travelling", "cleaner-nearby", "cleaner-arrived", "cleaning-progress-update", "issue-reported", "cleaning-completed", "booking-completed", "review-requested", "review-submitted", "booking-message", "dispute-opened", "dispute-reviewing", "dispute-resolved"]) assert(notificationEmailEventCopy[requiredEvent], `Email copy omitted ${requiredEvent}.`);
