...
const pricingPolicy = createBookingPricingPolicy({ targetMarginBasisPoints: 2000, labourOnCostBasisPoints: 1000, paymentFeeBasisPoints: 300, paymentFeeFixedPence: 20, travelCostPence: 500, suppliesCostPence: 250, otherCostPence: 0, invitationTtlMinutes: 180 });
...
assert(result.generatedAt === now.toISOString() && result.candidates.length === 2 && result.candidates[0].cleanerId === cleaner.userId && result.candidates[0].rank === 1 && result.candidates[0].estimatedCustomerPricePence < result.candidates[1].estimatedCustomerPricePence, "Matching did not exclude unpriceable/out-of-budget Cleaners or combine price with suitability deterministically.");
...
console.log("Matching tests passed: owner-only request ranking, full eligibility/availability/coverage filters, profitable budget-aware pricing, prior-relationship scoring, safe explanations and private-factor projection.");
