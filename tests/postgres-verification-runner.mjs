...
for (const required of [
  ...
  "sessions_expiry_purge_idx", "list_my_booking_summaries(integer)", "connect_social_identity(authentication_provider,text,citext,boolean,text,text,jsonb)", "configure_automatic_dispatch(uuid,boolean,smallint)", "claim_due_automatic_dispatch(uuid,integer,integer)", "cleaning_requests_automatic_dispatch_due_idx", "submit_cleaning_request(uuid,boolean,boolean)", "withdraw_cleaning_request(uuid,text)", "create_request_photo_upload_intent", "cleaning_request_photo_uploads_expiry_idx", "cleaning_requests_reviewed_submission_guard", "function-only cleaning-request lifecycle", "authentication_identities", "bookings_require_current_payment_before_job_start", "current_booking_payment_authorized(uuid)", "require_current_payment_before_job_start()", "has_function_privilege", "has_table_privilege", "tideway_app", "tideway_worker",
  "purge_expired_sessions(integer)", "invite_cleaner_before_eligibility_hardening", "automatic-dispatch-price-cap-required", "Landlord-approved maximum total", "bypass the current booking eligibility wrapper", "worker role has direct public-table privileges", "ROLLBACK"
