...
const fakeRepository = {
  ...
  async rescheduleOwnRequest(actor, suppliedRequestId, choice) { calls.push({ kind: "reschedule", actor, suppliedRequestId, choice }); return { cleaningRequestId: suppliedRequestId, status: "searching-for-cleaner", requestedStartAt: choice.requestedStartAt, requestedEndAt: "2026-07-22T12:00:00.000Z", scopeFingerprint: canonical.scopeFingerprint, rescheduledAt: now.toISOString() }; }
};
...
assert(submitted.status === "searching-for-cleaner" && submitted.photoCount === 2 && submitted.quotedTotalPence === 7850 && submitted.quotedMinutes === 180 && submitted.pricingConfigVersion === 4 && submitted.quotedAt === now.toISOString() && calls.at(-2).kind === "submit" && calls.at(-2).choice.cleanerPreviewAuthorized === true, "Reviewed room-scan submission was not explicit, owner-bound or returned with its saved platform quote.");
...
assert(await rejects(() => service.rescheduleOwnRequest({ userId: "cleaner", roles: ["cleaner"] }, requestId, { requestedStartAt: "2026-07-22T09:00:00.000Z" }), "Landlord account") && await rejects(() => service.rescheduleOwnRequest(landlord, requestId, { requestedStartAt: "2026-07-14T09:00:00.000Z" }), "future cleaning time"), "Request rescheduling accepted the wrong role or an unsafe time.");
...
const database = {
  async withUserTransaction(actor, operation) {
    return operation({ async query(text, values) {
      ...
      if (text.startsWith("SELECT quoted_total_pence")) return { rows: [{ quoted_total_pence: 7850, quoted_minutes: 180, pricing_config_version: 4, quoted_at: now.toISOString() }] };
      ...
      if (text.startsWith("SELECT tideway_private.reschedule_open_cleaning_request")) return { rows: [{ reschedule: { cleaningRequestId: requestId, status: "searching-for-cleaner", requestedStartAt: values[1], requestedEndAt: "2026-07-22T12:00:00.000Z", scopeFingerprint: canonical.scopeFingerprint, rescheduledAt: now.toISOString() } }] };
      return { rows: [] };
    } });
  }
};
...
const repositorySubmission = await repository.submitOwnRequest(landlord, requestId, { scopeReviewed: true, cleanerPreviewAuthorized: false });
...
await repository.rescheduleOwnRequest(landlord, requestId, { requestedStartAt: "2026-07-22T09:00:00.000Z" });
assert(repositorySubmission.quotedTotalPence === 7850 && repositorySubmission.quotedMinutes === 180 && repositoryDispatch.maximumCustomerPricePence === 15000 && databaseCalls[0].text.includes("id=$1::uuid AND landlord_user_id=$2::uuid") && databaseCalls[0].values[1] === landlordId && databaseCalls[1].values[1] === landlordId && databaseCalls[1].values.length === 17 && databaseCalls[2].text.includes("unnest($2::text[], $3::text[], $4::integer[])") && databaseCalls[3].text.includes("cleaning_request_status_history") && databaseCalls[3].values[2] === landlordId && databaseCalls[4].text.includes("request.landlord_user_id=$1::uuid") && databaseCalls[5].text.includes("submit_cleaning_request($1::uuid,$2::boolean,$3::boolean)") && databaseCalls[6].text.includes("quoted_total_pence") && databaseCalls[6].values[1] === landlordId && databaseCalls[7].text.includes("SELECT budget_pence") && databaseCalls[7].values[1] === landlordId && databaseCalls[8].text.includes("configure_automatic_dispatch($1::uuid,$2::boolean,$3::smallint)") && databaseCalls[9].text.includes("withdraw_cleaning_request($1::uuid,$2::text)") && databaseCalls[9].values[1] === "no-longer-needed" && databaseCalls[10].text.includes("reschedule_open_cleaning_request($1::uuid,$2::timestamptz)"), "Cleaning-request repository did not return the owner-bound saved quote, verify the approved maximum, parameterize writes or preserve the function-only request lifecycle.");
...
const rescheduleMigration = await readFile(new URL("../db/migrations/100_open_request_reschedule.sql", import.meta.url), "utf8");
