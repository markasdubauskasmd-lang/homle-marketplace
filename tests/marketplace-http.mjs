...
const cleaningRequestService = {
  ...
  async rescheduleOwnRequest(actor, cleaningRequestId, input) { calls.push({ kind: "request-reschedule", actor, cleaningRequestId, input }); return { cleaningRequestId, status: "searching-for-cleaner", requestedStartAt: input.requestedStartAt, requestedEndAt: "2026-07-22T12:00:00.000Z", scopeFingerprint: "a".repeat(64), rescheduledAt: "2026-07-15T15:00:00.000Z" }; }
};
...
const cleanerOnboardingDocumentService = {
  async listOwnDocuments(actor, section) { calls.push({ kind: "onboarding-document-list", actor, section }); return []; },
  async saveOwnDocument(actor, section, documentType, input) { calls.push({ kind: "onboarding-document-save", actor, section, documentType, input }); return { id: "eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee", section, documentType, filename: input.filename, mimeType: input.mimeType, sizeBytes: input.bytes.length, checksumSha256: "a".repeat(64), status: "uploaded", expiresOn: null, createdAt: "2026-08-05T10:00:00.000Z", updatedAt: "2026-08-05T10:00:00.000Z" }; },
  async getOwnDocument(actor, section, documentType) { calls.push({ kind: "onboarding-document-get", actor, section, documentType }); return { id: "eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee", section, documentType, filename: "passport.pdf", mimeType: "application/pdf", sizeBytes: storedOnboardingDocument.length, checksumSha256: "a".repeat(64), status: "uploaded", expiresOn: null, createdAt: "2026-08-05T10:00:00.000Z", updatedAt: "2026-08-05T10:00:00.000Z", bytes: storedOnboardingDocument }; },
  async deleteOwnDocument(actor, section, documentType) { calls.push({ kind: "onboarding-document-delete", actor, section, documentType }); return { id: "eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee", section, documentType, filename: "passport.pdf", mimeType: "application/pdf", sizeBytes: storedOnboardingDocument.length, checksumSha256: "a".repeat(64), status: "deleted", expiresOn: null, createdAt: "2026-08-05T10:00:00.000Z", updatedAt: "2026-08-05T10:00:00.000Z" }; }
};
...
const rateLimiter = {
  async consume(input) {
    ...
    if (!postgresRateLimitScopes.includes(input.scope)) throw new TypeError(`Rate-limit scope has no reviewed database policy: ${input.scope}`);
...
const landlordDocumentUpload = await dispatch(router, "PUT", "/api/marketplace/cleaner/onboarding/documents/identity/passportPhoto", { headers: { ...authHeaders, "content-type": "application/pdf", "x-document-filename": encodeURIComponent("passport.pdf") }, body: storedOnboardingDocument });
const onboardingDocumentSaved = await dispatch(router, "PUT", "/api/marketplace/cleaner/onboarding/documents/identity/passportPhoto", { headers: documentHeaders, body: storedOnboardingDocument });
const onboardingDocumentRead = await dispatch(router, "GET", "/api/marketplace/cleaner/onboarding/documents/identity/passportPhoto", { headers: { cookie: cleanerAuthHeaders.cookie } });
const onboardingDocumentDeleted = await dispatch(router, "DELETE", "/api/marketplace/cleaner/onboarding/documents/identity/passportPhoto", { headers: cleanerAuthHeaders });
assert(onboardingDocuments.response.statusCode === 200 && missingDocumentCsrf.response.statusCode === 403 && landlordDocumentUpload.response.statusCode === 403 && onboardingDocumentSaved.response.statusCode === 200 && onboardingDocumentSaved.body.document.filename === "passport.pdf" && Buffer.compare(calls.findLast((call) => call.kind === "onboarding-document-save").input.bytes, storedOnboardingDocument) === 0 && onboardingDocumentRead.response.statusCode === 200 && onboardingDocumentRead.response.headers["Content-Type"] === "application/pdf" && onboardingDocumentRead.response.headers["Cache-Control"] === "private, no-store" && Buffer.compare(onboardingDocumentRead.body, storedOnboardingDocument) === 0 && onboardingDocumentDeleted.response.statusCode === 200 && onboardingDocumentDeleted.body.document.status === "deleted", "Cleaner onboarding document routes lost owner isolation, CSRF protection, exact binary upload, private download or deletion.");
...
const missingRescheduleCsrf = await dispatch(router, "POST", "/api/marketplace/cleaning-requests/66666666-6666-4666-8666-666666666666/reschedule", { headers: { cookie: authHeaders.cookie, origin: authHeaders.origin, "content-type": authHeaders["content-type"] }, body: { requestedStartAt: "2026-07-22T09:00:00.000Z" } });
const requestRescheduled = await dispatch(router, "POST", "/api/marketplace/cleaning-requests/66666666-6666-4666-8666-666666666666/reschedule", { headers: authHeaders, body: { requestedStartAt: "2026-07-22T09:00:00.000Z" } });
assert(missingRescheduleCsrf.response.statusCode === 403 && requestRescheduled.response.statusCode === 200 && requestRescheduled.body.reschedule.status === "searching-for-cleaner" && calls.at(-1).kind === "request-reschedule" && calls.at(-1).actor.userId === sessions.landlord.user_id && calls.at(-1).input.requestedStartAt === "2026-07-22T09:00:00.000Z", "Open-request rescheduling lost Landlord role, CSRF, route identity or requested-time binding.");
...
const landlordNotificationRealtimeStream = await dispatch(router, "POST", "/api/marketplace/notifications/events", { headers: authHeaders, body: {} });
const cleanerNotificationRealtimePost = await dispatch(router, "POST", "/api/marketplace/notifications/events", { headers: cleanerAuthHeaders, body: {} });
assert(missingNotificationRealtimeOrigin.response.statusCode === 403 && missingNotificationRealtimeOrigin.body.code === "origin-rejected" && notificationRealtimeStream.response.statusCode === 200 && landlordNotificationRealtimeStream.response.statusCode === 200 && cleanerNotificationRealtimePost.response.statusCode === 403 && cleanerNotificationRealtimePost.body.code === "role-rejected" && calls.at(-1).kind === "notification-realtime-open" && calls.at(-1).actor.userId === sessions.landlord.user_id, "The account notification stream lost authentication, exact-origin, Landlord CSRF, role isolation or current-account binding.");
...
assert(geocodedRuntime.geocodingReady === true, "The reviewed postcode geocoder was not exposed as ready.");
