...
try {
  ...
  assert(authProviders.ok && authProvidersBody.providers?.roles?.join(",") === "cleaner,landlord" && typeof authProvidersBody.providers.google === "boolean" && typeof authProvidersBody.providers.emailPassword === "boolean" && !serialisedAuthProviders.includes("CLIENT_SECRET") && !serialisedAuthProviders.includes("SESSION_SECRET") && !serialisedAuthProviders.includes("DATABASE_URL"), "Public authentication provider discovery was missing roles, misreported capabilities or exposed secret configuration.");
