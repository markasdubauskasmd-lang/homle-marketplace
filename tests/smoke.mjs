...
try {
  ...
  assert(loginPage.ok && signupPage.ok && loginPageText.includes("Account access is not open yet") && signupPageText.includes("Account access is not open yet") && loginPageText.includes('href="/request">Request a clean</a>') && loginPageText.includes('href="/join">Join as a cleaner</a>') && !loginPageText.includes("Sign in with Google") && !signupPageText.includes('action="/api/account/') && authEntryAsset.ok && authEntryText.includes("Account access is safely unavailable") && !authEntryText.includes("/api/account/login"), "Account entry pages displayed a non-working login action or failed to retain the working pilot routes.");
