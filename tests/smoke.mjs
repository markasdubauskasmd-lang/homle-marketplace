...
try {
  ...
  assert(publicFormAsset.ok && publicFormAssetText.includes("focusedEntryRoutes") && publicFormAssetText.includes('document.body.classList.add("entry-route"') && publicFormAssetText.includes("history.replaceState") && publicFormAssetText.includes("enhanceGuidedForm") && publicFormAssetText.includes("step.dataset.guidedStep") && publicFormAssetText.includes("validateCurrentStep") && publicFormAssetText.includes('scrollIntoView({ behavior: "smooth", block: "start" })') && publicFormAssetText.includes("Choose at least one type of cleaning work before continuing.") && publicFormAssetText.includes("data-cleaner-status-link") && publicFormAssetText.includes("cleanerStatusToken") && publicFormAssetText.includes("guidedForm?.complete()") && publicFormAssetText.includes('"Idempotency-Key": pending.key') && publicFormAssetText.includes("pendingSubmissions.delete(form)"), "Focused entry routes, progressive customer/cleaner journeys, visible mobile step positioning, cleaner-tracker handoff, per-stage service validation or safe retry-key handling failed.");
