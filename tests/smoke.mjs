...
import { briefReadiness, briefScopeConfirmationIsCurrent, briefScopeFingerprint, briefSourceFingerprint, maxBriefPhotos, maxBriefVideos } from "../public/brief-readiness.js";
...
try {
  ...
  assert(home.ok && homeText.includes("Book a clean with every room clearly scoped") && homeText.includes('data-guided-kind="customer"') && homeText.includes('data-guided-kind="cleaner"') && homeText.includes("data-cleaner-status-link") && homeText.includes("Application received. Opening your private tracker") && guidedStepCount === 6 && homeText.includes("What needs cleaning?") && homeText.includes("When and how can the cleaner enter?") && homeText.includes("Where should we send the next step?") && homeText.includes("How can Tideway reach you?") && homeText.includes("What cleaning work suits you?") && homeText.includes("When could you first work?") && homeText.includes('name="firstAvailableDate"') && homeText.includes('name="firstAvailableStartTime"') && homeText.includes('name="firstAvailableEndTime"') && homeText.includes('href="/request">Book a clean</a>') && homeText.includes('href="/join">Join as a cleaner</a>') && !homeText.includes('href="/brief">Scan my rooms') && homeText.includes("postcode districts or areas") && homeText.includes("SW1A, SW4, or broader areas SW, SE"), "Uber-simple focused customer and cleaner entry routes, three-stage journeys, continuous private cleaner-tracker handoff, actionable first availability, guarded scan entry or structured cleaner travel-area guidance failed.");
  ...
  assert(sharedStyles.ok && sharedStylesText.includes("[hidden] { display: none !important; }") && sharedStylesText.includes(".guided-form-step { scroll-margin-top: 92px;") && sharedStylesText.includes(".entry-route main > section:not(.forms-section)") && sharedStylesText.includes(".entry-route-request #cleaner-application") && sharedStylesText.includes(".entry-route-join #request-cleaning"), "Shared styling allowed hidden private actions or form stages to remain visible, failed to keep guided-step headings below the sticky mobile header, or did not isolate the focused customer and cleaner entry screens.");
  ...
  assert(privacy.ok && privacyText.includes("Privacy notice") && privacyText.includes("one exact first-available date") && privacyText.includes("planning evidence only") && privacyText.includes("temporarily holds a client network address in memory") && privacyText.includes("private cleaner tracker") && privacyText.includes("screening-note, decision-note or authorisation-token data") && privacyText.includes("never used for matching until Tideway records a separate confirmation"), "Privacy page failed or omitted the initial availability, local anti-abuse, pending-availability or private cleaner-tracker disclosure.");

  const terms = await fetch(`${base}/terms`);
  const termsText = await terms.text();
  assert(terms.ok && termsText.includes("Pilot terms") && termsText.includes("private customer and cleaner trackers") && termsText.includes("does not guarantee work") && termsText.includes("first-available window") && termsText.includes("separately records that the exact window was re-confirmed") && termsText.includes("planning preference only") && termsText.includes("No recurring schedule") && termsText.includes("UK local time (Europe/London)"), "Terms page failed, implied that tracker or first availability guarantees work, or omitted the one-visit frequency or UK booking-clock boundary.");
  ...
  assert(adminScript.ok && adminScriptText.includes("function addLocalAuditPreview") && adminScriptText.includes("Open local customer preview") && adminScriptText.includes("Open local cleaner preview") && adminScriptText.includes("const privateUrl = draft.privateUrl || \"\"") && adminScriptText.includes("/api/admin/booking-drafts") && adminScriptText.includes("Load copy-only booking handoffs") && adminScriptText.includes("booking.publicSiteUrl") && !adminScriptText.includes("location.origin") && !adminScriptText.includes("Copy private link") && !adminScriptText.includes("Copy cleaner link") && !adminScriptText.includes("Copy verified booking-pack link"), "The control desk still exposed bare or local-origin recipient links outside the verified dispatch or booking-handoff packs.");
  ...
  assert(briefScript.ok && briefScriptText.includes('cameraInput.addEventListener("change"') && briefScriptText.includes("addSelectedVisuals(selected)") && briefScriptText.includes('saveButton.disabled = submitting || submissionComplete') && briefScriptText.includes("privateRequestToken") && briefScriptText.includes("history.replaceState") && briefScriptText.includes('"X-Request-Token": privateRequestToken') && briefScriptText.includes("new AbortController()") && briefScriptText.includes("took too long to prepare") && briefScriptText.includes('"Idempotency-Key": pendingSubmission.key') && briefScriptText.includes('location.assign(`/brief-complete'), "Direct camera handling, token-authorised tracker handoff, room-scan button recovery, idempotent retry handling, bounded photo/upload handling or dedicated completion handoff was missing.");
  ...
  assert(requestStatusPage.ok && requestStatusPageText.includes("Private request tracker") && requestStatusPageText.includes("data-frequency"), "Private customer request tracker page failed or omitted the requested frequency.");
  ...
  assert(requestStatusAsset.ok && requestStatusAssetText.includes('/brief?reference=${encodeURIComponent(result.request.reference)}#${token}'), "The private customer tracker did not carry its fragment token into the required room scan.");
  const cleanerStatusPage = await fetch(`${base}/cleaner-status`);
  const cleanerStatusPageText = await cleanerStatusPage.text();
  assert(cleanerStatusPage.ok && cleanerStatusPageText.includes("Private cleaner tracker") && cleanerStatusPageText.includes("First window supplied") && cleanerStatusPageText.includes("When are you available?") && cleanerStatusPageText.includes("It stays pending until Tideway confirms it") && cleanerStatusPageText.includes("This tracker does not approve, assign or promise work."), "Private cleaner application tracker page failed, omitted the first/pending exact-time action or implied work approval.");
  ...
  assert(cleanerStatusAsset.ok && cleanerStatusAssetText.includes("X-Cleaner-Status-Token") && cleanerStatusAssetText.includes("history.replaceState") && cleanerStatusAssetText.includes("firstAvailability") && cleanerStatusAssetText.includes("confirmedAvailabilityWindows") && cleanerStatusAssetText.includes("/api/cleaner-availability-requests") && cleanerStatusAssetText.includes("Idempotency-Key"), "Cleaner tracker did not keep its token out of request URLs, render captured readiness safely or submit retry-safe pending availability.");
  const quotePage = await fetch(`${base}/quote`);
  const quotePageText = await quotePage.text();
  assert(quotePage.ok && quotePageText.includes("Private customer review") && quotePageText.includes("This is a replacement quote") && quotePageText.includes("A fresh decision is required.") && quotePageText.includes("data-frequency") && quotePageText.includes("One-visit rule") && quotePageText.includes("UK local time"), "Private customer quote, frequency scope, UK booking clock or replacement-review page failed.");
  const quoteAsset = await fetch(`${base}/quote.js?v=smoke-test`);
  const quoteAssetText = await quoteAsset.text();
  assert(quoteAsset.ok && quoteAssetText.includes("previousCustomerAccepted") && quoteAssetText.includes("[data-frequency]") && quoteAssetText.includes('timeZone: "Europe/London"'), "Customer replacement-quote explanation, requested-frequency rendering or UK-local deadline display failed.");
  ...
  assert(opportunityAsset.ok && opportunityAssetText.includes("[data-frequency]") && opportunityAssetText.includes('timeZone: "Europe/London"'), "Cleaner opportunity did not render the requested frequency or UK-local deadline.");
  ...
  assert(initialRequestStatus.ok && initialRequestStatusBody.current.stage === "room-scan" && initialRequestStatusBody.links.roomScanRequired === true && initialRequestStatusBody.request.reference === requestBody.reference && initialRequestStatusBody.request.frequency === "Weekly", "New request tracker did not open at the required room-scan stage with the requested frequency.");
  ...
  assert(initialCleanerStatus.ok && initialCleanerStatusBody.current.stage === "application-review" && initialCleanerStatusBody.application.reference === cleanerBody.reference && initialCleanerStatusBody.application.firstAvailability?.availableDate === "2026-07-20" && initialCleanerStatusBody.application.firstAvailability?.startTime === "08:00" && initialCleanerStatusBody.application.firstAvailability?.endTime === "15:00" && initialCleanerStatusBody.application.firstAvailability?.status === "unconfirmed" && initialCleanerStatusBody.readiness.firstAvailabilityCaptured === true && initialCleanerStatusBody.readiness.readyForOpportunities === false && initialCleanerStatusBody.steps.find((step) => step.key === "application")?.state === "complete" && initialCleanerStatusBody.steps.find((step) => step.key === "availability")?.detail.includes("not confirmed for matching"), "New cleaner tracker did not retain the unconfirmed first window or open at received-but-not-screened status.");
  ...
  const earlyAvailability = await fetch(`${base}/api/cleaner-availability-requests`, { method: "POST", headers: { "content-type": "application/json", "x-cleaner-status-token": cleanerBody.cleanerStatusToken, "idempotency-key": "44444444-4444-4444-8444-444444444444" }, body: JSON.stringify({ availableDate: "2026-07-20", startTime: "08:00", endTime: "15:00" }) });
  assert(earlyAvailability.status === 422, "An unapproved cleaner submitted exact availability through the private tracker.");
  ...
  assert(adminBody.records.find((record) => record.id === cleanerBody.reference)?.firstAvailableDate === "2026-07-20" && adminBody.records.find((record) => record.id === cleanerBody.reference)?.firstAvailableStartTime === "08:00" && adminBody.records.find((record) => record.id === cleanerBody.reference)?.firstAvailableEndTime === "15:00" && adminBody.records.find((record) => record.id === cleanerBody.reference)?.cleanerAvailability?.length === 0, "The founder view lost the application-supplied first window or treated it as confirmed availability.");
  ...
  assert(approvedCleanerStatus.ok && approvedCleanerStatusBody.current.stage === "availability" && approvedCleanerStatusBody.readiness.approvalRecorded === true && approvedCleanerStatusBody.readiness.confirmedAvailabilityWindows === 0 && approvedCleanerStatusBody.readiness.pendingAvailabilityWindows === 0 && approvedCleanerStatusBody.links.availabilitySubmissionAllowed === true && approvedCleanerStatusBody.steps.find((step) => step.key === "availability")?.state === "current", "Approved cleaner tracker skipped the exact-availability requirement, hid its safe submission action or implied work readiness early.");
  const availabilityRequestKey = "55555555-5555-4555-8555-555555555555";
  ...
  const availabilityRequest = await fetch(`${base}/api/cleaner-availability-requests`, { method: "POST", headers: { "content-type": "application/json", "x-cleaner-status-token": cleanerBody.cleanerStatusToken, "idempotency-key": availabilityRequestKey }, body: JSON.stringify(availabilityRequestInput) });
  const availabilityRequestBody = await availabilityRequest.json();
  const availabilityRequestReplay = await fetch(`${base}/api/cleaner-availability-requests`, { method: "POST", headers: { "content-type": "application/json", "x-cleaner-status-token": cleanerBody.cleanerStatusToken, "idempotency-key": availabilityRequestKey }, body: JSON.stringify(availabilityRequestInput) });
  const availabilityRequestReplayBody = await availabilityRequestReplay.json();
  assert(availabilityRequest.status === 201 && availabilityRequestBody.request.reference.startsWith("AVR-") && availabilityRequestReplay.status === 200 && availabilityRequestReplayBody.replayed === true && availabilityRequestReplayBody.request.reference === availabilityRequestBody.request.reference, "Cleaner availability submission was not safely idempotent.");
  const changedAvailabilityReplay = await fetch(`${base}/api/cleaner-availability-requests`, { method: "POST", headers: { "content-type": "application/json", "x-cleaner-status-token": cleanerBody.cleanerStatusToken, "idempotency-key": availabilityRequestKey }, body: JSON.stringify({ ...availabilityRequestInput, endTime: "14:00" }) });
  assert(changedAvailabilityReplay.status === 409, "An availability retry key was reused for different exact times.");
  ...
  const confirmedAvailabilityRequest = await fetch(`${base}/api/admin/cleaner-availability-requests`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: availabilityRequestBody.request.reference, decision: "confirmed", note: "Test-only exact window verified by Tideway." }) });
  ...
  const reviewWithoutEvidence = await fetch(`${base}/api/admin/job-briefs/status`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ briefId: briefBody.reference, status: "reviewed", note: "Scope estimate entered without opening every visual.", scopeEstimateHours: 3.5, scopeConfidence: "high", scopeSignalConfirmations: ["oven-interior"] }) });
  ...
  const reviewedBrief = await fetch(`${base}/api/admin/job-briefs/status`, {
    method: "PATCH",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ briefId: briefBody.reference, status: "reviewed", note: "Checklist and private photo support a three-and-a-half-hour scope floor, including the oven interior.", scopeEstimateHours: 3.5, scopeConfidence: "high", scopeSignalConfirmations: ["oven-interior"], visualsReviewed: true, checklistReviewed: true })
  });
  const reviewedBriefBody = await reviewedBrief.json();
  assert(reviewedBrief.ok && reviewedBriefBody.status === "reviewed" && reviewedBriefBody.scopeEstimateHours === 3.5 && reviewedBriefBody.scopeConfidence === "high" && reviewedBriefBody.scopeSignals?.[0]?.code === "oven-interior" && reviewedBriefBody.scopeSignalConfirmations?.[0] === "oven-interior" && reviewedBriefBody.visualsReviewed === true && reviewedBriefBody.checklistReviewed === true && reviewedBriefBody.reviewEvidenceConfirmed === true, "Structured human scan approval, founder review evidence or price-sensitive scope confirmation was not recorded.");
  ...
  const reviewedTravelGateBrief = await fetch(`${base}/api/admin/job-briefs/status`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ briefId: travelGateBriefBody.reference, status: "reviewed", note: "Test-only two-hour travel-coverage scope estimate.", scopeEstimateHours: 2, scopeConfidence: "medium", visualsReviewed: true, checklistReviewed: true }) });
  ...
  assert(sentQuote.ok && sentQuoteBody.quote.frequency === "Weekly" && sentQuoteBody.quote.decisionAllowed === true && Date.parse(sentQuoteBody.quote.offerExpiresAt) >= sentAtLowerBound + 24 * 60 * 60 * 1000 - 2000 && sentQuoteBody.quote.expired === false && sentQuoteBody.quote.confirmedExtras?.[0]?.label === "Inside oven cleaning", "Sent customer quote did not expose its frozen frequency, response deadline and confirmed extra.");
  ...
  const reviewedOverlapScan = await fetch(`${base}/api/admin/job-briefs/status`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ briefId: overlapScanBody.reference, status: "reviewed", note: "Test-only two-hour scan estimate.", scopeEstimateHours: 2, scopeConfidence: "medium", visualsReviewed: true, checklistReviewed: true }) });
  ...
  const bookingInput = { proposalId: proposalBody.proposal.id, serviceAddress: "10 Clean Street, Westminster, London", servicePostcode: "SW1A 1AA", accessContactName: "Site Manager", accessContactPhone: "07123456781", accessInstructions: "Meet the site manager at reception. No access codes stored.", parkingNotes: "Paid parking nearby.", productsAndEquipment: "Cleaner brings standard products and equipment; customer provides site-specific consumables.", emergencyInstructions: "Stop work and call Tideway support if the site is unsafe or materially different.", paymentEvidenceReference: "PAY-TEST-001", paymentEvidenceAmount: 120, paymentEvidenceVerifiedAt: new Date().toISOString(), addressAndAccessConfirmed: true, finalChecklistConfirmed: true, paymentAuthorisationConfirmed: true, emergencyInstructionsConfirmed: true, internalNote: "Test confirmation only" };
  ...
  const prematureOutcome = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 120, cleanerPaid: 72, otherCosts: 10, refundAmount: 0, customerReceiptReference: "RECEIPT-EARLY-001", cleanerPayoutReference: "PAYOUT-EARLY-001", settlementVerifiedAt: new Date().toISOString(), settlementEvidenceNote: "Test-only early settlement evidence must remain blocked.", settlementConfirmed: true }) });
  ...
  const missingSettlementEvidence = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 120, cleanerPaid: 72 }) });
  ...
  const mismatchedCustomerReceipt = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 119.99, cleanerPaid: 72, ...settlementEvidence }) });
  ...
  const underpaidCleaner = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 120, cleanerPaid: 71.99, ...settlementEvidence }) });
  assert(underpaidCleaner.status === 422, "A cleaner payout below the agreed booking pay was accepted.");
  const sameSettlementReferences = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 120, cleanerPaid: 72, ...settlementEvidence, cleanerPayoutReference: settlementEvidence.customerReceiptReference }) });
  ...
  const staleSettlementEvidence = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 120, cleanerPaid: 72, ...settlementEvidence, settlementVerifiedAt: "2020-01-01T00:00:00.000Z" }) });
  assert(staleSettlementEvidence.status === 422, "Settlement evidence predating customer completion was accepted.");
  const futureSettlementEvidence = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 120, cleanerPaid: 72, ...settlementEvidence, settlementVerifiedAt: new Date(Date.now() + 60_000).toISOString() }) });
  assert(futureSettlementEvidence.status === 422, "Future-dated settlement verification was accepted.");
  const unconfirmedSettlementEvidence = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 120, cleanerPaid: 72, ...settlementEvidence, settlementConfirmed: false }) });
  ...
  const negativeActualCost = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 120, cleanerPaid: 72, paymentFees: -1, ...settlementEvidence }) });
  assert(negativeActualCost.status === 422, "A negative actual payment fee was accepted.");
  const excessiveActualHours = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 101, customerCollected: 120, cleanerPaid: 72, ...settlementEvidence }) });
  assert(excessiveActualHours.status === 422, "Completed-job evidence exceeded the supported actual-hours audit limit.");
  const excessiveActualCost = await fetch(`${base}/api/admin/job-outcomes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, actualHours: 4, customerCollected: 120, cleanerPaid: 72, otherCosts: 1000001, ...settlementEvidence }) });
  assert(excessiveActualCost.status === 422, "Completed-job evidence exceeded the supported financial audit limit.");
  const completedOutcomeInput = { bookingId: confirmedBookingBody.booking.id, actualHours: 4.5, customerCollected: 120, cleanerPaid: 72, paymentFees: 2, travelCosts: 1, suppliesCosts: 1, otherCosts: 6, refundAmount: 5, internalNote: "Test completion only", ...settlementEvidence };
  ...
  assert(outcomeAdjustment.status === 201 && outcomeAdjustmentBody.outcome.adjusted === true && outcomeAdjustmentBody.outcome.original.contribution === 33 && outcomeAdjustmentBody.outcome.actualHours === 6 && outcomeAdjustmentBody.outcome.refundAmount === 25 && outcomeAdjustmentBody.outcome.contribution === -12 && outcomeAdjustmentBody.outcome.profitable === false && outcomeAdjustmentBody.outcome.metTargetMargin === false, "Later re-clean/refund evidence did not revise the append-only completed-job economics correctly.");
  const cumulativeLimitAdjustment = await fetch(`${base}/api/admin/job-outcome-adjustments`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ bookingId: confirmedBookingBody.booking.id, reasonType: "late-provider-cost", sourceReference: "CASE-CUMULATIVE-LIMIT", additionalOtherCosts: 999990, externalActionConfirmed: true, internalNote: "Test-only cumulative financial limit must reject this adjustment before storage." }) });
  ...
  assert(healthyIntegrity.ok && healthyIntegrityBody.audit?.healthy === true && healthyIntegrityBody.audit.issueCount === 0 && healthyIntegrityBody.audit.files.length === 19, "The authenticated private-data integrity desk did not verify the complete healthy record set.");

  await appendFile(path.join(testDataDir, "job-outcome-adjustments.ndjson"), `${JSON.stringify({ id: "ADJ-FINANCIAL-CORRUPT", bookingId: confirmedBookingBody.booking.id, sourceReference: "CORRUPT-FINANCIAL-TEST", additionalHours: 0, additionalCustomerCollected: 0, additionalCleanerPaid: 0, additionalPaymentFees: 0, additionalTravelCosts: 0, additionalSuppliesCosts: 0, additionalOtherCosts: "1e309", additionalRefundAmount: 0, createdAt: new Date().toISOString() })}\n`, "utf8");
  ...
  assert(degradedIntegrity.ok && degradedIntegrityBody.audit?.healthy === false && degradedIntegrityBody.audit.issueCount >= 4 && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "malformed-record" && issue.file === "job-events.ndjson") && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "orphaned-reference" && issue.reference === "BKG-NOT-REAL") && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "invalid-financial-record" && issue.file === "job-outcome-adjustments.ndjson" && issue.reference === "ADJ-FINANCIAL-CORRUPT") && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "invalid-financial-config" && issue.file === "business-config.json"), "The non-destructive integrity audit missed malformed JSON, a cross-record orphan, semantic financial corruption or invalid launch economics.");
