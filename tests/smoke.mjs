...
try {
  ...
  const unprovenParticipantRehearsal = await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, participantRehearsalPassed: false, participantRehearsalEvidenceNote: "", participantRehearsalVerifiedDate: "" }) });
  ...
  const vagueParticipantRehearsal = await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, participantRehearsalEvidenceNote: "It passed." }) });
  assert(vagueParticipantRehearsal.status === 422, "A participant rehearsal was marked passed with vague evidence.");
  const futureParticipantRehearsal = await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, participantRehearsalVerifiedDate: "2026-12-31" }) });
