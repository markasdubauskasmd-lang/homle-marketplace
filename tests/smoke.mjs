...
try {
  ...
  const validRequestInput = { contactName: "Test Customer", email: "customer@example.com", phone: "07123456789", postcode: "SW1A 1AA", propertyType: "Flat or house", service: "Regular home clean", siteSize: "2 bedrooms and 1 bathroom", hazards: "None known", frequency: "Weekly", preferredDate: "2026-07-20", preferredTimeWindow: "Morning (8am–12pm)", consent: true };
  ...
  assert(adminBody.records.find((record) => record.id === requestBody.reference)?.customerType === "Cleaning customer" && adminBody.records.find((record) => record.id === requestBody.reference)?.accessNotes === "Confirm privately after booking", "The shorter public request did not receive safe server-owned customer/access defaults.");
