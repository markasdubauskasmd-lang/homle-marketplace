...
try {
  ...
  assert(homeText.includes("Private application preview") && homeText.includes("Unverified · not published") && homeText.includes("Contact details and your home postcode are never shown here") && homeText.includes("0 of 8 application details ready") && homeText.includes('data-cleaner-preview-progress'), "Cleaner onboarding omitted the truthful private live-profile preview or its applicant-only privacy boundary.");
  ...
  const cleanerPreviewAsset = await fetch(`${base}/cleaner-application-preview.js?v=smoke-test`);
  const cleanerPreviewAssetText = await cleanerPreviewAsset.text();
  assert(cleanerPreviewAsset.ok && cleanerPreviewAssetText.includes("export function cleanerApplicationPreview") && cleanerPreviewAssetText.includes("applicant supplied, unconfirmed") && cleanerPreviewAssetText.includes("parseCleanerTravelAreas") && !cleanerPreviewAssetText.includes("input.email") && !cleanerPreviewAssetText.includes("input.phone") && !cleanerPreviewAssetText.includes("input.postcode"), "Cleaner application preview projection was unavailable, accepted vague coverage or included private contact/home-postcode fields.");
  assert(publicFormAssetText.includes("readCleanerPreviewInput") && publicFormAssetText.includes("cleanerApplicationPreview(readCleanerPreviewInput(form))") && publicFormAssetText.includes("serviceList.replaceChildren") && !publicFormAssetText.includes("preview.innerHTML"), "The live Cleaner preview was not bound through safe text-only rendering.");
