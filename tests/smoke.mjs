...
try {
  ...
  assert(adminAsset.ok && adminAsset.headers.get("cache-control") === "no-cache" && adminAssetText.includes("Later refunds, re-cleans and costs") && adminAssetText.includes("/api/admin/job-outcome-adjustments") && adminAssetText.includes("paymentEvidenceReference") && adminAssetText.includes("Externally verified amount"), "Updated control-desk assets, payment evidence or post-completion adjustment workflow could remain stale or missing.");
  ...
  assert(refreshedBody.records.find((record) => record.id === requestBody.reference)?.outcome?.contribution === -12 && refreshedBody.records.find((record) => record.id === requestBody.reference)?.outcome?.original?.contribution === 33 && refreshedBody.records.find((record) => record.id === requestBody.reference)?.outcome?.adjustments?.[0]?.sourceReference === "CASE-QUALITY-001", "Append-only adjusted job outcome was not attached to the request.");
