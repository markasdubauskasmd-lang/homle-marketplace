...
try {
  ...
  assert(missingPublicOrigin.ok && missingPublicOriginBody.readiness.ready === false && missingPublicOriginBody.readiness.checks.contact === false && missingPublicOriginBody.readiness.missing.contact.includes("verified HTTPS public website origin"), "Missing public website origin passed launch readiness.");
  ...
  assert(bookingAudit.ok && bookingAuditBody.automatedReady === true && bookingAuditBody.checks.customerScopeConfirmed === true && bookingAuditBody.checks.cleanerTravelCovered === true && bookingAuditBody.customerDecision?.status === "accepted" && Object.values(bookingAuditBody.checks).every(Boolean), "Two-sided accepted proposal did not retain customer scope confirmation, accepted customer decision and cleaner travel coverage or pass the automated booking audit.");
  ...
  assert(confirmedBooking.status === 201 && confirmedBookingBody.booking.id.startsWith("BKG-") && confirmedBookingBody.booking.paymentEvidence.reference === "PAY-TEST-001" && confirmedBookingBody.booking.paymentEvidence.amount === 120 && confirmedBookingBody.booking.paymentEvidence.status === "authorisation-confirmed" && confirmedBookingBody.booking.paymentEvidence.providerName === "TestPay" && /^[A-Za-z0-9_-]{32}$/.test(confirmedBookingBody.booking.customerViewToken) && /^[A-Za-z0-9_-]{32}$/.test(confirmedBookingBody.booking.cleanerViewToken), "Fully confirmed booking, external payment evidence or its private view tokens were not recorded.");
  ...
  assert(refreshedBody.records.find((record) => record.id === requestBody.reference)?.booking?.details?.serviceAddress === "10 Clean Street, Westminster, London" && refreshedBody.records.find((record) => record.id === requestBody.reference)?.booking?.cleanerViewToken === confirmedBookingBody.booking.cleanerViewToken && refreshedBody.records.find((record) => record.id === requestBody.reference)?.booking?.paymentEvidence?.reference === "PAY-TEST-001", "Structured booking pack or private payment evidence was not retained in the control desk.");
