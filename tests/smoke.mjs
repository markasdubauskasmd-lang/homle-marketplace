...
// The live-flow fixtures book visits in the near future relative to the test run
// so calendar rollover can never expire an offer mid-suite (offer expiry is
// capped at the visit start). Ordering between the days is preserved.
const smokeDay = (offset) => new Date(Date.now() + offset * 86_400_000).toISOString().slice(0, 10);
const visitDay = smokeDay(10);
const visitDayPlus1 = smokeDay(11);
const visitDayPlus2 = smokeDay(12);
const visitDayPlus3 = smokeDay(13);
const visitDayPlus4 = smokeDay(14);
const visitDayPlus5 = smokeDay(15);
...
try {
  ...
  assert(lanPreviewHealth.ok && lanPreviewHome.ok && (await lanPreviewHome.text()).includes("data-ci-root"), "Optional same-process Wi-Fi preview listener did not serve the Homle site.");
  ...
  const optionalPhotoNoteReadiness = briefReadiness({ requestId: "REQ-1234ABCD", email: "customer@example.com", transcript: "Clean the kitchen.", tasks: ["Kitchen: Clean the worktops"], photos: [{ area: "Kitchen", note: "" }], checklistCurrent: true, scopeCompleteConfirmed: true, consent: true });
  assert(optionalPhotoNoteReadiness.ready === true && optionalPhotoNoteReadiness.checks.photoDetails === true, "A room-labelled visual still required a duplicate manual photo description.");
  ...
  const cleanerPersonalDetailsPage = await fetch(`${base}/cleaner/personal-details`);
  const cleanerBusinessDetailsPage = await fetch(`${base}/cleaner/business-details`);
  const cleanerIdentityVerificationPage = await fetch(`${base}/cleaner/identity-verification`);
  const cleanerRightToWorkPage = await fetch(`${base}/cleaner/right-to-work`);
  const cleanerBackgroundChecksPage = await fetch(`${base}/cleaner/background-checks`);
  const cleanerWorkAreasPage = await fetch(`${base}/cleaner/work-areas`);
  const cleanerExperiencePage = await fetch(`${base}/cleaner/experience`);
  const cleanerReferencesPage = await fetch(`${base}/cleaner/references`, { redirect: "manual" });
  ...
  const cleanerAvailabilityPage = await fetch(`${base}/cleaner/availability`, { redirect: "manual" });
  ...
  const adminPaymentsPage = await fetch(`${base}/admin/payments`);
  const cleanerPersonalDetailsText = await cleanerPersonalDetailsPage.text();
  const cleanerBusinessDetailsText = await cleanerBusinessDetailsPage.text();
  const cleanerIdentityVerificationText = await cleanerIdentityVerificationPage.text();
  const cleanerRightToWorkText = await cleanerRightToWorkPage.text();
  const cleanerBackgroundChecksText = await cleanerBackgroundChecksPage.text();
  const cleanerWorkAreasText = await cleanerWorkAreasPage.text();
  ...
  assert(cleanerRightToWorkText.includes("Right to work") && cleanerRightToWorkText.includes("data-rtw-form") && cleanerRightToWorkText.includes("Are you a British or Irish citizen?") && cleanerRightToWorkText.includes("Enter your share code"), "The Cleaner right-to-work page lost its conditional form.");
  assert(cleanerBackgroundChecksText.includes("Background checks (DBS)") && cleanerBackgroundChecksText.includes("data-background-form") && cleanerBackgroundChecksText.includes("DBS certificate"), "The Cleaner background-check page lost its form.");
  assert(cleanerWorkAreasText.includes("Work areas") && cleanerWorkAreasText.includes("data-work-form") && cleanerWorkAreasText.includes("data-work-map-markers"), "The Cleaner work-areas page lost its form.");
  assert(cleanerExperienceText.includes("Skills and Experience") && cleanerExperienceText.includes("data-experience-form") && cleanerExperienceText.includes("Are you joining as a Cleaner or Beautician?") && cleanerExperienceText.includes("Years of cleaning experience") && cleanerExperienceText.includes("Beautician specialisms") === false && cleanerExperienceText.includes("Hair cutting") && cleanerExperienceText.includes("CV &amp; certificates"), "The Skills and Experience page lost its service-specific form.");
  assert(cleanerReferencesPage.status === 308 && cleanerReferencesPage.headers.get("location") === "/cleaner/insurance" && cleanerReferencesPage.headers.get("cache-control") === "no-store", "The removed Cleaner References page does not redirect safely to Insurance.");
  assert(cleanerInsuranceText.includes("Insurance") && cleanerInsuranceText.includes("data-insurance-form") && cleanerInsuranceText.includes("data-insurance-policy-template") && cleanerInsuranceText.includes("Insurance type") && cleanerInsuranceText.includes("Policy details"), "The Cleaner insurance page lost its per-type policy form.");
  ...
  // The page joined the Cleaner workspace design, so its headline is a workspace
  // heading rather than the marketing sentence it used to set at display size.
  // The two claims that matter are the ones asserted: bank details never reach
  // Homle, and the next screen is Stripe's own domain.
  assert(cleanerPayoutText.includes("Earnings and payouts") && cleanerPayoutText.includes("never enter bank details anywhere on Homle") && cleanerPayoutText.includes("connect.stripe.com"), "The Cleaner payout handoff is unavailable.");
  // It is also the page whose static header used to tell a signed-in Landlord
  // that their account was a Cleaner. It goes through the shared Cleaner
  // bootstrap now, which confirms the workspace boundary before revealing it.
  assert(cleanerPayoutText.includes("homle-cleaner.css") && cleanerPayoutText.includes("data-cleaner-payout-gate"), "The Cleaner payout page left the Cleaner workspace system or lost its role gate.");
  assert(landlordDashboardText.includes("Checking secure Landlord access") && landlordDashboardText.includes("data-landlord-workspace hidden") && landlordDashboardText.includes("data-speech-toggle>Speak</button>") && landlordDashboardText.includes("data-save-request>Add images") && landlordDashboardText.includes("Images and notes are optional") && landlordDashboardText.includes("Thank you. Your cleaning request is ready for matching.") && landlordDashboardText.includes("Stripe checkout opens") && landlordDashboardText.includes("After a Cleaner accepts the exact total"), "The private Landlord dashboard lost its optional instructions/images path or fail-closed request-to-checkout boundary.");
  ...
  assert(adminPaymentsText.includes("Administrator · test payments only") && adminPaymentsText.includes("data-admin-payments-workspace hidden") && adminPaymentsText.includes("Every button contacts the configured test payment provider"), "The Administrator payment page lost its guarded controls.");
  ...
  const forgottenPasswordPage = await fetch(`${base}/forgot-password`);
  ...
  assert(loginPageText.includes('href="/forgot-password">Forgot your password?</a>') && !loginPageText.includes('href="/reset-password">Forgot your password?</a>'), "The sign-in page does not send visible password-recovery traffic through the conventional capability-gated route.");
  assert(loginPage.ok && signupPage.ok && verificationPage.ok && forgottenPasswordPage.ok && resetPage.ok && onboardingPage.ok && loginPageText.includes("Checking secure account access") && signupPageText.includes("Checking secure account access") && !loginPageText.includes("data-pilot-actions") && !loginPageText.includes("or go straight to a workspace") && !loginPageText.includes('href="/landlord/dashboard"') && !loginPageText.includes('href="/cleaner/dashboard"') && loginPageText.includes("creates its account automatically") && loginPageText.includes("data-account-runtime hidden") && loginPageText.includes("data-account-controls disabled") && verificationPageText.includes('data-account-form="verification-request"') && !loginPageText.includes("Sign in with Google") && !signupPageText.includes('action="/api/') && authEntryAsset.ok && accountIntentAsset.ok && accountIntentText.includes('["book", "work"]') && accountIntentText.includes("accountIntentLifetimeMs") && authEntryText.includes('"/forgot-password": { form: "reset"') && authEntryText.includes("Account access is safely unavailable") && authEntryText.includes("providers.emailPassword === true") && authEntryText.includes("availableAccountMethodLabel(providers)") && !authEntryText.includes("Continue with Google, Apple or Facebook") && authEntryText.includes('history.replaceState(null, "", `${location.pathname}${location.search}`)') && authEntryText.includes('link.href = `${link.pathname}?intent=${accountIntent}`') && authEntryText.includes("intent: accountIntent") && authEntryText.includes('location.assign(`/login?intent=${accountIntent}#email=verified`)') && authEntryText.includes('const selectedRole = bookingIntent ? "landlord" : "cleaner"') && authEntryText.includes('location.assign("/onboarding?intent=book")') && authEntryText.includes('location.assign("/onboarding?intent=work")') && authEntryText.includes('"/api/marketplace/auth/workspace"') && authEntryText.includes("/api/marketplace/auth/login") && authEntryText.includes("/api/marketplace/auth/verification/resend"), "Capability-gated account-first entry/recovery pages displayed a non-working or cross-role action, advertised an unavailable provider, lost the conventional recovery URL, lost email verification intent, retained a fragment token, permitted an arbitrary redirect or failed to preserve secure multi-workspace onboarding.");
  ...
  assert(sharedStyles.ok && sharedStylesText.includes("[hidden] { display: none !important; }") && sharedStylesText.includes(".guided-form-step { scroll-margin-top: 92px;") && sharedStylesText.includes(".entry-route main > section:not(.forms-section)") && sharedStylesText.includes(".entry-route-request #cleaner-application") && sharedStylesText.includes(".entry-route-join #request-cleaning") && sharedStylesText.includes(".launch-parallel-action") && sharedStylesText.includes(".brief-hero .eyebrow { color: var(--homle-eyebrow") && sharedStylesText.includes(".capture-room-control") && sharedStylesText.includes(".photo-picker.is-disabled") && sharedStylesText.includes(".capture-voice-feedback") && sharedStylesText.includes(".capture-voice-action { width: 100%; }") && sharedStylesText.includes(".speech-room-context") && sharedStylesText.includes(".cleaner-handoff-room-missing") && sharedStylesText.includes(".cleaner-handoff-boundaries") && sharedStylesText.includes(".checklist-change-columns") && sharedStylesText.includes(".checklist-change-actions .button { width: 100%; }") && sharedStylesText.includes(".account-grid { display: grid;") && sharedStylesText.includes(".account-role:has(input:checked)") && sharedStylesText.includes(".account-actions .button { width: 100%; }") && sharedStylesText.includes(".account-form .button { width: 100%; }"), "Shared styling allowed hidden private actions or form stages to remain visible, gave the room-scan hero an unreadable step label, omitted the one-panel current-room photo/voice capture context, room-grouped Cleaner handoff or mobile checklist-change review, failed mobile account forms/actions, omitted launch guidance, or did not isolate focused entry screens.");
  assert(sharedStylesText.includes(".config-preview-economics") && sharedStylesText.includes("grid-template-columns: repeat(3, minmax(0, 1fr))") && sharedStylesText.includes("@media (max-width: 680px) { .config-preview-economics { grid-template-columns: 1fr; } }") && sharedStylesText.includes(".proposal-economics-preview") && sharedStylesText.includes(".proposal-economics-safe") && sharedStylesText.includes(".proposal-economics-blocked"), "The no-save minimum-job economics rehearsal or inline profitable-draft check omitted its responsive status layout.");
  ...
  assert(adminPageText.includes('name="labourOnCostPercent"') && adminPageText.includes('name="minimumContributionPounds"') && adminPageText.includes('name="travelCostPerKm"') && adminPageText.includes('name="travelDistanceMultiplier"') && adminPageText.includes('name="pricingTravelDistanceKm"') && adminPageText.includes('id="quote-travel-distance"') && adminPageText.includes("data-configured-travel-cost"), "Admin launch details omitted founder-set labour, absolute contribution or distance-aware travel assumptions and their quote rehearsal evidence.");
  assert(adminPageText.includes('id="readiness-continue"') && adminPageText.includes("Continue launch setup") && (adminPageText.match(/data-readiness-action/g) || []).length === 8 && (adminPageText.match(/data-readiness-action type="button"/g) || []).length === 8 && adminPageText.includes("/readiness-navigator.js"), "Admin launch readiness omitted its guided continue action or eight non-submitting area controls.");
  ...
  assert(adminScript.ok && adminScriptText.includes("function addLocalAuditPreview") && adminScriptText.includes("Open Landlord dashboard") && adminScriptText.includes("Open local cleaner preview") && adminScriptText.includes("const privateUrl = draft.privateUrl || \"\"") && adminScriptText.includes("/api/admin/booking-drafts") && adminScriptText.includes("Load copy-only booking handoffs") && adminScriptText.includes("booking.publicSiteUrl") && adminScriptText.includes("economics.configuredMeetsTarget") && adminScriptText.includes("economics.configuredRateGap") && adminScriptText.includes("function proposalEconomicsPanel") && adminScriptText.includes("Use ${money.format(safeRate.customerRate)}/hour minimum") && adminScriptText.includes("state.config.variableCostsConfirmed !== true") && adminScriptText.includes("submit.disabled = !marginSafe") && !adminScriptText.includes("location.origin") && !adminScriptText.includes("Copy private link") && !adminScriptText.includes("Copy cleaner link") && !adminScriptText.includes("Copy verified booking-pack link"), "The control desk omitted authoritative rehearsal/inline proposal economics, dashboard handoff or safe recipient-link boundaries.");
  for (const retiredRoute of ["/brief", "/brief-complete", "/request-status", "/cleaner-status", "/quote"]) {
    const retiredPage = await fetch(`${base}${retiredRoute}`);
    assert(retiredPage.status === 404, `${retiredRoute} remains publicly served.`);
  }
  for (const retiredAsset of ["/brief.js", "/brief-complete.js", "/request-status.js", "/cleaner-status.js", "/quote.js"]) {
    const retiredScript = await fetch(`${base}${retiredAsset}?v=smoke-test`);
    assert(retiredScript.status === 404, `${retiredAsset} remains publicly served.`);
  }
  ...
  assert(customerBookingPage.status === 404 && cleanerAssignmentPage.status === 404 && bookingPackAsset.status === 404, "Retired booking-pack pages or controller remain publicly served.");
  ...
  assert(adminAssetText.includes("/api/admin/config/preview") && adminAssetText.includes("renderConfigPreview") && adminAssetText.includes("Nothing has been saved") && adminAssetText.includes("attachParticipantRehearsalImport") && adminAssetText.includes("participantRehearsalEvidence") && !adminAssetText.includes("renderReadiness(result.readiness);\n    renderConfigPreview"), "Admin assets omitted the non-persisting launch rehearsal, hosted-evidence import or confused preview readiness with recorded readiness.");
  ...
  assert(adminAssetText.includes("funnel.dispatchReadyCleaners === 0") && adminAssetText.includes("cleaner-supply-stages") && adminAssetText.includes("funnel.supplyBottleneck") && adminAssetText.includes("Open next Cleaner record") && adminAssetText.includes("launch-supply-action") && adminAssetText.includes('https://homlle.com/cleaner/onboarding') && adminAssetText.includes("Copy application link") && adminAssetText.includes("Copy honest recruitment message") && adminAssetText.includes("copyDraft") && adminAssetText.includes("Applying does not guarantee work") && adminAssetText.includes("An application is not dispatch-ready"), "Admin launch runway omitted the evidence-backed Cleaner supply progression, next review action or honest no-send recruitment handoff.");
  ...
  assert(publicFormAsset.status === 404, "Retired public form controller remains publicly served.");
  ...
  assert(removedCleanerPreviewAsset.status === 404, "Removed Cleaner profile-preview code remains publicly shipped.");
  ...
  const [landingCss, landingScript, retiredLandingCss, retiredLandingScript, retiredContentAddressedScript, compactLogo128, compactLogo192, originalLogo, cleanHero960, dirtyHero960, originalHero, scanAngleWebp, scanAnglePng, supportingWideWebp, supportingNarrowWebp, supportingJpeg, optimizedClip, retiredClip] = await Promise.all([
    fetch(`${base}/landing-f56e7ce9.css`),
  ...
  const invalidPhone = await fetch(`${base}/api/cleaning-requests`, {
    ...
    body: JSON.stringify({ contactName: "Test Customer", email: "customer@example.com", phone: "123", postcode: "SW1A 1AA", customerType: "Landlord", propertyType: "Flat or house", service: "Regular home clean", siteSize: "2 bedrooms and 1 bathroom", accessNotes: "Collect keys from the office", hazards: "None known", consent: true })
  });
  ...
  const invalidPostcode = await fetch(`${base}/api/cleaning-requests`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ contactName: "Test Customer", email: "customer@example.com", phone: "07123456789", postcode: "Central London", customerType: "Landlord", propertyType: "Flat or house", service: "Regular home clean", siteSize: "2 bedrooms and 1 bathroom", accessNotes: "Collect keys from the office", hazards: "None known", consent: true })
  });
  ...
  const validRequestInput = { contactName: "Test Customer", email: "customer@example.com", postcode: "SW1A 1AA", propertyType: "Flat or house", service: "Regular home clean", hazards: "None known", frequency: "Weekly", preferredDate: visitDay, preferredTimeWindow: "Morning (8am–12pm)", consent: true };
  ...
  assert(initialRequestStatus.ok && initialRequestStatusBody.current.stage === "room-scan" && initialRequestStatusBody.links.roomScanRequired === true && initialRequestStatusBody.request.reference === requestBody.reference && initialRequestStatusBody.request.frequency === "Weekly" && initialRequestStatusBody.request.preferredDate === visitDay && initialRequestStatusBody.request.preferredTimeWindow === "Morning (8am–12pm)" && initialRequestStatusBody.scheduleChange.allowed === true && initialRequestStatusBody.scheduleChange.history.length === 0, "New request tracker did not open at room scan with its requested timing and customer-owned pre-booking schedule control.");
  ...
  const invalidScheduleToken = await fetch(`${base}/api/request-schedule`, { method: "POST", headers: { "content-type": "application/json", "x-request-token": "not-a-private-request-token" }, body: JSON.stringify({ preferredDate: visitDayPlus1, preferredTimeWindow: "Flexible", reason: "The property handover date changed.", confirmed: true }) });
  ...
  const scheduleChange = await fetch(`${base}/api/request-schedule`, { method: "POST", headers: { "content-type": "application/json", "x-request-token": requestBody.customerStatusToken }, body: JSON.stringify({ preferredDate: visitDayPlus1, preferredTimeWindow: "Afternoon (12pm–5pm)", reason: "The property handover moved to the following afternoon.", confirmed: true }) });
  const scheduleChangeBody = await scheduleChange.json();
  assert(scheduleChange.status === 201 && scheduleChangeBody.reference.startsWith("RSC-") && scheduleChangeBody.preferredDate === visitDayPlus1, "A valid customer-owned requested timing change was not recorded.");
  ...
  assert(changedScheduleStatusBody.request.preferredDate === visitDayPlus1 && changedScheduleStatusBody.request.preferredTimeWindow === "Afternoon (12pm–5pm)" && changedScheduleStatusBody.scheduleChange.originalPreferredDate === visitDay && changedScheduleStatusBody.scheduleChange.history.length === 1 && changedScheduleStatusBody.scheduleChange.history[0].reason.includes("handover"), "The private tracker did not project the latest customer timing while preserving its original and append-only history.");
  const replayedScheduleChange = await fetch(`${base}/api/request-schedule`, { method: "POST", headers: { "content-type": "application/json", "x-request-token": requestBody.customerStatusToken }, body: JSON.stringify({ preferredDate: visitDayPlus1, preferredTimeWindow: "Afternoon (12pm–5pm)", reason: "Repeated browser submission should not create another history item.", confirmed: true }) });
  ...
  const restoredSchedule = await fetch(`${base}/api/request-schedule`, { method: "POST", headers: { "content-type": "application/json", "x-request-token": requestBody.customerStatusToken }, body: JSON.stringify({ preferredDate: visitDay, preferredTimeWindow: "Morning (8am–12pm)", reason: "Restore the original test timing for later matching checks.", confirmed: true }) });
  ...
  assert(restoredScheduleStatusBody.request.preferredDate === visitDay && restoredScheduleStatusBody.scheduleChange.history.length === 2 && !JSON.stringify(restoredScheduleStatusBody).includes("customer@example.com"), "The latest requested timing or tracker privacy was lost after multiple append-only changes.");
  ...
  const optionalRoomNote = await fetch(`${base}/api/job-briefs`, { method: "POST", headers: { "content-type": "application/json", "x-request-token": requestBody.customerStatusToken }, body: JSON.stringify({ requestId: requestBody.reference, email: "customer@example.com", transcript: "Clean the kitchen worktops.", checklist: ["Kitchen: Clean the kitchen worktops"], photos: [{ area: "Kitchen", dataUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9Zs7sAAAAASUVORK5CYII=" }], scopeCompleteConfirmed: true, consent: true }) });
  const optionalRoomNoteBody = await optionalRoomNote.json();
  assert(optionalRoomNote.status === 201 && optionalRoomNoteBody.photos?.[0]?.note === "See the confirmed Kitchen checklist for cleaning instructions.", "The server did not safely label a room visual when the customer skipped the optional duplicate description.");
  ...
  const validCleanerInput = { fullName: "Test Cleaner", email: "cleaner@example.com", phone: "07123456789", postcode: "SE1 7PB", travelAreas: "SW1A and South London", experience: "1–3 years", availability: "Weekdays", firstAvailableDate: visitDay, firstAvailableStartTime: "08:00", firstAvailableEndTime: "15:00", serviceDomestic: true, rightToWork: true, consent: true };
  ...
  const availabilityRequestInput = { availableDate: visitDay, startTime: "08:00", endTime: "15:00", note: "Available after the morning train." };
  ...
  const requestBriefs = adminBody.records.find((record) => record.id === requestBody.reference)?.briefs || [];
  const mainRequestBrief = requestBriefs.find((brief) => brief.id === briefBody.reference);
  assert(adminBody.records.find((record) => record.id === requestBody.reference)?.customerType === "Cleaning customer" && adminBody.records.find((record) => record.id === requestBody.reference)?.accessNotes === "Confirm privately after booking" && adminBody.records.find((record) => record.id === requestBody.reference)?.phone === "", "The shorter public request did not receive safe server-owned defaults or still required a secondary phone contact.");
  assert(requestBriefs.length === 2 && mainRequestBrief?.customerScopeConfirmed === true && Date.parse(mainRequestBrief?.customerScopeConfirmedAt) > 0 && requestBriefs.every((brief) => brief.customerScopeConfirmed === true), "Photo job briefs or their customer scope confirmations were not attached to the request, or an unconfirmed attempt was stored.");
  assert(mainRequestBrief?.status === "landlord-draft", "New photo job brief did not enter the human review queue.");
  ...
  assert(adminBody.launchFunnel?.stages?.find((stage) => stage.key === "requests")?.count === 1 && adminBody.launchFunnel?.stages?.find((stage) => stage.key === "scans")?.count === 1 && adminBody.launchFunnel?.stages?.find((stage) => stage.key === "reviewed")?.count === 0 && adminBody.launchFunnel?.supplyStages?.find((stage) => stage.key === "applications")?.count === 2 && adminBody.launchFunnel?.supplyStages?.find((stage) => stage.key === "screened")?.count === 0 && adminBody.launchFunnel?.supplyStages?.find((stage) => stage.key === "approved")?.count === 0 && adminBody.launchFunnel?.supplyStages?.find((stage) => stage.key === "available")?.count === 0 && adminBody.launchFunnel?.supplyBottleneck?.key === "screening" && adminBody.launchFunnel?.dispatchReadyCleaners === 0 && adminBody.launchFunnel?.goal?.achieved === false && adminBody.launchFunnel?.bottleneck?.key === "launch-readiness" && adminBody.launchFunnel?.parallelAction?.key === "scan-review", "Initial first-booking funnel did not separate launch readiness, demand progress and the evidence-backed Cleaner supply bottleneck.");
  ...
  assert(initialConfig.ok && initialConfigBody.readiness.completed === 0 && initialConfigBody.readiness.total === 8 && initialConfigBody.readiness.next?.key === "identity" && initialConfigBody.readiness.missing?.identity?.includes("legal owner name") && initialConfigBody.readiness.missing?.contact?.includes("domain and deployment evidence naming this hostname") && initialConfigBody.readiness.missing?.contact?.includes("public website verification date") && initialConfigBody.readiness.missing?.insurance?.includes("insurance provider") && initialConfigBody.readiness.missing?.payments?.includes("provider verification date") && initialConfigBody.readiness.missing?.participantRehearsal?.includes("two-account mobile booking and payment rehearsal") && initialConfigBody.storageSafety?.safeForPrivatePilot === true && initialConfigBody.storageSafety?.relocationRequired === false && initialConfigBody.activationReadiness?.completed === 1 && initialConfigBody.activationReadiness?.total === 11 && initialConfigBody.activationReadiness?.ready === false && initialConfigBody.activationReadiness?.checks?.privateDataStorage === true && initialConfigBody.activationReadiness?.checks?.privateMedia === false && initialConfigBody.activationReadiness?.checks?.transactionalEmail === false && initialConfigBody.activationReadiness?.checks?.realtimeUpdates === false && initialConfigBody.activationReadiness?.checks?.postcodeGeocoding === false && initialConfigBody.activationReadiness?.checks?.matchingPricing === false && initialConfigBody.activationReadiness?.next?.key === "accountAccess" && initialConfigBody.activationReadiness?.next?.action?.includes("account email delivery") && !JSON.stringify(initialConfigBody.activationReadiness).includes("DATABASE_URL") && initialConfigBody.economics.available === false, "Initial launch readiness did not separate exact founder decisions, account-first technical activation, participant rehearsal, safe temporary storage or incomplete economics without exposing environment details.");
  ...
  const hostedRehearsalReport = { status: "verified", bookingFingerprint: "fedcba654321", accounts: { landlord: "approved-and-verified", cleaner: "approved-and-verified" }, bookingLifecycle: "completed", checklist: { tasks: 4, resolved: true }, privateMedia: { beforeAndAfter: true, currentLocationRemoved: true }, messaging: "two-way", review: "exactly-one", stripeTestCycle: "authorized-captured-transferred-reversed-and-refunded", outsiderAccess: "denied", database: "homle_marketplace_homle_staging", host: "private-db.example", writesPerformed: false };
  const completeConfig = { legalOwnerName: "Test Owner", businessStructure: "Sole trader", legalBusinessName: "Test Homle", tradingAddress: "1 Test Street, London", supportEmail: "support@example.com", supportPhone: "07123456789", publicSiteUrl: "https://tideway.example.com", publicSiteEvidenceNote: "tideway.example.com ownership, HTTPS certificate and deployed Homle pages were checked.", publicSiteVerifiedDate: "2026-07-14", pilotPostcodes: "SW1A, SW2, SW4", cleanerModel: "Worker", insuranceStatus: "active", insuranceProvider: "Test Insurer", insuranceEvidenceNote: "Test policy cover, limit and securely stored certificate were reviewed.", insuranceReviewDate: "2026-12-31", paymentProviderName: "TestPay", paymentProviderStatus: "live", paymentProviderEvidenceNote: "Test live account, payout destination and refund route were reviewed.", paymentProviderVerifiedDate: "2026-07-14", participantRehearsalPassed: true, participantRehearsalEvidenceSource: "hosted-verifier", participantRehearsalBookingFingerprint: "fedcba654321", participantRehearsalEvidenceNote: "Two non-customer accounts completed the two-device booking, test payment, progress and cleanup rehearsal.", participantRehearsalVerifiedDate: "2026-08-16", participantRehearsalReport: JSON.stringify(hostedRehearsalReport), refundProcess: "Owner approves and records refunds within five working days.", customerHourlyRate: 30, cleanerHourlyPay: 18, labourOnCostPercent: 5, minimumHours: 2, minimumContributionMarginPercent: 25, minimumContributionPounds: 18, paymentFeePercent: 1, paymentFeeFixed: 0, travelCostPerJob: 1, travelCostPerKm: 0, travelDistanceMultiplier: 2, pricingTravelDistanceKm: 0, suppliesCostPerJob: 1, riskContingencyPercent: 1, variableCostsConfirmed: true, cancellationPolicy: "24 hours notice.", paymentTiming: "Payment authorised at booking and captured after completion", customerQuoteValidityHours: 24, cleanerOpportunityValidityHours: 12, inactiveMediaRetentionDays: 90, completedMediaRetentionDays: 365 };
  ...
  const rehearsalReportPreview = await fetch(`${base}/api/admin/config/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ participantRehearsalReport: JSON.stringify(hostedRehearsalReport) }) });
  const rehearsalReportPreviewBody = await rehearsalReportPreview.json();
  assert(rehearsalReportPreview.ok && rehearsalReportPreviewBody.participantRehearsalEvidence?.source === "hosted-verifier" && rehearsalReportPreviewBody.participantRehearsalEvidence?.bookingFingerprint === "fedcba654321" && rehearsalReportPreviewBody.participantRehearsalEvidence?.evidenceNote.includes("4 resolved Cleaner tasks") && !JSON.stringify(rehearsalReportPreviewBody).includes("private-db.example") && !JSON.stringify(rehearsalReportPreviewBody).includes("homle_marketplace_homle_staging"), "The privacy-safe hosted rehearsal report was not validated or leaked database details.");
  const invalidRehearsalReportPreview = await fetch(`${base}/api/admin/config/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ participantRehearsalReport: JSON.stringify({ ...hostedRehearsalReport, outsiderAccess: "allowed" }) }) });
  const invalidRehearsalReportPreviewBody = await invalidRehearsalReportPreview.json();
  assert(invalidRehearsalReportPreview.status === 422 && invalidRehearsalReportPreviewBody.errors.some((error) => error.includes("incomplete or invalid")), "An unsafe or incomplete hosted rehearsal report passed validation.");
  const forgedRehearsalEvidencePreview = await fetch(`${base}/api/admin/config/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ participantRehearsalPassed: true, participantRehearsalEvidenceSource: "hosted-verifier", participantRehearsalBookingFingerprint: "fedcba654321", participantRehearsalEvidenceNote: "A manually typed claim that was never validated by the hosted verifier.", participantRehearsalVerifiedDate: "2026-08-16" }) });
  const forgedRehearsalEvidencePreviewBody = await forgedRehearsalEvidencePreview.json();
  assert(forgedRehearsalEvidencePreview.status === 422 && forgedRehearsalEvidencePreviewBody.errors.some((error) => error.includes("newly validated report")), "A manually forged hosted-verifier claim passed participant rehearsal validation.");
  ...
  assert(configPreview.ok && configPreviewBody.ok === true && configPreviewBody.persisted === false && configPreviewBody.readiness.ready === true && !Object.hasOwn(configPreviewBody, "config") && configPreviewBody.economics.available === true && configPreviewBody.economics.costAssumptionsConfirmed === true && configPreviewBody.economics.minimumHours === 2 && configPreviewBody.economics.configured.customerTotal === 60 && configPreviewBody.economics.configured.cleanerPay === 36 && configPreviewBody.economics.configured.nonCleanerCosts === 5 && configPreviewBody.economics.configured.contribution === 19 && configPreviewBody.economics.configuredMeetsTarget === true && configPreviewBody.economics.targetSafeCustomerRate === 29.49 && configPreviewBody.economics.targetSafeCustomerTotal === 58.98 && configPreviewBody.economics.targetRateSupported === true && configPreviewBody.economics.configuredRateGap === 0 && configAfterPreview === configBeforePreview, "Launch readiness rehearsal wrote configuration, leaked submitted details or returned incorrect minimum-job economics.");
  const distanceConfigPreview = await fetch(`${base}/api/admin/config/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, travelCostPerKm: 0.45, pricingTravelDistanceKm: 10 }) });
  const distanceConfigPreviewBody = await distanceConfigPreview.json();
  assert(distanceConfigPreview.ok && distanceConfigPreviewBody.persisted === false && distanceConfigPreviewBody.economics.pricingTravelDistanceKm === 10 && distanceConfigPreviewBody.economics.configured.travelCosts === 10 && distanceConfigPreviewBody.economics.configured.nonCleanerCosts === 14 && distanceConfigPreviewBody.economics.configured.contribution === 10 && distanceConfigPreviewBody.economics.configuredMeetsTarget === false, "Distance-based travel rehearsal omitted the verified journey or failed to expose its effect on contribution.");
  const missingDistancePreview = await fetch(`${base}/api/admin/config/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, travelCostPerKm: 0.45, pricingTravelDistanceKm: 0 }) });
  const missingDistancePreviewBody = await missingDistancePreview.json();
  assert(missingDistancePreview.status === 422 && missingDistancePreviewBody.persisted === false && missingDistancePreviewBody.errors.some((error) => error.includes("one-way travel distance")), "Per-kilometre travel pricing was accepted without a conservative rehearsal distance.");
  const negativeTravelPreview = await fetch(`${base}/api/admin/config/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, travelCostPerKm: -0.1 }) });
  assert(negativeTravelPreview.status === 422, "A negative per-kilometre travel cost was silently converted into a valid assumption.");
  ...
  const importedRehearsalConfig = await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, participantRehearsalPassed: false, participantRehearsalEvidenceSource: "", participantRehearsalBookingFingerprint: "", participantRehearsalEvidenceNote: "", participantRehearsalVerifiedDate: "", participantRehearsalReport: JSON.stringify(hostedRehearsalReport) }) });
  const importedRehearsalConfigBody = await importedRehearsalConfig.json();
  const importedRehearsalStored = JSON.parse(await readFile(configPath, "utf8"));
  assert(importedRehearsalConfig.ok && importedRehearsalConfigBody.config.participantRehearsalPassed === true && importedRehearsalConfigBody.config.participantRehearsalEvidenceSource === "hosted-verifier" && importedRehearsalConfigBody.config.participantRehearsalBookingFingerprint === "fedcba654321" && importedRehearsalConfigBody.readiness.ready === true && !Object.hasOwn(importedRehearsalStored, "participantRehearsalReport") && !JSON.stringify(importedRehearsalStored).includes("private-db.example") && !JSON.stringify(importedRehearsalStored).includes("homle_marketplace_homle_staging"), "Validated participant evidence was not safely projected or retained private verifier input.");
  const { participantRehearsalPassed: _omittedPassed, participantRehearsalReport: _omittedReport, ...configWithoutRehearsalControls } = completeConfig;
  const preservedRehearsalConfig = await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(configWithoutRehearsalControls) });
  const preservedRehearsalConfigBody = await preservedRehearsalConfig.json();
  assert(preservedRehearsalConfig.ok && preservedRehearsalConfigBody.config.participantRehearsalPassed === true && preservedRehearsalConfigBody.readiness.ready === true, "An unrelated Admin settings save erased previously verified participant rehearsal evidence.");
  ...
  const unprovenParticipantRehearsal = await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, participantRehearsalReport: "", participantRehearsalPassed: false, participantRehearsalEvidenceNote: "", participantRehearsalVerifiedDate: "" }) });
  const unprovenParticipantRehearsalBody = await unprovenParticipantRehearsal.json();
  assert(unprovenParticipantRehearsal.ok && unprovenParticipantRehearsalBody.readiness.ready === false && unprovenParticipantRehearsalBody.readiness.checks.participantRehearsal === false && unprovenParticipantRehearsalBody.readiness.missing.participantRehearsal.includes("two-account mobile booking and payment rehearsal"), "An unperformed participant rehearsal passed launch readiness.");
  const vagueParticipantRehearsal = await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, participantRehearsalReport: "", participantRehearsalEvidenceNote: "It passed." }) });
  assert(vagueParticipantRehearsal.status === 422, "A participant rehearsal was marked passed with vague evidence.");
  const futureParticipantRehearsal = await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, participantRehearsalReport: "", participantRehearsalVerifiedDate: "2026-12-31" }) });
  assert(futureParticipantRehearsal.status === 422, "A future participant-rehearsal verification date was accepted.");
  ...
  const missingContributionFloor = await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, minimumContributionPounds: 0 }) });
  const missingContributionFloorBody = await missingContributionFloor.json();
  assert(missingContributionFloor.ok && missingContributionFloorBody.readiness.ready === false && missingContributionFloorBody.readiness.missing.economics.includes("founder minimum contribution per booking"), "Missing founder per-booking contribution floor did not block launch readiness.");
  ...
  const expectedScanUrl = "https://tideway.example.com/landlord/dashboard";
  ...
  const overlappingPendingAvailability = await fetch(`${base}/api/cleaner-availability-requests`, { method: "POST", headers: { "content-type": "application/json", "x-cleaner-status-token": cleanerBody.cleanerStatusToken, "idempotency-key": "66666666-6666-4666-8666-666666666666" }, body: JSON.stringify({ availableDate: visitDay, startTime: "10:00", endTime: "16:00" }) });
  ...
  const uncoveredCleanerAvailability = await fetch(`${base}/api/admin/cleaner-availability`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ cleanerId: uncoveredCleanerBody.reference, availableDate: visitDay, startTime: "08:00", endTime: "15:00", confirmationNote: "Test-only availability confirmed for the travel-coverage gate." }) });
  ...
  const uncoveredCleanerProposal = await fetch(`${base}/api/admin/proposals`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: requestBody.reference, cleanerId: uncoveredCleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 4, customerRate: 30, cleanerRate: 18, otherCosts: 0 }) });
  ...
  const noAvailabilityProposal = await fetch(`${base}/api/admin/proposals`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 4, customerRate: 30, cleanerRate: 18, otherCosts: 10 }) });
  ...
  const unverifiedAvailability = await fetch(`${base}/api/admin/cleaner-availability`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ cleanerId: cleanerBody.reference, availableDate: visitDay, startTime: "08:00", endTime: "15:00", confirmationNote: "short" }) });
  assert(unverifiedAvailability.status === 422, "Cleaner availability was recorded without a meaningful confirmation note.");
  const availabilityWindows = [
    { availableDate: visitDayPlus2, startTime: "08:00", endTime: "13:00" },
    { availableDate: visitDayPlus3, startTime: "08:00", endTime: "13:00" }
  ];
  ...
  const declinedRequest = await fetch(`${base}/api/cleaner-availability-requests`, { method: "POST", headers: { "content-type": "application/json", "x-cleaner-status-token": cleanerBody.cleanerStatusToken, "idempotency-key": "77777777-7777-4777-8777-777777777777" }, body: JSON.stringify({ availableDate: visitDayPlus4, startTime: "09:00", endTime: "14:00", note: "Test-only later window." }) });
  ...
  const overlappingAvailability = await fetch(`${base}/api/admin/cleaner-availability`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ cleanerId: cleanerBody.reference, availableDate: visitDay, startTime: "10:00", endTime: "16:00", confirmationNote: "Test-only overlapping availability confirmation window." }) });
  assert(overlappingAvailability.status === 409, "Overlapping confirmed availability windows were accepted.");
  const outsideAvailabilityProposal = await fetch(`${base}/api/admin/proposals`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "14:00", estimatedHours: 2, customerRate: 30, cleanerRate: 18, otherCosts: 0 }) });
  ...
  const unscannedRequest = await fetch(`${base}/api/cleaning-requests`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ contactName: "No Scan Customer", email: "noscan@example.com", phone: "07123456788", postcode: "SW1A 2AA", customerType: "Homeowner or tenant", propertyType: "Flat or house", service: "Regular home clean", siteSize: "1 bedroom and 1 bathroom", accessNotes: "Meet at the property", hazards: "None known", preferredDate: visitDayPlus2, preferredTimeWindow: "Flexible", consent: true })
  });
  ...
  const unscannedProposal = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: unscannedRequestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDayPlus2, proposedStartTime: "09:00", estimatedHours: 3, customerRate: 30, cleanerRate: 18, otherCosts: 5 })
  });
  ...
  const losingProposal = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 4, customerRate: 15, cleanerRate: 18, otherCosts: 0 })
  });
  ...
  const belowMinimumHoursProposal = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 1, customerRate: 30, cleanerRate: 18, otherCosts: 0 })
  });
  ...
  const thinMarginProposal = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 4, customerRate: 30, cleanerRate: 23.5, otherCosts: 0 })
  });
  ...
  const overflowProposal = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 4, customerRate: "1e309", cleanerRate: 18, otherCosts: 0 })
  });
  ...
  const extremeCostProposal = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 4, customerRate: 30, cleanerRate: 18, otherCosts: 100001 })
  });
  assert(extremeCostProposal.status === 422, "Proposal costs above the supported financial limit were accepted.");

  const missingTimeProposal = await fetch(`${base}/api/admin/proposals`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, estimatedHours: 4, customerRate: 30, cleanerRate: 18, otherCosts: 0 }) });
  assert(missingTimeProposal.status === 422, "Proposal without an exact start time was accepted.");
  const overnightProposal = await fetch(`${base}/api/admin/proposals`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "23:00", estimatedHours: 4, customerRate: 30, cleanerRate: 18, otherCosts: 0 }) });
  ...
  await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(completeConfig) });
  const absoluteContributionBlocked = await fetch(`${base}/api/admin/proposals`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 2, customerRate: 29, cleanerRate: 18, otherCosts: 0, travelDistanceKm: 10 }) });
  assert(absoluteContributionBlocked.status === 422, "A high-margin small job below the founder's pounds-per-booking contribution floor was accepted.");

  const validProposal = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 4, customerRate: 30, cleanerRate: 18, otherCosts: 10, travelDistanceKm: 10, note: "Draft only" })
  });
  const proposalBody = await validProposal.json();
  assert(validProposal.status === 201 && proposalBody.proposal.economicsModelVersion === 2 && proposalBody.proposal.profitTargetVersion === 1 && proposalBody.proposal.targetMarginPercent === 25 && proposalBody.proposal.targetContribution === 18 && proposalBody.proposal.travelDistanceKm === 10 && proposalBody.proposal.costAssumptions.travelDistanceMultiplier === 2 && proposalBody.proposal.contribution === 30 && proposalBody.proposal.labourOnCosts === 3.6 && proposalBody.proposal.paymentFees === 1.2 && proposalBody.proposal.riskContingency === 1.2 && proposalBody.proposal.nonCleanerCosts === 18 && proposalBody.proposal.proposedEndTime === "13:00" && /^[A-Za-z0-9_-]{32}$/.test(proposalBody.proposal.reviewToken) && /^[A-Za-z0-9_-]{32}$/.test(proposalBody.proposal.cleanerReviewToken), "Valid distance-versioned draft proposal failed, omitted its frozen journey/cost/profit-target model, calculated incorrectly, omitted its schedule or omitted a private review token.");
  const blockedScheduleDuringProposal = await fetch(`${base}/api/request-schedule`, { method: "POST", headers: { "content-type": "application/json", "x-request-token": requestBody.customerStatusToken }, body: JSON.stringify({ preferredDate: visitDayPlus1, preferredTimeWindow: "Flexible", reason: "Attempt to move timing after quote preparation started.", confirmed: true }) });
  ...
  await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, minimumContributionPounds: 31 }) });
  const increasedContributionBlocked = await fetch(`${base}/api/admin/proposals/status`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ proposalId: proposalBody.proposal.id, status: "ready" }) });
  assert(increasedContributionBlocked.status === 422, "Existing proposal bypassed a newly increased founder per-booking contribution floor.");

  await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, minimumContributionMarginPercent: 20, minimumContributionPounds: 15 }) });
  const changedProfitTargetsBlocked = await fetch(`${base}/api/admin/proposals/status`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ proposalId: proposalBody.proposal.id, status: "ready" }) });
  const changedProfitTargetDrafts = await fetch(`${base}/api/admin/proposal-drafts?proposalId=${proposalBody.proposal.id}`);
  const changedProfitTargetDraftsBody = await changedProfitTargetDrafts.json();
  assert(changedProfitTargetsBlocked.status === 422 && changedProfitTargetDrafts.ok && changedProfitTargetDraftsBody.sendAllowed === false && changedProfitTargetDraftsBody.warnings.some((warning) => warning.includes("profit targets changed")), "An old proposal remained usable after both founder profit targets changed to lower values that its raw economics still passed.");
  ...
  const eveningRequest = await fetch(`${base}/api/cleaning-requests`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contactName: "Evening Customer", email: "evening@example.com", phone: "07123456777", postcode: "SW1A 2AA", customerType: "Landlord", propertyType: "Flat or house", service: "Regular home clean", siteSize: "1 bedroom and 1 bathroom", accessNotes: "Test access only", hazards: "None known", preferredDate: visitDayPlus2, preferredTimeWindow: "Evening (5pm–8pm)", consent: true }) });
  ...
  const underScopedProposal = await fetch(`${base}/api/admin/proposals`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 3, customerRate: 30, cleanerRate: 18, otherCosts: 5 }) });
  ...
  const restoredAvailability = await fetch(`${base}/api/admin/cleaner-availability`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ cleanerId: cleanerBody.reference, availableDate: visitDay, startTime: "08:00", endTime: "15:00", confirmationNote: "Test-only cleaner reconfirmed this exact availability window." }) });
  ...
  assert(quotePreview.ok && quotePreviewBody.quote.reference === proposalBody.proposal.id && quotePreviewBody.quote.frequency === "Weekly" && quotePreviewBody.quote.siteSize === "Reviewed room scan: Kitchen" && quotePreviewBody.quote.proposedStartTime === "09:00" && quotePreviewBody.quote.proposedEndTime === "13:00" && quotePreviewBody.quote.decisionAllowed === false && quotePreviewBody.quote.checklist.includes("Kitchen: Clean inside the oven") && quotePreviewBody.quote.confirmedExtras?.[0]?.code === "oven-interior", "Private customer quote preview omitted the scan-derived site scope, approved frequency, confirmed extra or schedule, or opened decisions too early.");
  ...
  assert(opportunityPreview.ok && opportunityPreviewBody.opportunity.reference === proposalBody.proposal.id && opportunityPreviewBody.opportunity.frequency === "Weekly" && opportunityPreviewBody.opportunity.siteSize === "Reviewed room scan: Kitchen" && opportunityPreviewBody.opportunity.proposedStartTime === "09:00" && opportunityPreviewBody.opportunity.proposedEndTime === "13:00" && opportunityPreviewBody.opportunity.decisionAllowed === false && opportunityPreviewBody.opportunity.cleanerPay === 72 && opportunityPreviewBody.opportunity.checklist.includes("Kitchen: Clean inside the oven") && opportunityPreviewBody.opportunity.confirmedExtras?.[0]?.code === "oven-interior" && opportunityPreviewBody.opportunity.photoSharingConsent === true && opportunityPreviewBody.opportunity.photoAccessAllowed === false && opportunityPreviewBody.opportunity.roomPhotos.length === 0, "Private cleaner opportunity preview omitted the scan-derived site scope, requested frequency, reviewed tasks, confirmed extra, schedule or pay, opened decisions too early or exposed photos before sending.");
  ...
  await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...completeConfig, minimumContributionMarginPercent: 20, minimumContributionPounds: 15 }) });
  const retargetedQuote = await fetch(`${base}/api/quote`, { headers: { "x-quote-token": proposalBody.proposal.reviewToken } });
  const retargetedQuoteBody = await retargetedQuote.json();
  const retargetedOpportunity = await fetch(`${base}/api/opportunity`, { headers: { "x-opportunity-token": proposalBody.proposal.cleanerReviewToken } });
  const retargetedOpportunityBody = await retargetedOpportunity.json();
  const retargetedCustomerDecision = await fetch(`${base}/api/quote/decision`, { method: "POST", headers: { "content-type": "application/json", "x-quote-token": proposalBody.proposal.reviewToken }, body: JSON.stringify({ decision: "accepted", typedName: "Test Customer", scopeConfirmed: true, scheduleConfirmed: true, termsAccepted: true }) });
  const retargetedCleanerDecision = await fetch(`${base}/api/opportunity/decision`, { method: "POST", headers: { "content-type": "application/json", "x-opportunity-token": proposalBody.proposal.cleanerReviewToken }, body: JSON.stringify({ decision: "accepted", typedName: "Test Cleaner", scopeConfirmed: true, payConfirmed: true, availabilityConfirmed: true }) });
  assert(retargetedQuote.ok && retargetedQuoteBody.quote.pricingChanged === true && retargetedQuoteBody.quote.decisionAllowed === false && retargetedOpportunity.ok && retargetedOpportunityBody.opportunity.pricingChanged === true && retargetedOpportunityBody.opportunity.decisionAllowed === false && retargetedCustomerDecision.status === 409 && retargetedCleanerDecision.status === 409, "A sent customer quote or Cleaner opportunity stayed actionable after the founder profit targets changed.");
  await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(completeConfig) });
  ...
  const overlapRequest = await fetch(`${base}/api/cleaning-requests`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ contactName: "Overlap Customer", email: "overlap@example.com", phone: "07123456780", postcode: "SW1A 2AA", customerType: "Landlord", propertyType: "Flat or house", service: "Regular home clean", siteSize: "1 bedroom and 1 bathroom", accessNotes: "Access to be confirmed", hazards: "None known", frequency: "One-off", preferredDate: visitDay, preferredTimeWindow: "Afternoon (12pm–5pm)", consent: true })
  });
  ...
  const heldCapacityProposal = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: overlapRequestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "11:00", estimatedHours: 2, customerRate: 30, cleanerRate: 18, otherCosts: 0 })
  });
  ...
  const overlapProposal = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: overlapRequestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDayPlus2, proposedStartTime: "09:00", estimatedHours: 2, customerRate: 30, cleanerRate: 18, otherCosts: 0 })
  });
  ...
  assert(alternativeTimingQuote.ok && alternativeTimingQuoteBody.quote.requestedDate === visitDay && alternativeTimingQuoteBody.quote.proposedDate === visitDayPlus2 && alternativeTimingQuoteBody.quote.requestedTimeWindow === "Afternoon (12pm–5pm)" && alternativeTimingQuoteBody.quote.alternativeTiming === true && alternativeTimingQuoteBody.quote.alternativeTimingReasons.dateChanged === true && alternativeTimingQuoteBody.quote.alternativeTimingReasons.arrivalOutsideRequestedWindow === true, "Alternative-date/time quote did not distinguish the customer's frozen request from the exact proposed visit.");
  ...
  assert(capacityAwareMatching.ok && firstCapacityAwareSlot?.availableDate === visitDay && firstCapacityAwareSlot.suggestedStartTime === "13:00" && firstCapacityAwareSlot.suggestedEndTime === "15:00" && firstCapacityAwareSlot.capacityAdjusted === true && firstCapacityAwareSlot.heldIntervalsAvoided === 1, "Matching did not move the next suggested visit around an existing live capacity hold.");
  ...
  const changedReplacement = await fetch(`${base}/api/admin/proposals`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: overlapRequestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDayPlus2, proposedStartTime: "09:00", estimatedHours: 2, customerRate: 31, cleanerRate: 18, otherCosts: 0 })
  });
  ...
  const reverifiedAvailability = await fetch(`${base}/api/admin/cleaner-availability`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ cleanerId: cleanerBody.reference, availableDate: visitDay, startTime: "08:00", endTime: "15:00", confirmationNote: "Test-only availability reverified before cleaner acceptance." }) });
  ...
  assert(finalisationQueueBody.launchFunnel?.dispatchReadyCleaners >= 1 && finalisationQueueBody.launchFunnel?.supplyStages?.find((stage) => stage.key === "available")?.count >= 1 && finalisationQueueBody.launchFunnel?.supplyBottleneck?.key === "ready" && finalisationQueueBody.launchFunnel?.stages?.find((stage) => stage.key === "offers")?.count >= 1 && finalisationQueueBody.launchFunnel?.stages?.find((stage) => stage.key === "accepted")?.count >= 1 && finalisationQueueBody.launchFunnel?.stages?.find((stage) => stage.key === "bookings")?.count === 0 && finalisationQueueBody.launchFunnel?.bottleneck?.key === "booking-confirmation" && finalisationQueueBody.launchFunnel?.parallelAction === null, "Ready first-booking funnel did not identify dispatch-ready supply and final booking confirmation or stop presenting a parallel launch-gate action.");
  ...
  assert(confirmedBookingBody.booking?.plannedLabourOnCosts === 3.6 && confirmedBookingBody.booking?.profitTargetVersion === 1 && confirmedBookingBody.booking?.targetMarginPercent === 25 && confirmedBookingBody.booking?.targetContribution === 18, "Confirmed booking did not freeze the proposal's labour on-costs and both founder profit targets.");
  ...
  assert(bookingHandoffs.ok && bookingHandoffsBody.handoffReady === true && bookingHandoffsBody.warnings.length === 0 && bookingHandoffsBody.customer.recipient.email === "customer@example.com" && bookingHandoffsBody.cleaner.recipient.email === "cleaner@example.com" && bookingHandoffsBody.customer.privateUrl === "https://tideway.example.com/landlord/dashboard" && bookingHandoffsBody.cleaner.privateUrl === "https://tideway.example.com/cleaner/dashboard" && bookingHandoffsBody.customer.body.includes("Requested frequency: Weekly") && bookingHandoffsBody.customer.body.includes("UK local time") && bookingHandoffsBody.customer.body.includes("one dated visit only") && bookingHandoffsBody.cleaner.body.includes("Requested frequency: Weekly") && bookingHandoffsBody.cleaner.body.includes("UK local time") && bookingHandoffsBody.cleaner.body.includes("one dated assignment only"), "Confirmed-booking handoffs did not pair the role dashboards, frequency, UK booking clock and one-visit boundary with their intended recipients.");
  ...
  await fetch(`${base}/api/admin/config`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(completeConfig) });
  const postBookingProposal = await fetch(`${base}/api/admin/proposals`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: requestBody.reference, cleanerId: cleanerBody.reference, proposedDate: visitDay, proposedStartTime: "09:00", estimatedHours: 4, customerRate: 30, cleanerRate: 18, otherCosts: 0 }) });
  ...
  assert(bookedCapacityMatching.ok && firstBookedCapacitySlot?.availableDate === visitDay && firstBookedCapacitySlot.suggestedStartTime === "13:00" && firstBookedCapacitySlot.suggestedEndTime === "15:00" && firstBookedCapacitySlot.capacityAdjusted === true, "Confirmed booking capacity was not removed from later matching suggestions.");
  ...
  assert(bookedTracker.ok && bookedTrackerBody.current.stage === "booking-confirmed" && bookedTrackerBody.request.frequency === "Weekly" && bookedTrackerBody.request.siteSize === "Reviewed room scan: Kitchen" && bookedTrackerBody.links.bookingToken === confirmedBookingBody.booking.customerViewToken && bookedTrackerBody.visit.reference === confirmedBookingBody.booking.id && !JSON.stringify(bookedTrackerBody).includes("cleanerViewToken"), "Customer tracker did not retain the scan-derived scope and requested frequency or link the confirmed customer booking safely.");
  ...
  assert(customerPack.ok && customerPackBody.booking.audience === "customer" && customerPackBody.booking.frequency === "Weekly" && customerPackBody.booking.siteSize === "Reviewed room scan: Kitchen" && customerPackBody.booking.jobTiming?.arrivalCanBeRecorded === false && customerPackBody.booking.jobTiming?.completionCanBeRecorded === false && customerPackBody.booking.serviceAddress === "10 Clean Street, Westminster, London" && customerPackBody.booking.customerTotal === 120 && customerPackBody.booking.checklist.includes("Kitchen: Clean inside the oven") && customerPackBody.booking.confirmedExtras?.[0]?.code === "oven-interior" && customerPackBody.booking.roomPhotos?.[0]?.note === "Worktops, floor and inside oven need attention" && customerPackBody.booking.roomPhotos?.[1]?.kind === "video", "Customer booking pack omitted scan-derived scope, requested frequency, job-day timing gate, confirmed address, price, checklist, confirmed extra or protected photo/video details.");
  ...
  assert(cleanerPack.ok && cleanerPackBody.booking.audience === "cleaner" && cleanerPackBody.booking.frequency === "Weekly" && cleanerPackBody.booking.siteSize === "Reviewed room scan: Kitchen" && cleanerPackBody.booking.jobTiming?.arrivalCanBeRecorded === false && cleanerPackBody.booking.jobTiming?.arrivalOpensAt && cleanerPackBody.booking.serviceAddress === "10 Clean Street, Westminster, London" && cleanerPackBody.booking.accessContactName === "Site Manager" && cleanerPackBody.booking.accessContactPhone === "07123456781" && cleanerPackBody.booking.cleanerPay === 72 && cleanerPackBody.booking.confirmedExtras?.[0]?.label === "Inside oven cleaning" && cleanerPackBody.booking.roomPhotos?.[0]?.area === "Kitchen" && cleanerPackBody.booking.roomPhotos?.[1]?.mimeType === "video/webm", "Cleaner assignment pack omitted scan-derived scope, requested frequency, job-day timing gate, confirmed visit, access, pay, confirmed extra or room-scan media details.");
  ...
  assert(customerPackWithRequestBody.booking.proposedDate === visitDay && customerPackWithRequestBody.booking.proposedStartTime === "09:00", "A reschedule request silently changed the confirmed booking.");
  ...
  const proposalDataPath = path.join(testDataDir, "match-proposals.ndjson");
  const storedProposalRecords = (await readFile(proposalDataPath, "utf8")).trim().split(/\r?\n/).map((line) => JSON.parse(line));
  const currentStoredProposal = storedProposalRecords.find((record) => record.id === proposalBody.proposal.id);
  const legacyStoredProposal = JSON.parse(JSON.stringify(currentStoredProposal));
  legacyStoredProposal.id = "PRO-LEGACY01";
  delete legacyStoredProposal.economicsModelVersion;
  delete legacyStoredProposal.travelDistanceKm;
  delete legacyStoredProposal.costAssumptions.travelCostPerKm;
  delete legacyStoredProposal.costAssumptions.travelDistanceMultiplier;
  currentStoredProposal.costAssumptions.travelCostPerKm = 0.45;
  currentStoredProposal.travelCosts = 10;
  currentStoredProposal.nonCleanerCosts = 27;
  currentStoredProposal.contribution = 21;
  currentStoredProposal.marginPercent = 17.5;
  await writeFile(proposalDataPath, `${[...storedProposalRecords, legacyStoredProposal].map((record) => JSON.stringify(record)).join("\n")}\n`, "utf8");
  const healthyIntegrity = await fetch(`${base}/api/admin/data-integrity`);
  const healthyIntegrityBody = await healthyIntegrity.json();
  assert(healthyIntegrity.ok && healthyIntegrityBody.audit?.healthy === true && healthyIntegrityBody.audit.issueCount === 0 && healthyIntegrityBody.audit.files.length === 21, "The authenticated private-data integrity desk did not verify the complete healthy record set, including legacy flat-travel proposals and append-only customer timing changes.");

  const tamperedProposalRecords = (await readFile(proposalDataPath, "utf8")).trim().split(/\r?\n/).map((line) => JSON.parse(line));
  const tamperedDistanceProposal = tamperedProposalRecords.find((record) => record.id === proposalBody.proposal.id);
  tamperedDistanceProposal.travelDistanceKm += 1;
  await writeFile(proposalDataPath, `${tamperedProposalRecords.map((record) => JSON.stringify(record)).join("\n")}\n`, "utf8");
  ...
  await appendFile(path.join(testDataDir, "request-schedule-updates.ndjson"), `${JSON.stringify({ id: "RSC-CORRUPT1", requestId: requestBody.reference, previousDate: "2026-01-01", previousTimeWindow: "Flexible", preferredDate: visitDayPlus2, preferredTimeWindow: "Flexible", reason: "Corrupted disconnected timing history for the integrity test.", source: "customer-private-schedule-change", createdAt: new Date().toISOString() })}\n`, "utf8");
