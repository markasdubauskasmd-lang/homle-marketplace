...
try {
  ...
  assert(terms.ok && termsText.includes("Pilot terms") && termsText.includes("private customer and cleaner trackers") && termsText.includes("does not guarantee work") && termsText.includes("planning preference only") && termsText.includes("No recurring schedule"), "Terms page failed, implied that tracker status guarantees work, or omitted the one-visit frequency boundary.");
  ...
  assert(quotePage.ok && quotePageText.includes("Private customer review") && quotePageText.includes("This is a replacement quote") && quotePageText.includes("A fresh decision is required.") && quotePageText.includes("data-frequency") && quotePageText.includes("One-visit rule"), "Private customer quote, frequency scope or replacement-review page failed.");
  const quoteAsset = await fetch(`${base}/quote.js?v=smoke-test`);
  const quoteAssetText = await quoteAsset.text();
  assert(quoteAsset.ok && quoteAssetText.includes("previousCustomerAccepted") && quoteAssetText.includes("[data-frequency]"), "Customer replacement-quote explanation or requested-frequency rendering asset failed.");
  const opportunityPage = await fetch(`${base}/opportunity`);
  const opportunityPageText = await opportunityPage.text();
  assert(opportunityPage.ok && opportunityPageText.includes("Private cleaner review") && opportunityPageText.includes("data-frequency") && opportunityPageText.includes("One-visit rule"), "Private cleaner opportunity page failed or omitted its frequency and one-visit scope.");
  const opportunityAsset = await fetch(`${base}/opportunity.js?v=smoke-test`);
  assert(opportunityAsset.ok && (await opportunityAsset.text()).includes("[data-frequency]"), "Cleaner opportunity did not render the requested frequency.");
  ...
  assert(customerBookingPage.ok && cleanerAssignmentPage.ok && customerBookingPageText.includes("Protected visit details") && customerBookingPageText.includes("Report a cleaning quality issue") && customerBookingPageText.includes("data-frequency") && customerBookingPageText.includes("One-visit rule") && (await cleanerAssignmentPage.text()).includes("Protected visit details"), "Private confirmed-booking pages, frequency boundary or cleaning-quality reporting route failed.");
  ...
  assert(preSendDispatchPack.ok && preSendDispatchPackBody.sendAllowed === true && preSendDispatchPackBody.handoffReady === false && preSendDispatchPackBody.customer.handoffReady === false && preSendDispatchPackBody.cleaner.handoffReady === false && preSendDispatchPackBody.customer.privateUrl === `https://tideway.example.com/quote#${proposalBody.proposal.reviewToken}` && preSendDispatchPackBody.cleaner.privateUrl === `https://tideway.example.com/opportunity#${proposalBody.proposal.cleanerReviewToken}` && preSendDispatchPackBody.customer.body.includes("Requested frequency: Weekly") && preSendDispatchPackBody.customer.body.includes("one dated visit") && preSendDispatchPackBody.cleaner.body.includes("Requested frequency: Weekly") && preSendDispatchPackBody.cleaner.body.includes("one dated visit") && !preSendDispatchPackBody.customer.body.includes(proposalBody.proposal.reviewToken) && !preSendDispatchPackBody.cleaner.body.includes(proposalBody.proposal.cleanerReviewToken), "Review-stage drafts omitted the verified origin, requested frequency or one-visit boundary, or exposed a sendable private handoff before the offer was recorded as sent.");
  ...
  assert(bookingHandoffs.ok && bookingHandoffsBody.handoffReady === true && bookingHandoffsBody.warnings.length === 0 && bookingHandoffsBody.customer.recipient.email === "customer@example.com" && bookingHandoffsBody.cleaner.recipient.email === "cleaner@example.com" && bookingHandoffsBody.customer.privateUrl === `https://tideway.example.com/booking-confirmation#${confirmedBookingBody.booking.customerViewToken}` && bookingHandoffsBody.cleaner.privateUrl === `https://tideway.example.com/assignment#${confirmedBookingBody.booking.cleanerViewToken}` && bookingHandoffsBody.customer.body.includes("Requested frequency: Weekly") && bookingHandoffsBody.customer.body.includes("one dated visit only") && bookingHandoffsBody.cleaner.body.includes("Requested frequency: Weekly") && bookingHandoffsBody.cleaner.body.includes("one dated assignment only"), "Confirmed-booking handoffs did not pair the frozen public links, frequency and one-visit boundary with their intended recipients.");
  ...
  assert(customerPack.ok && customerPackBody.booking.audience === "customer" && customerPackBody.booking.frequency === "Weekly" && customerPackBody.booking.serviceAddress === "10 Clean Street, Westminster, London" && customerPackBody.booking.customerTotal === 120 && customerPackBody.booking.checklist.includes("Kitchen: Clean inside the oven") && customerPackBody.booking.confirmedExtras?.[0]?.code === "oven-interior" && customerPackBody.booking.roomPhotos?.[0]?.note === "Worktops, floor and inside oven need attention" && customerPackBody.booking.roomPhotos?.[1]?.kind === "video", "Customer booking pack omitted requested frequency, confirmed address, price, checklist, confirmed extra or protected photo/video details.");
  ...
  assert(degradedIntegrity.ok && degradedIntegrityBody.audit?.healthy === false && degradedIntegrityBody.audit.issueCount >= 2 && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "malformed-record" && issue.file === "job-events.ndjson") && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "orphaned-reference" && issue.reference === "BKG-NOT-REAL"), "The non-destructive integrity audit missed a truncated line or cross-record orphan.");
