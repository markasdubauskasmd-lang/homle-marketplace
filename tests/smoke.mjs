...
try {
  ...
  assert(briefPage.ok && briefPageText.includes("Request details carried over.") && briefPageText.includes("Open this scan from your private request tracker") && briefPageText.includes("data-request-email-label") && briefPageText.includes('id="photo-count">0/10') && briefPageText.includes('id="brief-camera" type="file" accept="image/*" capture="environment"') && briefPageText.includes("Take a room photo") && briefPageText.includes("Choose existing photos or videos") && galleryInputTag.includes("multiple") && !galleryInputTag.includes("capture=") && briefPageText.includes("the floor needs mopping") && briefPageText.includes("turns conditions into direct tasks") && briefPageText.includes("Say “do not”, “no need” or “skip”") && briefPageText.includes("Excluded work stays visible to the Cleaner but is not treated as requested extra cleaning time") && briefPageText.includes('aria-label="Cleaner handoff preview"') && briefPageText.includes("no more than two videos of 30 seconds and 15 MB each") && briefPageText.includes("complete upload is capped at 20 MB") && briefPageText.includes("Checking room scan") && briefPageText.includes("summarise again so the cleaner receives the latest scope") && briefPageText.includes("Extra time may be needed") && briefPageText.includes("require this confirmation again"), "Direct mobile camera capture, natural speech-to-task and exclusion guidance, grouped Cleaner handoff preview, separate gallery/video selection, bounded whole-property limits, checklist-freshness guidance, live readiness panel, private tracker handoff, customer-facing scope warning or change-sensitive scope confirmation failed.");
