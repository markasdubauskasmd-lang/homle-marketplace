...
try {
  ...
  const validBrief = await fetch(`${base}/api/job-briefs`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ requestId: requestBody.reference, email: "customer@example.com", transcript: "Please wipe every kitchen worktop. Also mop the kitchen floor.", checklist: ["Kitchen: Wipe every kitchen worktop", "Kitchen: Mop the kitchen floor"], photos: [{ area: "Kitchen", note: "Worktops and floor need attention", dataUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9Zs7sAAAAASUVORK5CYII=" }], consent: true, sharePhotosWithSelectedCleaner: true })
  });
  const briefBody = await validBrief.json();
  assert(validBrief.status === 201 && briefBody.reference.startsWith("BRF-") && briefBody.checklist.length === 2 && briefBody.customerStatusToken === requestBody.customerStatusToken && briefBody.cleanerPhotoSharingConsent === true, "Valid photo job brief failed, omitted checklist bullets, lost the private tracker handoff or failed to record selected-cleaner photo permission.");
  ...
  assert(sentOpportunity.ok && sentOpportunityBody.opportunity.decisionAllowed === true && Date.parse(sentOpportunityBody.opportunity.offerExpiresAt) >= sentAtLowerBound + 12 * 60 * 60 * 1000 - 2000 && sentOpportunityBody.opportunity.expired === false && sentOpportunityBody.opportunity.photoAccessAllowed === true && sentOpportunityBody.opportunity.roomPhotos?.[0]?.note === "Worktops and floor need attention" && !JSON.stringify(sentOpportunityBody).includes("storedPath"), "Sent cleaner opportunity did not expose its frozen deadline and authorised visual scope safely.");
