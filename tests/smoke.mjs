...
try {
  ...
  assert(validProposal.status === 201 && proposalBody.proposal.economicsModelVersion === 2 && proposalBody.proposal.travelDistanceKm === 10 && proposalBody.proposal.costAssumptions.travelDistanceMultiplier === 2 && proposalBody.proposal.contribution === 30 && proposalBody.proposal.labourOnCosts === 3.6 && proposalBody.proposal.paymentFees === 1.2 && proposalBody.proposal.riskContingency === 1.2 && proposalBody.proposal.nonCleanerCosts === 18 && proposalBody.proposal.proposedEndTime === "13:00" && /^[A-Za-z0-9_-]{32}$/.test(proposalBody.proposal.reviewToken) && /^[A-Za-z0-9_-]{32}$/.test(proposalBody.proposal.cleanerReviewToken), "Valid distance-versioned draft proposal failed, omitted its frozen journey/cost model, calculated incorrectly, omitted its schedule or omitted a private review token.");
  ...
  assert(confirmedBookingBody.booking?.plannedLabourOnCosts === 3.6 && confirmedBookingBody.booking?.targetMarginPercent === 25 && confirmedBookingBody.booking?.targetContribution === 18, "Confirmed booking did not freeze the proposal's labour on-costs and both founder profit targets.");
