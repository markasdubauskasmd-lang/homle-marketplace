...
try {
  ...
  assert(cleanerTrainingText.includes("data-training-lessons") && cleanerTrainingText.includes("data-exam-host") && (cleanerTrainingText.match(/<video /g)||[]).length === 4 && cleanerTrainingText.includes("04-working-with-homlle-compatible.mp4") && cleanerTrainingText.includes("Download the written guide") && !cleanerTrainingText.includes("Sadie Fletcher"), "The onboarding videos, saved exam or downloadable guide are missing.");
  assert(cleanerContractsText.includes("data-contract-list") && cleanerContractsText.includes("Drafts cannot be signed") && !cleanerContractsText.includes("86.157.44.102"), "Contracts must preserve draft restrictions and avoid fabricated signatures.");
  ...
  assert(adminCasesText.includes("Marketplace trust and safety") && adminCasesText.includes("data-admin-cases-workspace hidden") && adminCasesText.includes("A resolution may return money to the customer who paid, and never charges anyone or pays a Cleaner"), "The Administrator case page lost its guarded controls.");
  ...
  const [landingCss, landingScript, retiredLandingCss, retiredLandingScript, retiredContentAddressedScript, compactLogo128, compactLogo192, originalLogo, cleanHero960, dirtyHero960, originalHero, scanAngleWebp, scanAnglePng, supportingWideWebp, supportingNarrowWebp, supportingJpeg, optimizedClip, retiredClip] = await Promise.all([
    fetch(`${base}/landing-1571bff0.css`),
    fetch(`${base}/landing-a97a6ca6.js`),
