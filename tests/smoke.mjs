...
try {
  ...
  assert(customerBookingPage.ok && cleanerAssignmentPage.ok && customerBookingPageText.includes("Protected visit details") && customerBookingPageText.includes("Report a cleaning quality issue") && (await cleanerAssignmentPage.text()).includes("Protected visit details"), "Private confirmed-booking pages or cleaning-quality reporting route failed.");
  ...
  assert(bookingAudit.ok && bookingAuditBody.automatedReady === true && bookingAuditBody.checks.customerScopeConfirmed === true && bookingAuditBody.checks.cleanerTravelCovered === true && bookingAuditBody.checks.publicOriginFrozen === true && bookingAuditBody.publicSiteUrl === "https://tideway.example.com" && bookingAuditBody.customerDecision?.status === "accepted" && Object.values(bookingAuditBody.checks).every(Boolean), "Two-sided accepted proposal did not retain customer scope confirmation, frozen public origin, accepted customer decision and cleaner travel coverage or pass the automated booking audit.");
