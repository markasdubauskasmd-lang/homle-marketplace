...
try {
  ...
  assert(loginPageText.includes("data-account-ready-dashboard") && loginPageText.includes("data-account-avatar") && loginPageText.includes("data-account-side-title") && authEntryText.includes('accountReadyDashboard.href = cleaner ? "/cleaner/dashboard" : "/landlord/dashboard"') && authEntryText.includes("renderAccountAvatar(result.account)") && authEntryText.includes("pilotActions.hidden = true") && authEntryText.includes('setAccountSide(cleaner ? "cleaner" : "landlord")') && authEntryText.includes("Landlord properties and booking controls never appear in the Cleaner dashboard") && authEntryText.includes("Cleaner profile, availability and earnings controls never appear in the Landlord dashboard"), "The signed-in account handoff still mixes Landlord and Cleaner actions, lacks a profile photo/initials identity, or cannot open the selected role dashboard directly.");
