...
try {
  ...
  assert(requestStatusPage.ok && requestStatusPageText.includes("Private request tracker") && requestStatusPageText.includes("data-frequency") && requestStatusPageText.includes("Close this unbooked request") && requestStatusPageText.includes("does not delete the audit record or cancel a confirmed visit"), "Private customer request tracker failed or omitted frequency and the bounded pre-booking withdrawal control.");
  ...
  assert(requestStatusAsset.ok && requestStatusAssetText.includes('/brief?reference=${encodeURIComponent(result.request.reference)}#${token}') && requestStatusAssetText.includes("/api/request-withdrawal") && requestStatusAssetText.includes('"X-Request-Token": token') && requestStatusAssetText.includes("result.withdrawal?.allowed"), "The private customer tracker did not preserve its room-scan handoff or expose the token-authorised bounded withdrawal control.");
