...
try {
  ...
  assert(home.ok && homeText.includes("Book a clean with every room clearly scoped") && homeText.includes("Pilot preparation · coverage not yet confirmed") && homeText.includes("verify coverage, cleaner availability and a clear quote before any booking") && homeText.includes("area checked before quote") && !homeText.includes("London pilot") && !homeText.includes("Rental property · London") && !homeText.includes("applications open") && homeText.includes('data-guided-kind="customer"') && homeText.includes('data-guided-kind="cleaner"') && homeText.includes("data-cleaner-status-link") && homeText.includes("Application received. Opening your private tracker") && guidedStepCount === 6 && homeText.includes("What needs cleaning?") && homeText.includes("When and how can the cleaner enter?") && homeText.includes("Where should we send the next step?") && homeText.includes("How can Tideway reach you?") && homeText.includes("What cleaning work suits you?") && homeText.includes("When could you first work?") && homeText.includes('name="firstAvailableDate"') && homeText.includes('name="firstAvailableStartTime"') && homeText.includes('name="firstAvailableEndTime"') && homeText.includes('href="/request">Book a clean</a>') && homeText.includes('href="/join">Join as a cleaner</a>') && !homeText.includes('href="/brief">Scan my rooms') && homeText.includes("postcode districts or areas") && homeText.includes("SW1A, SW4, or broader areas SW, SE"), "Truthful pilot-preparation copy, Uber-simple focused entry routes, three-stage journeys, continuous private cleaner-tracker handoff, actionable first availability, guarded scan entry or structured cleaner travel-area guidance failed.");
  ...
  assert(briefPage.ok && briefPageText.includes("Request details carried over.") && briefPageText.includes("Open this scan from your private request tracker") && briefPageText.includes("data-request-email-label") && briefPageText.includes('id="photo-count">0/10') && briefPageText.includes('id="brief-camera" type="file" accept="image/*" capture="environment"') && briefPageText.includes("Take a room photo") && briefPageText.includes("Choose existing photos or videos") && galleryInputTag.includes("multiple") && !galleryInputTag.includes("capture=") && briefPageText.includes("the floor needs mopping") && briefPageText.includes("turns conditions into direct tasks") && briefPageText.includes("Say “do not”, “no need” or “skip”") && briefPageText.includes("Excluded work stays visible to the Cleaner but is not treated as requested extra cleaning time") && briefPageText.includes("no more than two videos of 30 seconds and 15 MB each") && briefPageText.includes("complete upload is capped at 20 MB") && briefPageText.includes("Checking room scan") && briefPageText.includes("summarise again so the cleaner receives the latest scope") && briefPageText.includes("Extra time may be needed") && briefPageText.includes("require this confirmation again"), "Direct mobile camera capture, natural speech-to-task and exclusion guidance, separate gallery/video selection, bounded whole-property limits, checklist-freshness guidance, live readiness panel, private tracker handoff, customer-facing scope warning or change-sensitive scope confirmation failed.");
  ...
  assert(requestStatusAsset.ok && requestStatusAssetText.includes('/brief?reference=${encodeURIComponent(result.request.reference)}#${token}') && requestStatusAssetText.includes("/api/request-withdrawal") && requestStatusAssetText.includes('"X-Request-Token": token') && requestStatusAssetText.includes("result.withdrawal?.allowed") && requestStatusAssetText.includes('timeZone: "Europe/London"') && requestStatusAssetText.includes("result.request.preferredTimeWindow"), "The private customer tracker did not preserve its room-scan handoff, render UK-local requested timing or expose the token-authorised bounded withdrawal control.");
  ...
  assert(quoteAsset.ok && quoteAssetText.includes("previousCustomerAccepted") && quoteAssetText.includes("requestClosed") && quoteAssetText.includes("[data-frequency]") && quoteAssetText.includes('timeZone: "Europe/London"'), "Customer replacement-quote explanation, closed-request gate, requested-frequency rendering or UK-local deadline display failed.");
  ...
  assert(adminAssetText.includes("funnel.parallelAction") && adminAssetText.includes("launch-parallel-action") && adminAssetText.includes('["scan", "supply", "profit"]') && adminAssetText.includes("brief.open = true") && adminAssetText.includes("Open scan review"), "Admin assets omitted safe parallel guidance, focused action filters or direct scan-review navigation.");
  ...
  assert(initialRequestStatus.ok && initialRequestStatusBody.current.stage === "room-scan" && initialRequestStatusBody.links.roomScanRequired === true && initialRequestStatusBody.request.reference === requestBody.reference && initialRequestStatusBody.request.frequency === "Weekly" && initialRequestStatusBody.request.preferredDate === "2026-07-20" && initialRequestStatusBody.request.preferredTimeWindow === "Morning (8am–12pm)", "New request tracker did not open at the required room-scan stage with the requested frequency, date and arrival preference.");
  ...
  assert(profilelessCleaner.status === 422 && profilelessCleanerBody.errors?.some((error) => error.includes("Professional introduction")) && profilelessCleanerBody.errors?.some((error) => error.includes("language")), "Cleaner application stored without a complete private professional profile starter.");

  const validCleanerInput = { fullName: "Test Cleaner", email: "cleaner@example.com", phone: "07123456789", postcode: "SE1 7PB", travelAreas: "SW1A and South London", experience: "1–3 years", professionalBio: "I clean rental homes carefully and work through every agreed room task.", languages: "English, Polish", equipmentPlan: "confirm-per-opportunity", availability: "Weekdays", firstAvailableDate: "2026-07-20", firstAvailableStartTime: "08:00", firstAvailableEndTime: "15:00", serviceTurnovers: true, rightToWork: true, consent: true };
  ...
  assert(initialCleanerStatus.ok && initialCleanerStatusBody.current.stage === "application-review" && initialCleanerStatusBody.application.reference === cleanerBody.reference && initialCleanerStatusBody.application.profileStarter?.captured === true && initialCleanerStatusBody.application.profileStarter?.languageCount === 2 && initialCleanerStatusBody.application.firstAvailability?.availableDate === "2026-07-20" && initialCleanerStatusBody.application.firstAvailability?.startTime === "08:00" && initialCleanerStatusBody.application.firstAvailability?.endTime === "15:00" && initialCleanerStatusBody.application.firstAvailability?.status === "unconfirmed" && initialCleanerStatusBody.readiness.profileStarterCaptured === true && initialCleanerStatusBody.readiness.firstAvailabilityCaptured === true && initialCleanerStatusBody.readiness.readyForOpportunities === false && initialCleanerStatusBody.steps.find((step) => step.key === "application")?.state === "complete" && initialCleanerStatusBody.steps.find((step) => step.key === "availability")?.detail.includes("not confirmed for matching"), "New cleaner tracker did not safely acknowledge the profile starter, retain the unconfirmed first window or open at received-but-not-screened status.");
  const initialCleanerTrackerSerialised = JSON.stringify(initialCleanerStatusBody);
  assert(!initialCleanerTrackerSerialised.includes("Test Cleaner") && !initialCleanerTrackerSerialised.includes("cleaner@example.com") && !initialCleanerTrackerSerialised.includes("07123456789") && !initialCleanerTrackerSerialised.includes("SE1 7PB") && !initialCleanerTrackerSerialised.includes("SW1A") && !initialCleanerTrackerSerialised.includes("I clean rental homes") && !initialCleanerTrackerSerialised.includes("Polish") && !initialCleanerTrackerSerialised.includes("cleanerStatusToken"), "Cleaner tracker exposed identity, contact, profile content, travel or authorisation-token data.");
  ...
  const navigableReadinessLabels = [...new Set([...Object.values(initialConfigBody.readiness.missing).flat(), "viable margin and percentage-cost stack"])];
  ...
  const uncoveredCleaner = await fetch(`${base}/api/cleaner-applications`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ fullName: "Outside Area Cleaner", email: "outside@example.com", phone: "07123456782", postcode: "NE1 1AA", travelAreas: "NE1", experience: "1–3 years", professionalBio: "I clean rental homes carefully and confirm every task before starting work.", languages: "English", equipmentPlan: "equipment-and-products-supplied", availability: "Weekdays", firstAvailableDate: "2026-07-20", firstAvailableStartTime: "08:00", firstAvailableEndTime: "15:00", serviceTurnovers: true, rightToWork: true, consent: true })
  });
  ...
  const reviewedBrief = await fetch(`${base}/api/admin/job-briefs/status`, {
    method: "PATCH",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ briefId: briefBody.reference, status: "reviewed", note: "Checklist and private photo support a three-and-a-half-hour scope floor, including the oven interior.", scopeEstimateHours: 3.5, scopeConfidence: "high", scopeSignalConfirmations: ["oven-interior"], visualsReviewed: true, reviewedVisualIds, checklistReviewed: true })
  });
  const reviewedBriefBody = await reviewedBrief.json();
  assert(reviewedBrief.ok && reviewedBriefBody.status === "reviewed" && reviewedBriefBody.scopeEstimateHours === 3.5 && reviewedBriefBody.scopeConfidence === "high" && reviewedBriefBody.scopeSignals?.[0]?.code === "oven-interior" && reviewedBriefBody.scopeSignalConfirmations?.[0] === "oven-interior" && reviewedBriefBody.visualsReviewed === true && reviewedBriefBody.reviewedVisualIds.length === briefBody.photos.length && reviewedBriefBody.perVisualReviewEvidenceConfirmed === true && reviewedBriefBody.checklistReviewed === true && reviewedBriefBody.reviewEvidenceConfirmed === true, "Structured human scan approval, exact per-visual founder evidence or price-sensitive scope confirmation was not recorded.");
  ...
  const reviewedTravelGateBrief = await fetch(`${base}/api/admin/job-briefs/status`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ briefId: travelGateBriefBody.reference, status: "reviewed", note: "Test-only two-hour travel-coverage scope estimate.", scopeEstimateHours: 2, scopeConfidence: "medium", visualsReviewed: true, reviewedVisualIds: travelGateBriefBody.photos.map((photo) => photo.id), checklistReviewed: true }) });
  ...
  const reviewedEveningBrief = await fetch(`${base}/api/admin/job-briefs/status`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ briefId: eveningBriefBody.reference, status: "reviewed", note: "Test-only two-hour evening scope estimate.", scopeEstimateHours: 2, scopeConfidence: "medium", visualsReviewed: true, reviewedVisualIds: eveningBriefBody.photos.map((photo) => photo.id), checklistReviewed: true }) });
  ...
  const reviewedOverlapScan = await fetch(`${base}/api/admin/job-briefs/status`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ briefId: overlapScanBody.reference, status: "reviewed", note: "Test-only two-hour scan estimate.", scopeEstimateHours: 2, scopeConfidence: "medium", visualsReviewed: true, reviewedVisualIds: overlapScanBody.photos.map((photo) => photo.id), checklistReviewed: true }) });
  ...
  assert(degradedIntegrity.ok && degradedIntegrityBody.audit?.healthy === false && degradedIntegrityBody.audit.issueCount >= 5 && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "malformed-record" && issue.file === "job-events.ndjson") && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "orphaned-reference" && issue.reference === "BKG-NOT-REAL") && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "invalid-visual-review-evidence" && issue.reference === briefBody.reference) && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "invalid-financial-record" && issue.file === "job-outcome-adjustments.ndjson" && issue.reference === "ADJ-FINANCIAL-CORRUPT") && degradedIntegrityBody.audit.issues.some((issue) => issue.code === "invalid-financial-config" && issue.file === "business-config.json"), "The non-destructive integrity audit missed malformed JSON, a cross-record orphan, exact visual-review evidence corruption, semantic financial corruption or invalid launch economics.");
