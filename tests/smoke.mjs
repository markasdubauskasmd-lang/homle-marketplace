...
try {
  ...
  assert(cleanerStatusPage.ok && cleanerStatusPageText.includes("Private cleaner tracker") && cleanerStatusPageText.includes("First window supplied") && cleanerStatusPageText.includes("Complete your professional profile") && cleanerStatusPageText.includes("not published by this form") && cleanerStatusPageText.includes("When are you available?") && cleanerStatusPageText.includes("It stays pending until Tideway confirms it") && cleanerStatusPageText.includes("This tracker does not approve, assign or promise work."), "Private cleaner application tracker page failed, omitted the private profile/first-time actions or implied publication or work approval.");
  ...
  assert(initialCleanerStatus.ok && initialCleanerStatusBody.current.stage === "application-review" && initialCleanerStatusBody.application.reference === cleanerBody.reference && initialCleanerStatusBody.application.profileStarter?.captured === true && initialCleanerStatusBody.application.profileStarter?.languageCount === 2 && initialCleanerStatusBody.application.firstAvailability?.availableDate === "2026-07-20" && initialCleanerStatusBody.application.firstAvailability?.startTime === "08:00" && initialCleanerStatusBody.application.firstAvailability?.endTime === "15:00" && initialCleanerStatusBody.application.firstAvailability?.status === "unconfirmed" && initialCleanerStatusBody.readiness.profileStarterCaptured === true && initialCleanerStatusBody.readiness.firstAvailabilityCaptured === true && initialCleanerStatusBody.readiness.readyForOpportunities === false && initialCleanerStatusBody.links.profileStarterSubmissionAllowed === true && initialCleanerStatusBody.steps.find((step) => step.key === "application")?.state === "complete" && initialCleanerStatusBody.steps.find((step) => step.key === "profile")?.state === "complete" && initialCleanerStatusBody.steps.find((step) => step.key === "availability")?.detail.includes("not confirmed for matching"), "New cleaner tracker did not safely acknowledge the editable private profile starter, retain the unconfirmed first window or open at received-but-not-screened status.");
  ...
  assert(approvedCleanerStatus.ok && approvedCleanerStatusBody.current.stage === "availability" && approvedCleanerStatusBody.readiness.approvalRecorded === true && approvedCleanerStatusBody.readiness.confirmedAvailabilityWindows === 0 && approvedCleanerStatusBody.readiness.pendingAvailabilityWindows === 0 && approvedCleanerStatusBody.links.profileStarterSubmissionAllowed === false && approvedCleanerStatusBody.links.availabilitySubmissionAllowed === true && approvedCleanerStatusBody.steps.find((step) => step.key === "availability")?.state === "current", "Approved cleaner tracker skipped the exact-availability requirement, left screened profile editing open, hid its safe submission action or implied work readiness early.");
  ...
  assert(healthyIntegrity.ok && healthyIntegrityBody.audit?.healthy === true && healthyIntegrityBody.audit.issueCount === 0 && healthyIntegrityBody.audit.files.length === 20, "The authenticated private-data integrity desk did not verify the complete healthy record set.");
