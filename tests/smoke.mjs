...
try {
  ...
  assert(lanPreviewHealth.ok && lanPreviewHome.ok && (await lanPreviewHome.text()).includes("We clean it"), "Optional same-process Wi-Fi preview listener did not serve the Homle site.");
  ...
  assert(home.ok && homeText.includes("We clean it") && homeText.includes("Your home, reset") && homeText.includes("Come home") && homeText.includes("Small details.") && homeText.includes("Homle is accepting guided pilot requests") && homeText.includes('href="/request" data-book-entry>Request a clean</a>') && homeText.includes('href="/join" data-cleaner-entry>Work as a cleaner</a>') && homeText.includes('href="/login" data-account-entry hidden>Sign in</a>') && homeText.includes('href="/request" data-directory-entry>Find a cleaner</a>') && !homeText.includes('data-guided-kind="customer"') && !homeText.includes('data-guided-kind="cleaner"') && !homeText.includes('/api/cleaning-requests') && !homeText.includes('/app.js') && homeText.length < requestEntryPageText.length, "The public homepage did not preserve truthful role-aware fallbacks or still shipped the long pilot forms and their application script.");
  ...
  assert(cleanerDirectoryPage.ok && cleanerEditorPage.status === 404 && cleanerPersonalDetailsPage.ok && cleanerBusinessDetailsPage.ok && cleanerIdentityVerificationPage.ok && cleanerBackgroundChecksPage.ok && cleanerWorkAreasPage.ok && cleanerPayoutPage.ok && landlordDashboardPage.ok && adminCasesPage.ok && adminPaymentsPage.ok && cleanerPersonalDetailsPage.headers.get("cache-control") === "no-store" && cleanerBusinessDetailsPage.headers.get("cache-control") === "no-store" && cleanerIdentityVerificationPage.headers.get("cache-control") === "no-store" && cleanerBackgroundChecksPage.headers.get("cache-control") === "no-store" && cleanerWorkAreasPage.headers.get("cache-control") === "no-store" && cleanerPayoutPage.headers.get("cache-control") === "no-store" && adminCasesPage.headers.get("cache-control") === "no-store" && adminPaymentsPage.headers.get("cache-control") === "no-store" && homeText.includes('href="/request" data-directory-entry>Find a cleaner</a>') && homeText.includes('href="/join" data-cleaner-entry>Work as a cleaner</a>') && cleanerDirectoryText.includes("Find the right Cleaner for the job") && cleanerDirectoryText.includes("data-directory-state") && cleanerPersonalDetailsText.includes("Personal details") && cleanerPersonalDetailsText.includes("data-personal-form") && cleanerPersonalDetailsText.includes("Emergency contact") && cleanerBusinessDetailsText.includes("Business details") && cleanerBusinessDetailsText.includes("data-business-form") && cleanerBusinessDetailsText.includes("Solo cleaner") && cleanerIdentityVerificationText.includes("Identity verification") && cleanerIdentityVerificationText.includes("data-identity-form") && cleanerIdentityVerificationText.includes("Passport photo page") && cleanerBackgroundChecksText.includes("Background checks (DBS)") && cleanerBackgroundChecksText.includes("data-background-form") && cleanerBackgroundChecksText.includes("DBS certificate") && cleanerWorkAreasText.includes("Work areas") && cleanerWorkAreasText.includes("data-work-form") && cleanerWorkAreasText.includes("data-work-map-markers") && cleanerPayoutText.includes("Get paid without sharing bank details with Homle") && cleanerPayoutText.includes("connect.stripe.com") && landlordDashboardText.includes("Checking secure Landlord access") && landlordDashboardText.includes("data-landlord-workspace hidden") && landlordDashboardText.includes("Save private draft") && landlordDashboardText.includes("Thank you. Your cleaning request is ready for matching.") && landlordDashboardText.includes("No payment was taken here.") && adminCasesText.includes("Marketplace trust and safety") && adminCasesText.includes("data-admin-cases-workspace hidden") && adminCasesText.includes("This screen never refunds, charges or pays anyone") && adminPaymentsText.includes("Administrator · test payments only") && adminPaymentsText.includes("data-admin-payments-workspace hidden") && adminPaymentsText.includes("Every button contacts the configured test payment provider"), "The real Cleaner/Landlord/Administrator routes and Cleaner registration forms, settlement handoff, payout handoff, homepage entry point, removed profile-builder boundary or fail-closed controls are not served by the pilot runtime.");
  ...
  const briefResponseBodyIndex = briefScriptText.indexOf("result = await response.json()");
  const briefTimeoutClearIndex = briefScriptText.indexOf("clearTimeout(requestTimer)", briefResponseBodyIndex);
  assert(briefResponseBodyIndex >= 0 && briefTimeoutClearIndex > briefResponseBodyIndex, "The room-scan timeout stopped before the response body finished, allowing a partial mobile response to leave the save control spinning indefinitely.");
  assert(briefScriptText.includes("videoCameraInput.disabled = disabled") && briefScriptText.includes('videoCameraInput.addEventListener("change"') && briefScriptText.includes("videoCameraInput.value = \"\"") && briefScriptText.includes("await addSelectedVisuals(selected)"), "Direct room-video capture did not share the room gate, reset behavior and existing duration/size validation path.");
  assert(briefScriptText.includes("function setScanInteractionLocked") && briefScriptText.includes("setScanInteractionLocked(true)") && briefScriptText.includes("if (!submissionComplete) setScanInteractionLocked(false)") && briefScriptText.includes('form.setAttribute("aria-busy", locked ? "true" : "false")'), "Room capture remained mutable while its private photo/video payload was being prepared or did not expose an accessible busy state.");
  assert(briefScriptText.includes("Speech capture was still running. Review the final transcript") && briefScriptText.includes("recognition?.stop()") && briefScriptText.includes("submissionComplete || listening || !browserOnline") && briefScriptText.includes("recognition.onstart") && briefScriptText.includes("renderReadiness()"), "The room scan could appear ready or begin encoding while live speech was still changing its scope.");
  const briefSpeechErrorStart = briefScriptText.indexOf("recognition.onerror = (event)");
  const briefSpeechErrorEnd = briefScriptText.indexOf("recognition.onresult", briefSpeechErrorStart);
  const briefSpeechErrorFlow = briefScriptText.slice(briefSpeechErrorStart, briefSpeechErrorEnd);
  assert(briefSpeechErrorStart >= 0 && briefSpeechErrorEnd > briefSpeechErrorStart && briefSpeechErrorFlow.includes("listening = false") && briefSpeechErrorFlow.includes('voiceButton.textContent = "Start speaking"') && briefSpeechErrorFlow.includes("voiceErrorMessage") && briefSpeechErrorFlow.includes("renderReadiness()"), "A mobile speech-recognition error can leave the room scan stuck in listening mode with its save action disabled.");
  const briefSpeechStartFlow = briefScriptText.slice(briefScriptText.indexOf('voiceButton.addEventListener("click"'), briefScriptText.indexOf("} else {\n  voiceButton.disabled", briefScriptText.indexOf('voiceButton.addEventListener("click"')));
  assert(briefSpeechStartFlow.includes("try {") && briefSpeechStartFlow.includes("catch {") && briefSpeechStartFlow.indexOf("recognition.start()") < briefSpeechStartFlow.indexOf("appendCurrentRoomMarker()") && briefSpeechStartFlow.includes("your room notes were not changed") && briefSpeechStartFlow.includes("renderReadiness()"), "A rapid second speech tap can throw an unhandled browser error or append a false room marker before recognition actually starts.");
  ...
  const publicResponseBodyIndex = publicFormAssetText.indexOf("result = await response.json()");
  const publicTimeoutClearIndex = publicFormAssetText.indexOf("clearTimeout(requestTimer)", publicResponseBodyIndex);
  assert(publicResponseBodyIndex >= 0 && publicTimeoutClearIndex > publicResponseBodyIndex, "The public request timeout stopped before the response body finished, allowing a partial response to leave submission spinning indefinitely.");
  ...
  assert(accessSafetyAsset.ok && accessSafetyAssetText.includes("containsSensitiveAccessDetails") && accessSafetyAssetText.includes("only after a booking is accepted") && requestEntryPageText.includes("Private access instructions come only after a booking is accepted") && requestEntryPageText.includes('name="accessNotes" value="Confirm privately after booking"') && (requestEntryPageText.match(/data-access-detail-safe/g) || []).length === 2 && publicFormAssetText.includes('querySelectorAll("[data-access-detail-safe]")'), "The shorter public request did not explain or enforce the pre-booking access-secret boundary across its remaining free-text fields.");
