...
assert(dashboardScript.includes('/api/marketplace/landlord/support-requests?limit=25&offset=0') && dashboardScript.includes('activeBookingChangeRequestFor(supportRequests, booking.bookingId)') && dashboardScript.includes('"View change request"') && dashboardScript.includes('Cleaner commitment and payment remain unchanged'), "The Landlord dashboard does not reconcile an open booking-change request with its confirmed booking card.");
