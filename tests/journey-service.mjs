...
const ready = await service.getJourneyReadiness(cleaner, bookingId);
assert(ready.bookingId === bookingId && ready.canStartJourney === true && calls[0].kind === "context", "Cleaner journey readiness did not use the actor-bound current-payment preflight.");
calls.length = 0;
...
assert((await unpaidService.getJourneyReadiness(cleaner, bookingId)).canStartJourney === false && unpaidCalls.join(",") === "context", "An unpaid confirmed booking did not fail closed before requesting Cleaner location.");
...
assert(await rejects(() => service.getJourneyReadiness(landlord, bookingId), "Cleaner"), "A Landlord could inspect the Cleaner-only journey readiness endpoint.");
