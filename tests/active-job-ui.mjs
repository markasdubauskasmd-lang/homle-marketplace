...
assert(script.includes('"X-CSRF-Token"') && !script.includes("innerHTML"), "Active-job mutations lost CSRF/session protection or introduced unsafe HTML rendering.");
assert(!/(google|mapbox|openstreetmap|leaflet)/i.test(script) && html.includes("When Google routing is enabled") && html.includes("current and destination points"), "The active-job client contacts a map provider directly or fails to disclose the approved server-side Google routing boundary.");
