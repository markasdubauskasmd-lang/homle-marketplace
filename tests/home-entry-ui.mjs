...
for (const mode of ["account", "authentication", "concierge"]) {
  ...
  assert(presentation.cleanerPath === "/cleaner/onboarding", `${mode} mode does not open the dedicated Cleaner onboarding journey.`);
...
assert(page.includes('href="/signup?intent=book" data-book-entry') && page.includes('href="/cleaner/onboarding" data-cleaner-entry'), "Homepage does not keep booking account-first while routing Cleaner applicants to dedicated onboarding.");
...
assert(page.includes('data-home-manual-entry') && page.includes('data-home-workspace-entry') && (page.match(/data-home-signed-out-only/g) || []).length >= 2, "The landing page cannot replace signed-out conversion prompts after session recovery.");
assert(script.includes("function applySignedInLanding(workspace)") && script.includes('"/landlord/dashboard#landlord-requests"') && script.includes("Continue to manual request") && script.includes("Open ${workspaceLabel} dashboard") && script.includes("element.hidden = signedIn"), "Signed-in visitors still see sign-up/login loops instead of their existing workspace actions.");
...
assert(script.includes('signedInWorkspace?.role === "landlord" ? "/landlord/book"') && script.includes("link.href = presentation.cleanerPath"), "Homepage role links do not preserve Landlord booking or dedicated Cleaner onboarding.");
...
assert(server.includes('"/landlord/dashboard": "landlord-dashboard.html"') && server.includes('"/cleaner/dashboard": "cleaner-dashboard.html"') && server.includes('"/cleaner/onboarding": "cleaner-registration.html"'), "Current role dashboards or dedicated Cleaner onboarding are not served.");
