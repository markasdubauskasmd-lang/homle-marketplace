...
const cleanerSignup = await dispatch(router, "POST", "/api/marketplace/auth/signup", { email: "cleaner@example.com", username: "sparkle-cleaner", password: "long Cleaner password", intent: "work" }, publicHeaders);
const cleanerRegisterCall = calls.filter((call) => call.kind === "register").at(-1);
const cleanerSignupDelivery = deliveries.at(-1);
assert(cleanerSignup.response.statusCode === 202 && cleanerRegisterCall.input.accountType === "cleaner" && cleanerRegisterCall.input.username === "sparkle-cleaner" && cleanerRegisterCall.input.displayName === "sparkle-cleaner" && cleanerSignupDelivery.link.endsWith("&intent=work"), "Cleaner signup did not preserve its username, select the Cleaner-only database transaction or retain the onboarding destination.");
