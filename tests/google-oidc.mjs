...
assert(accountScript.includes('post("/api/marketplace/auth/onboarding-session", {})') && accountScript.includes("storedCsrf() || await recoverOnboardingCsrf()"), "A refreshed Google onboarding page still strands an authenticated role-pending account when its one-time browser setup token is gone.");
