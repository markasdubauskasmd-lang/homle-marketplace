...
assert(accountPage.includes("data-account-ready") && accountScript.includes('fetch("/api/marketplace/account"') && accountScript.includes('return destination && !workspaceReady ? "/account-ready" : destination') && accountScript.includes("accountReadyPresentation(result.account, workspaceReady)") && accountScript.includes('fetch("/api/health"'), "Authentication-only social onboarding can still fall into a disabled dashboard or cannot verify the created role profile.");
