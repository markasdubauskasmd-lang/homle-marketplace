...
const linkAttempt = provider.begin({ purpose: "link" });
const linkLocation = new URL(linkAttempt.location);
...
const accountStyles = await readFile(new URL("../public/styles.css", import.meta.url), "utf8");
assert(accountPage.includes('<meta name="robots" content="noindex, nofollow">') && accountPage.includes('href="/privacy">Privacy notice</a>') && accountPage.includes('href="/terms">Pilot terms</a>') && !accountPage.includes("two routes that need no account"), "Account creation can be indexed, hides its governing terms or still describes a retired account-free journey.");
assert(accountPage.includes('data-social-actions hidden') && accountPage.includes('data-social-provider="google"') && accountPage.includes('data-social-provider="apple"') && accountPage.includes('data-social-provider="facebook"') && /data-social-provider="google"[^>]+hidden/.test(accountPage) && /data-social-provider="apple"[^>]+hidden/.test(accountPage) && /data-social-provider="facebook"[^>]+hidden/.test(accountPage), "The account page exposed a provider before capability discovery or omitted its gated controls.");
assert(accountScript.includes("providers[link.dataset.socialProvider] === true") && accountScript.indexOf("history.replaceState") < accountScript.indexOf('accountFetch("/api/auth/providers"') && accountScript.includes('fragment.get("csrfToken")') && accountScript.includes("storeCsrf(socialCsrfToken)"), "The account browser flow did not require an explicit provider capability or remove callback fragments before network activity.");
...
assert(accountScript.includes('post("/api/marketplace/auth/onboarding-session", {})') && accountScript.includes("signedInAccount?.roles?.length ? await recoverWorkspaceCsrf() : await recoverOnboardingCsrf()"), "A refreshed Google onboarding page still strands an authenticated account when its one-time browser setup token is gone.");
assert(accountScript.includes("const accountRequestTimeoutMs = 15_000") && accountScript.includes("controller.abort()") && accountScript.includes('selectedMode.form === "onboarding" && await openSignedInWorkspace()') && accountScript.includes('if (kind === "onboarding")') && accountScript.includes("if (await openSignedInWorkspace()) return"), "A slow or interrupted onboarding response can still leave the role button spinning forever or fail to resume an account that was already saved.");
assert(accountScript.includes("accountIntentWorkspaceActivation(account, accountIntent)") && accountScript.includes('post("/api/marketplace/auth/workspace", { role }, csrfToken)') && accountScript.includes('post("/api/marketplace/auth/session", {})') && accountScript.includes("signedInAccount = await activateIntendedWorkspace(result.account)"), "A work or booking sign-in intent redirects without securely activating the intended saved workspace first.");
...
assert(accountPage.includes("You can add the other marketplace workspace later") && accountScript.includes('location.assign("/onboarding?intent=book")') && accountScript.includes('location.assign("/onboarding?intent=work")') && accountScript.includes('signedInAccount?.roles?.length ? "/api/marketplace/auth/workspace" : "/api/marketplace/onboarding"') && accountScript.includes("same verified account"), "An existing one-role account cannot explicitly add its second workspace during the intended sign-in journey.");
assert(accountPage.includes("data-account-ready") && accountScript.includes('fetch("/api/marketplace/account"') && accountScript.includes("accountWorkspaceDestination(account, accountIntent, workspaceReady)") && accountScript.includes("accountReadyPresentation(result.account, workspaceReady)") && accountScript.includes('fetch("/api/health"'), "Authentication-only social onboarding can still fall into a disabled dashboard or cannot verify the created role profile.");
