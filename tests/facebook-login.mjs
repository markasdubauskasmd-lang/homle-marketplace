...
const cleaner = provider.begin({ intent: "work" });
const cleanerUrl = new URL(cleaner.location);
const cleanerClaims = await provider.complete(new URL(`${provider.callbackUrl}?code=cleaner-code&state=${encodeURIComponent(cleanerUrl.searchParams.get("state"))}`), cleaner.setCookie.split(";", 1)[0]);
assert.equal(cleanerClaims.flowIntent, "work");
