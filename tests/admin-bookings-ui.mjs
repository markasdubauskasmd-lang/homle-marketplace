...
const [page, script, migration, contributionMigration, grants, server] = await Promise.all([
...
assert(script.includes("/api/marketplace/admin/bookings") && script.includes("/matching-readiness") && script.includes("No Cleaner was contacted or invited") && script.includes("plannedContributionPence") && script.includes("Approved profit floors") && script.includes("targetContributionPence") && !script.includes("innerHTML"), "The operations UI lost the protected APIs, no-contact boundary, both frozen profit targets or safe DOM rendering.");
