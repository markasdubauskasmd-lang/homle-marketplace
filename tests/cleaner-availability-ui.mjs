...
const [page, script, requestJsonModule, styles, server, repository] = await Promise.all([
  ...
  readFile(new URL("../public/request-json.js", import.meta.url), "utf8"),
...
// The private-request guarantees now live in the shared owner rather than being
// inlined here, so they are asserted where they are implemented. Weakening this to
// "the module imports something" would stop guarding same-origin credentials at all.
assert(requestJsonModule.includes('credentials: "same-origin"') && requestJsonModule.includes('cache: "no-store"') && script.includes('createRequestJson({'), "The shared private request lost same-origin credentials or no-store, or availability stopped using it.");
// The token read moved to public/session-csrf.js and is asserted, along with every
// module that must delegate to it, in tests/private-request-boundary.mjs.
assert(script.includes('"X-CSRF-Token": csrf') && script.includes('method: "DELETE"') && !script.includes("innerHTML") && !script.includes("localStorage"), "Availability lost authenticated CSRF protection, safe rendering or its private non-persistent boundary.");
